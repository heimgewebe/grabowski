...
class AgentCompetitionTests(unittest.TestCase):
    def test_runner_resolution_finds_manifest_bound_release_asset(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            module_file = (
                ...
                / "python3.10"
                / "site-packages"
                / "grabowski_agent_competition.py"
            )
    ...
    def test_runner_resolution_prefers_source_tree(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source_runner.write_text("# source runner\n", encoding="utf-8")
            release = Path(temporary) / "release"
            packaged_runner = (
                release / "inputs" / "tools" / "external_programming_candidate.py"
            )
