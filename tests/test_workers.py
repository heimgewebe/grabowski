...
const readOffCanvasFallbackScenario = scenario === 'read-off-canvas-fallback';
...
const readPaintContainmentFallbackScenario = scenario === 'read-paint-containment-fallback';
const readBackfaceHiddenFallbackScenario = scenario === 'read-backface-hidden-fallback';
  ...
  const makeDocument = (documentSpec) => {
    ...
    documentSpec.forEach((node, nodeIndex) => {
      ...
      layout.styles.push([
        ...
        stringIndex(node.contain || 'none'),
        stringIndex(node.backfaceVisibility || 'visible'),
      ]);
      layout.bounds.push(node.bounds || [10 + nodeIndex, 10 + nodeIndex, 100, 20]);
      layout.text.push(stringIndex(node.layoutText || ''));
    });
    const contentWidth = Math.max(1200, ...layout.bounds.map((box) => Number(box[0]) + Number(box[2])));
    const contentHeight = Math.max(800, ...layout.bounds.map((box) => Number(box[1]) + Number(box[3])));
    return {nodes, layout, contentWidth, contentHeight};
  };
...
function fallbackSnapshot() {
  ...
  if (readOffCanvasFallbackScenario) return makeSemanticSnapshot([
    {parent: -1, backendNodeId: 101, nodeName: 'BUTTON', attributes: ['aria-label', ' ']},
    {parent: 0, backendNodeId: 102, nodeName: 'SPAN'},
    {parent: 1, backendNodeId: 103, nodeName: '#text', layoutText: 'off-canvas-' + marker, bounds: [-10000, 20, 90, 20]},
    {parent: 0, backendNodeId: 104, nodeName: 'SPAN'},
    {parent: 3, backendNodeId: 105, nodeName: '#text', layoutText: 'Continue', bounds: [60, 18, 55, 15]},
  ]);
  ...
  if (readPaintContainmentFallbackScenario) return makeSemanticSnapshot([
    {parent: -1, backendNodeId: 100, nodeName: 'DIV', contain: 'paint', bounds: [10, 10, 40, 40]},
    {parent: 0, backendNodeId: 101, nodeName: 'BUTTON', attributes: ['aria-label', ' '], bounds: [80, 10, 80, 30]},
    {parent: 1, backendNodeId: 102, nodeName: '#text', layoutText: 'paint-contained-' + marker, bounds: [90, 15, 60, 20]},
  ]);
  if (readBackfaceHiddenFallbackScenario) return makeSemanticSnapshot([
    {parent: -1, backendNodeId: 101, nodeName: 'BUTTON', attributes: ['aria-label', ' '], backfaceVisibility: 'hidden', bounds: [10, 10, 100, 30]},
    {parent: 0, backendNodeId: 102, nodeName: '#text', layoutText: 'backface-' + marker, bounds: [20, 15, 70, 20]},
  ]);
  ...
  send(raw) {
    ...
    switch (request.method) {
      ...
      case 'DOMSnapshot.captureSnapshot': {
        const expectedStyles = ['visibility', 'opacity', 'content-visibility', 'filter', 'clip-path', 'clip', 'overflow-x', 'overflow-y', 'color', '-webkit-text-fill-color', 'mask-image', 'contain', 'backface-visibility'];
      ...
      case 'DOM.describeNode': {
        ...
        if (readAncestorOpacityFallbackScenario || readAncestorFilterOpacityFallbackScenario || readUrlFilterFallbackScenario || readAncestorAriaHiddenFallbackScenario || readAncestorInertFallbackScenario || readTextHeavyPageFallbackScenario || readLongUnrelatedTextFallbackScenario || readMeterRoleFallbackScenario || readTransformZeroFallbackScenario || readTransformZeroDescendantFallbackScenario || readOffCanvasFallbackScenario || readClipPathFallbackScenario || readMaskImageFallbackScenario || readPaintContainmentFallbackScenario || readBackfaceHiddenFallbackScenario || readEmbeddingOpacityFallbackScenario || readOverflowClippedFallbackScenario || readOverflowContainedFallbackScenario || readTransparentColorFallbackScenario || readTransparentFillFallbackScenario || readSvgTextFallbackScenario || readVisibilityOverrideFallbackScenario || readDesignModeFallbackScenario || readChildDesignModeFallbackScenario) {
          reply({node: {
            backendNodeId: 101,
            nodeType: 1,
            localName: 'button',
...
""",
    ...
    def test_browser_semantic_node_revalidates_element_without_public_selector(self) -> None:
        ...
        self.assertIn("computedStyles: ['visibility', 'opacity', 'content-visibility', 'filter', 'clip-path', 'clip', 'overflow-x', 'overflow-y', 'color', '-webkit-text-fill-color', 'mask-image', 'contain', 'backface-visibility']", source)
        self.assertIn("function semanticPaintContainment", source)
        ...
        self.assertIn("function semanticLayoutBoundsWithinDocument", source)
        self.assertIn("document.contentWidth", source)
        self.assertIn("document.contentHeight", source)
    ...
    def test_browser_semantic_node_excludes_off_canvas_descendant_text(self) -> None:
        execution, receipt = self._run_browser_semantic_node("read-off-canvas-fallback")
        self.assertEqual(execution.returncode, 0, execution.stderr)
        self.assertTrue(receipt["ok"])
        element = receipt["state"]["elements"][0]
        self.assertEqual(element["name"], "Continue")
    ...
    def test_browser_semantic_node_excludes_paint_containment_clipped_text(self) -> None:
        execution, receipt = self._run_browser_semantic_node(
            "read-paint-containment-fallback"
        )
        self.assertEqual(execution.returncode, 0, execution.stderr)
        self.assertTrue(receipt["ok"])
        self.assertEqual(receipt["state"]["elements"][0]["name"], "")

    def test_browser_semantic_node_excludes_backface_hidden_text(self) -> None:
        execution, receipt = self._run_browser_semantic_node(
            "read-backface-hidden-fallback"
        )
        self.assertEqual(execution.returncode, 0, execution.stderr)
        self.assertTrue(receipt["ok"])
        self.assertEqual(receipt["state"]["elements"][0]["name"], "")
    ...
    def test_browser_semantic_gateway_observe_exposes_bounded_name_but_audit_does_not(self) -> None:
        ...
        self.assertEqual(bounded["admission"], "fail_closed")
        self.assertEqual(
    ...
    def test_browser_semantic_maulwurfx_submit_stays_fail_closed_until_contract_promotion(self) -> None:
        worker = self._running_browser(port=9395)
        textbox = {
            "backend_node_id": "101",
            "role": "textbox",
    ...
    def test_browser_semantic_maulwurfx_submit_is_fixed_bounded_and_ambiguous(self) -> None:
        ...
        with patch.object(
            workers,
            "BROWSER_EFFECT_CLASSES_IMPLEMENTED",
            frozenset({*workers.BROWSER_EFFECT_CLASSES_IMPLEMENTED, "bounded_external_submit"}),
        ), patch.object(
            workers, "_observe", return_value=self._running_observation()
        ), patch.object(
    ...
    def test_browser_semantic_maulwurfx_node_guards_empty_textbox_before_submit(self) -> None:
        ...
        insert_index = section.index("Input.insertText")
        focus_recheck_index = section.index("document.activeElement === this", insert_index)
        dispatch_index = section.index("Input.dispatchKeyEvent")
        self.assertLess(insert_index, focus_recheck_index)
        self.assertLess(focus_recheck_index, dispatch_index)
