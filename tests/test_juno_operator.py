...
class ModelTests(unittest.TestCase):
    def test_overall_status_is_fail_closed(self) -> None:
        ...
        self.assertEqual(
            models.Snapshot(1, models.utc_now(), (healthy,)).overall_status,
            "healthy",
        )
        self.assertEqual(
            models.Snapshot(1, models.utc_now(), (healthy, unknown)).overall_status,
            "unknown",
        )
...
class PathTests(unittest.TestCase):
    def test_document_ancestor_uses_name_not_container_uuid(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(
                roots,
                [("Juno-Dokumente", project.parent.resolve())],
            )

    def test_local_catalog_accepts_bounded_document_provider_path(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            config = Path(temporary) / "storage-roots.local.json"
            configured_path = (
                "/private/var/mobile/Containers/Shared/AppGroup/ABC/"
            ...
            self.assertEqual(
                paths.load_local_storage_roots(config),
                [("Auf meinem iPad", Path(configured_path))],
            )

    def test_local_catalog_rejects_path_outside_document_domains(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            config = Path(temporary) / "storage-roots.local.json"
            config.write_text(
                json.dumps(
...
class CollectorTests(unittest.TestCase):
    ...
    def test_invalid_local_catalog_is_visible_and_fail_closed(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary) / "Documents"
            project = root / "Juno Operator"
            project.mkdir(parents=True)
            config = project / "storage-roots.local.json"
            config.write_text(
                '{"schema_version":1,"roots":[{"label":"x","path":"/etc"}]}',
                encoding="utf-8",
            )
            result = collectors.collect_storage(
                project,
...
class AppTests(unittest.TestCase):
    def _project(self, root: Path) -> Path:
        ...
        (project / "config" / "targets.json").write_text(
    ...
    def test_incident_is_non_overwriting_and_hash_bound(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            checksums = json.loads(
                (incident / "checksums.json").read_text(encoding="utf-8")
            )
            self.assertEqual(
                set(checksums),
                {"dashboard.html", "snapshot.json", "summary.md"},
            )
