...
class DurableSystemdContractTests(unittest.TestCase):
    def test_operator_unit_is_loopback_only_and_independent(self) -> None:
        ...
        self.assertIn(
            "Environment=PATH=/home/alex/.local/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/snap/bin", text
        )
    ...
    def test_tunnel_restart_follows_signed_ingress_without_failure_binding(self) -> None:
        text = (
            ROOT
            / "systemd"
        ...
        self.assertIn("Wants=grabowski-transport-ingress.service", text)
        self.assertIn("After=grabowski-transport-ingress.service", text)
