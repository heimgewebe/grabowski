...
def fake_remote_materialize_operator(
    ...
    *,
    ...
    module._git_branch_preimage = Mock(
        side_effect=lambda _repo: {
            "branch": fake_git.branch,
            "head": fake_git.head,
    ...
    def mutate(
        ...
        branch_attempt: dict[str, object] | None = None,
    ) -> dict[str, object]:
        ...
        if mutation_status == "completed":
            ...
            return {
                ...
                "branch_mutation": {
                    "status": "completed",
                    "post_head": fake_git.materialize_remote_head,
                    "preimage_sha256": branch_attempt["expected_preimage_sha256"],
        ...
        return {
            ...
            "branch_mutation": {
                ...
                "post_head": fake_git.head,
                "receipt_sha256": "c" * 64,
            },
        }

    module.grabowski_git = Mock(side_effect=mutate)
    return module
...
class FakeGh:
    ...
    def __call__(self, repo: Path, argv: list[str]) -> dict[str, object]:
        ...
        if argv[:2] == ["api", "graphql"]:
            query = next(
                (item for item in argv if isinstance(item, str) and item.startswith("query=")),
                "",
            )
            if "mergeQueueEntry" in query:
                if self.merge_queue_failure:
                    return {"returncode": 1, "stdout": "", "stderr": "queue readback failed"}
                queue_view = {
                    "state": self.view.get("state"),
                    "headRefOid": self.view.get("headRefOid"),
                    "baseRefName": self.view.get("baseRefName"),
                    "baseRefOid": self.view.get("baseRefOid"),
        ...
        if argv[:2] == ["pr", "merge"]:
            ...
            if self.post_merge_queue_entry is not None:
                self.merge_queue_entry = deepcopy(self.post_merge_queue_entry)
    ...
    def __call__(self, repo: Path, argv: list[str]) -> dict[str, object]:
        ...
        if argv[:1] == ["api"]:
            ...
            if "/compare/" in endpoint:
                ...
                status = (
                    "ahead"
                    if contains_expected_base or contains_prior_head
                    else "diverged"
                )
...
class GripFoundationTests(unittest.TestCase):
    ...
    def test_checkout_binding_terminal_preview_grip_is_read_only_and_delegates(self) -> None:
        ...
        preview = {
            ...
            "safe_to_apply": True,
            "source_evidence": {"evidence_sha256": "b" * 64},
            "coordination": {"blocking": False},
            "preview_sha256": "c" * 64,
            "preview_created_at_unix": 100,
            "preview_expires_at_unix": 160,
        }
        with patch(
    ...
    def test_checkout_binding_terminal_apply_grip_requires_mutation_and_exact_cas(self) -> None:
        ...
        with patch(
            ...
            blocked = grips.grip_run(
                "checkout-binding-terminal-apply", parameters
            )
        ...
        self.assertEqual("passed", result["status"])
        terminal_apply.assert_called_once_with(
            checkout_key,
            "owner-a",
            expected_preview,
            100,
            "reconcile-terminal-missing-checkout",
        )
    ...
    def test_checkout_binding_terminal_apply_recovers_present_receipt_from_missing_followup_preview(self) -> None:
        checkout_key = "a" * 64
        expected_preview = "c" * 64
        terminal_receipt = {
            "checkout_key": checkout_key,
        ...
        with patch(
            ...
            return_value={
                ...
                "supersedes_reconciliation_receipt": terminal_receipt,
            },
        ) as terminal_preview:
            result = grips.grip_run(
                "checkout-binding-terminal-apply",
    ...
    def test_saga_orchestration_rejects_pr_merge_receipt_without_complete_audit(self) -> None:
        plan = sagas.build_plan(
            "pr-settlement", _saga_pr_target(), "t121-audit-binding-missing-audit"
        )
        expected = plan["expected_identity"]
        output = {"decision": "executed", "actions": [], "executions": []}
        receipt = {
        ...
        with self.assertRaisesRegex(
            grip_orchestration.SagaError,
            "lacks complete audit evidence for pr-merge",
        ):
            grip_orchestration.validate_captain_audit_binding(
                binding, plan_value=plan, captain_result_value=captain
            )

    def test_saga_orchestration_rejects_mixed_receipt_and_audit_identity(self) -> None:
        plan = sagas.build_plan(
            "pr-settlement", _saga_pr_target(), "t121-audit-binding-mixed-identity"
        )
        provenance = {
            "provenance_schema_version": 2,
            "execution_invoked": True,
        ...
        body = {
            ...
            "kind": grip_orchestration.CAPTAIN_AUDIT_BINDING_KIND,
            "authority": "verified_grabowski_audit_chain",
            "intent_record_sha256": trusted["intent_record_sha256"],
            "completion_record_sha256": trusted["completion_record_sha256"],
            "action": trusted["action"],
            "target_sha256": trusted["target_sha256"],
            "expected_head": trusted["expected_head"],
            "expected_base": trusted["expected_base"],
        ...
        with (
            ...
            self.assertRaisesRegex(
                grip_orchestration.SagaError,
                "receipt identity differs from verified audit evidence",
            ),
        ):
            grip_orchestration.validate_captain_audit_binding(
                binding, plan_value=plan, captain_result_value=captain
    ...
    def test_remote_head_materialize_fast_forwards_exact_lane_via_branch_cas(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
        ...
        self.assertEqual("passed", result["receipt"]["status"])
        self.assertEqual("fast_forwarded", result["output"]["action"])
        ...
        mutation = operator.grabowski_git.call_args
        self.assertEqual(["merge", "--ff-only", "b" * 40], mutation.args[1])
        attempt = mutation.kwargs["branch_attempt"]
        self.assertEqual("lane:" + "1" * 32, attempt["owner_id"])
        self.assertEqual("writer", attempt["branch"])
        self.assertEqual("e" * 64, attempt["expected_preimage_sha256"])
        checks = {
            item["id"]: item["status"]
            for item in result["receipt"]["checks"]
        }
    ...
    def test_pr_base_converge_updates_existing_pr_with_exact_base_head_cas(
        self,
    ) -> None:
        with (
        ...
        self.assertEqual("a" * 40, result["output"]["old_head"])
        self.assertEqual("b" * 40, result["output"]["new_head"])
        self.assertEqual("pushed_and_read_back", result["output"]["cas_status"])
        kwargs = cas.call_args.kwargs
        self.assertEqual("e" * 40, kwargs["base_sha"])
        self.assertEqual("a" * 40, kwargs["head_sha"])
        self.assertEqual("feat/work", kwargs["head_branch"])
        self.assertEqual(77, kwargs["pr_number"])
    ...
    def test_pr_base_converge_is_noop_when_head_already_contains_base(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            gh = AlreadyCurrent()
            with patch.object(merge_guard, "_exact_base_content_git_head_cas_update_pr_head") as cas:
                result = grips.run_grip(
                    "pr-base-converge",
                    {
                        "repo": tmp,
    ...
    def test_pr_base_converge_rejects_concurrent_head_replacement_after_update(
        self,
    ) -> None:
        with (
            tempfile.TemporaryDirectory() as tmp,
    ...
    def test_pr_base_converge_revalidates_live_head_after_ancestry_checks(self) -> None:
        class DriftAfterAncestry(FakePrBaseConvergeGh):
            ...
            def __call__(self, repo: Path, argv: list[str]) -> dict[str, object]:
                ...
                if argv[:2] == ["pr", "view"]:
                    ...
                    if (
                        self.updated
                        and self.compare_reads >= 3
                        and self.view_reads >= 3
                    ):
        ...
        self.assertEqual("failed", result["receipt"]["status"])
        self.assertIn(
            "drifted after exact-base ancestry verification", result["output"]["error"]
        )
        checks = {item["id"]: item["status"] for item in result["receipt"]["checks"]}
        self.assertEqual("pass", checks["base_contained_after"])
        self.assertEqual("fail", checks["final_pr_binding"])

    def test_pr_base_converge_known_head_effect_with_concurrent_base_advance_requires_fresh_same_pr_convergence(
        self,
    ) -> None:
        ...
        self.assertEqual("failed", result["receipt"]["status"])
        self.assertIn(
            "base advanced after exact-base head CAS", result["output"]["error"]
        )
    ...
    def test_pr_base_converge_unknown_cas_outcome_forbids_replay_or_successor(
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(
                ...
                "_exact_base_content_git_head_cas_update_pr_head",
                side_effect=fake_pr_base_converge_cas(
                    gh, status="outcome_unknown", returncode=2
                ),
            ) as cas:
                result = grips.run_grip(
                    "pr-base-converge",
        ...
        self.assertEqual("failed", result["receipt"]["status"])
        self.assertIn("outcome is unknown", result["output"]["error"])
        self.assertEqual(1, cas.call_count)
        self.assertFalse(any(call[:2] == ("pr", "create") for call in gh.calls))
    ...
    def test_pr_base_converge_cas_rejection_requires_fresh_preflight(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            gh = FakePrBaseConvergeGh()
            with patch.object(
                merge_guard,
                "_exact_base_content_git_head_cas_update_pr_head",
                side_effect=fake_pr_base_converge_cas(
                    gh, status="not_applied_proven", returncode=1
                ),
            ) as cas:
                result = grips.run_grip(
                    "pr-base-converge",
        ...
        self.assertEqual("failed", result["receipt"]["status"])
        self.assertIn("proven not applied", result["output"]["error"])
        self.assertEqual(1, cas.call_count)
        self.assertFalse(any(call[:2] == ("pr", "create") for call in gh.calls))
...
class CaptainAuthorityPathTests(unittest.TestCase):
    ...
    def _exact_cas_success_evidence(self, **overrides: object) -> dict[str, object]:
        evidence: dict[str, object] = {
            ...
            "commit_identity": {"status": "resolved"},
    ...
    def _exact_cas_parameters(self) -> dict[str, object]:
        ...
        return authorized_captain_run_parameters(actions=[action])
    ...
    def test_captain_run_treats_exact_merge_queue_entry_as_scheduled_after_dispatch(self) -> None:
        ...
        gh = FakeGh(
            ...
            merge_stderr="The merge strategy for main is set by the merge queue",
            post_merge_queue_entry={
                "id": "MQE_test",
                "position": 1,
                "estimatedTimeToMerge": 565,
            },
        )
        ...
        self.assertFalse(execution["merge_completion_verified"])
    ...
    def test_captain_run_blocks_mismatched_existing_merge_queue_binding(self) -> None:
        ...
        gh = FakeGh(
            ...
            merge_queue_entry={"id": "MQE_drift", "position": 1},
            merge_queue_view_overrides={"headRefOid": "f" * 40},
        )
        gh.active_rules.append(
            {
                "type": "merge_queue",
    ...
    def test_exact_base_cas_reconciles_closed_pr_from_atomic_readback(self) -> None:
        ...
        self.assertEqual(
            ...
            execution["merge_reconciliation"],
        )
        reconciliation = execution["post_merge_reconciliation"]
        self.assertEqual(
            "verified_base_mutation_pr_metadata_unsettled",
            reconciliation["status"],
        )
        self.assertFalse(reconciliation["github_pr_merged_metadata_verified"])
        self.assertTrue(reconciliation["base_ref_readback_proven"])
        self.assertTrue(reconciliation["head_branch_delete_readback_proven"])
        self.assertFalse(reconciliation["duplicate_dispatch_permitted"])
        self.assertEqual("CLOSED", execution["verified_pr"]["state"])
        guard = execution["merge_lease_guard"]
        self.assertTrue(guard["post_merge_verification_passed"])
        self.assertEqual("pushed_and_read_back", guard["exact_base_git_cas_merge"]["status"])

    def test_exact_base_cas_closed_pr_reconciliation_fails_without_base_readback_proof(self) -> None:
        parameters = self._exact_cas_parameters()
        gh = FakeGh(
            view={
                "number": 96,
        ...
        def fake_exact_cas(*args, on_dispatch=None, **kwargs):
            ...
            return (
                ...
                self._exact_cas_success_evidence(base_ref_readback_proven=False),
            )
        ...
        execution = result["output"]["executions"][0]
        self.assertFalse(execution["verification_passed"])
        self.assertEqual("pr_not_merged_after_execution", execution["verification_error"])
        reconciliation = execution["exact_base_git_cas_closed_pr_reconciliation"]
        self.assertEqual("rejected", reconciliation["status"])
        self.assertIn(
            "exact_base_cas_reconciliation_receipt_mismatch:base_ref_readback_proven",
            reconciliation["errors"],
        )

    def test_exact_base_cas_closed_pr_reconciliation_fails_on_head_branch_mismatch(self) -> None:
        parameters = self._exact_cas_parameters()
        gh = FakeGh(
            view={
                "number": 96,
        ...
        def fake_exact_cas(*args, on_dispatch=None, **kwargs):
            ...
            return (
                ...
                self._exact_cas_success_evidence(head_branch="feat/other"),
            )
        ...
        self.assertEqual("rejected", reconciliation["status"])
        self.assertIn(
            "exact_base_cas_reconciliation_receipt_mismatch:head_branch",
            reconciliation["errors"],
        )

    def test_exact_base_cas_closed_pr_reconciliation_requires_post_view(self) -> None:
        parameters = self._exact_cas_parameters()
        ...
        runner.dispatch_called = True
        runner.receipt["dispatch_called"] = True
        runner.receipt["dispatch_mode"] = "exact_base_git_cas"
        runner.receipt["exact_base_git_cas_merge"] = self._exact_cas_success_evidence()
        closed_view = {
            "number": 96,
            "state": "CLOSED",
            "mergedAt": None,
        ...
        reconciliation, errors = runner.reconcile_exact_base_git_cas_closed_pr(
            viewed=closed_view,
            verify_errors=["pr_not_merged_after_execution"],
            queue_errors=["merge_queue_pr_state_not_open"],
            repo_slug="heimgewebe/grabowski",
            pr_number=96,
            expected_head=CAPTAIN_HEAD,
            expected_base="main",
    ...
    def test_exact_base_cas_closed_pr_reconciliation_rejects_authority_binding_drift(self) -> None:
        parameters = self._exact_cas_parameters()
        runner = merge_guard.CaptainMergeGuardRunner(
            repo_path=Path.cwd(),
            action=parameters["actions"][0],
        ...
        runner.receipt["dispatch_mode"] = "exact_base_git_cas"
        runner.receipt["exact_base_git_cas_merge"] = self._exact_cas_success_evidence()
        runner.receipt["exact_base_git_cas_post_view"] = {
            "returncode": 0,
            "expected_merge_sha": "b" * 40,
            "matched": False,
        }
        closed_view = {
            "number": 96,
        ...
        cases = (
            (
                "pr-number",
                {"number": 97},
                ["merge_queue_pr_state_not_open"],
                "exact_base_cas_reconciliation_pr_number_mismatch",
            ),
            (
                "base-ref",
                {"baseRefName": "release"},
                ["merge_queue_pr_state_not_open"],
                "exact_base_cas_reconciliation_pr_base_mismatch",
            ),
            (
                "base-sha",
                {"baseRefOid": "f" * 40},
                ["merge_queue_pr_state_not_open"],
                "exact_base_cas_reconciliation_pr_base_sha_mismatch",
            ),
            (
                "head-repository",
                {"headRepository": {"nameWithOwner": "heimgewebe/other"}},
                ["merge_queue_pr_state_not_open"],
                "exact_base_cas_reconciliation_head_repository_mismatch",
            ),
            (
                "cross-repository",
                {"isCrossRepository": True},
                ["merge_queue_pr_state_not_open"],
                "exact_base_cas_reconciliation_cross_repository_state_invalid",
            ),
            (
                "unexpected-queue-error",
                {},
                ["merge_queue_pr_state_not_open", "merge_queue_base_mismatch"],
                "exact_base_cas_reconciliation_queue_error:merge_queue_base_mismatch",
            ),
        )

        for label, view_overrides, queue_errors, expected_error in cases:
            with self.subTest(label=label):
                viewed = dict(closed_view)
                viewed.update(view_overrides)
                reconciliation, errors = runner.reconcile_exact_base_git_cas_closed_pr(
                    viewed=viewed,
                    verify_errors=["pr_not_merged_after_execution"],
    ...
    def test_exact_base_cas_reconciliation_does_not_accept_open_pr_metadata(self) -> None:
        ...
        self.assertEqual("rejected", reconciliation["status"])
        self.assertIn(
            "exact_base_cas_reconciliation_pr_not_closed",
            reconciliation["errors"],
        )

    def test_exact_base_cas_closed_pr_reconciliation_fails_on_head_drift(self) -> None:
        parameters = self._exact_cas_parameters()
        gh = FakeGh(
            view={
                "number": 96,
        ...
        self.assertEqual("rejected", reconciliation["status"])
        self.assertIn(
            "exact_base_cas_reconciliation_pr_head_mismatch",
            reconciliation["errors"],
        )
        self.assertIn(
            "exact_base_cas_reconciliation_queue_error:merge_queue_head_mismatch",
            reconciliation["errors"],
        )
    ...
    def test_atomic_merge_guard_reconciles_proven_raw_and_canonical_diff_aliases(self) -> None:
        ...
        previous_canonical_diff_sha256 = merge_guard.github_pr_diff_identity_sha256_v1(
            diff_text.encode("utf-8")
        )
    ...
    def test_atomic_merge_guard_accepts_previous_canonical_diff_identity(self) -> None:
        diff_text = (
            "diff --git a/src/changed.py b/src/changed.py\n"
            "index 123456789abcdef..abcdef0123456789 100644\n"
            "--- a/src/changed.py\n"
            "+++ b/src/changed.py\n"
            "@@ -1 +1 @@ def render(value):\n"
            "-old\n"
            "+new\n"
        )
        raw_diff_sha256 = hashlib.sha256(diff_text.encode("utf-8")).hexdigest()
        ...
        self.assertNotEqual(raw_diff_sha256, previous_canonical_diff_sha256)
        self.assertNotEqual(previous_canonical_diff_sha256, canonical_diff_sha256)
        parameters = authorized_captain_run_parameters(
            diff_sha256=previous_canonical_diff_sha256,
            codex_review_evidence=captain_codex_review_evidence(
                diff_sha256=previous_canonical_diff_sha256
            ),
        )
        review_evidence = parameters["review_evidence"]
        assert isinstance(review_evidence, dict)
        review_evidence["diff_sha256"] = previous_canonical_diff_sha256
        parameters["execution_intent"] = captain_execution_intent(parameters)
        view = {
            "number": 96,
            "state": "OPEN",
        ...
        codex_state = captain_codex_live_state(
            view,
            diff_text=diff_text,
            request_pages=[[request_comment]],
        )
        codex_state["request_comment"] = request_comment
        gh = FakeGh(
            view=view,
            diff_text=diff_text,
    ...
    def test_atomic_merge_guard_hashes_raw_diff_bytes_without_newline_translation(self) -> None:
        ...
        parameters["codex_review_evidence"] = captain_codex_review_evidence(
            diff_sha256=parameters["diff_sha256"]
        )
        parameters["execution_intent"] = captain_execution_intent(parameters)
        gh = FakeGh(view={
            "number": 96, "state": "OPEN", "baseRefName": "main",
    ...
    def test_atomic_merge_guard_external_merge_does_not_manufacture_compliance(self) -> None:
        ...
        self.assertTrue(reconciliation["external_merge_observed"])
        self.assertFalse(reconciliation["dispatch_called"])
        self.assertTrue(reconciliation["duplicate_dispatch_prevented"])
        self.assertNotIn("pre_merge_diff_delivery_required", reconciliation)
        self.assertEqual(
    ...
    def test_user_visible_diff_delivery_is_not_a_merge_gate(self) -> None:
        parameters = captain_parameters()
        self.assertNotIn("merge_delivery_receipt", parameters)
        self.assertNotIn("merge_delivery_receipt_sha256", parameters)

        result = self.run_captain(parameters)

        gate_ids = [gate["id"] for gate in result["output"]["gates"]]
        self.assertNotIn("diff-delivery-recorded", gate_ids)
        self.assertEqual("pass", self.gate(result, "diff-bound")["status"])
        self.assertTrue(result["output"]["manual_decision_candidate"])
