...
def _validate_policy(policy: Any) -> None:
    ...
    if "forbidden_capabilities" in policy:
        _validate_capability_list(policy["forbidden_capabilities"], label="forbidden_capabilities")
    ...
    for name, profile in profiles.items():
        ...
        if "capabilities" in profile:
            ...
            if "secret_export" in capabilities and not (
                _profile_root_values(policy, profile, "secret_export_roots")
            ):
                raise RuntimeError(
                    f"Access profile {name} enables secret_export without "
                    "secret_export_roots"
                )
            if "browser_profile_read" in capabilities and not (
                _profile_root_values(policy, profile, "browser_profile_roots")
            ):
                raise RuntimeError(
...
def _display_path(path: Path) -> str:
    return _redact_sensitive_text(str(path))[0]
...
def _secret_use_environment(
    ...
    for key, value in extra_environment.items():
        ...
        if any(secret and secret in value for secret in redaction_values):
            raise PermissionError("Secret value or encoded secret value may not be placed in environment")
        environment[key] = value
    return environment
...
def _read_limited_process_pipes(
    ...
    *,
    ...
    while selector.get_map():
        ...
        if process.poll() is not None:
            # Drain final EOF notifications without admitting more than the cap.
            continue
...
@mcp.tool(name="grabowski_secret_reveal", annotations=READ_ANNOTATIONS)
def grabowski_secret_reveal(
    ...
    _append_audit({
        ...
        "rollback": {
            "available": False,
            "reason": "secret reveal is read-only",
            "created_sha256": None,
        },
    })
    return {
...
def grabowski_secret_use(
    ...
    if (
        ...
        raise ValueError(
            f"timeout_seconds must be between 1 and {hard_timeout}"
        )
    if (
        ...
        raise ValueError(
            f"max_output_bytes must be between 1 and {hard_output}"
        )

    working_directory = _resolve_secret_use_cwd(cwd)
    command_template = _validate_secret_use_argv(argv, cwd=working_directory, secret_data=snapshot["data"])
