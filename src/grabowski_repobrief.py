...
def sidecar_path(manifest: dict[str, Any], manifest_path: Path, role: str) -> str | None:
    ...
    bundle_path = _resolve_bounded(manifest_base, bundle["path"], kind="bundleManifest")
    bundle_base = bundle_path.parent.resolve()
    for artifact in manifest.get("artifacts", []):
        if isinstance(artifact, dict) and artifact.get("role") == role and isinstance(artifact.get("path"), str):
            return str(_resolve_bounded(bundle_base, artifact["path"], kind=f"artifact:{role}"))
    return None
...
def _freshness_status(snapshot_commit: object, current_head: object) -> str:
    if not isinstance(snapshot_commit, str) or not snapshot_commit:
        return "unknown"
    return "exact" if snapshot_commit == current_head else "baseline"

def context(repo: Path, runner: CommandRunner, orientation: dict[str, Any], parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    if raw_root is None:
        ...
        return unavailable("invalid_parameter", reason="repobrief_publication_root must be a non-empty string")
    if not publication_root.exists():
        return unavailable("missing_publication_root", publication_root=str(publication_root))

    remote_result = optional_git(repo, runner, ["remote", "get-url", "origin"])
    remote = str(remote_result.get("stdout") or "").strip() if remote_result.get("returncode") == 0 else None
    repo_segment = repo_from_remote(remote, str(orientation.get("root") or repo))
    if not repo_segment:
        return unavailable("unresolved_repository", reason="could not derive RepoBrief repository segment")
    if repo_segment in EXCLUDED_REPOSITORIES:
        return unavailable(
            "excluded",
            repository=repo_segment,
            reason="repository is intentionally excluded from RepoBrief fleet publication",
        )

    searched: list[str] = []
    for ref in ref_candidates(repo, runner, orientation):
        manifest_path = publication_root / "external" / "repobrief" / repo_segment / ref / "manifest.json"
        searched.append(str(manifest_path))
        manifest, error = read_manifest(manifest_path)
        if error is not None:
            if error["status"] == "missing":
                continue
            return {**error, "repository": repo_segment, "ref": ref}
        ...
        try:
            ...
            if isinstance(bundle, dict) and isinstance(bundle.get("path"), str):
                bundle_path = str(_resolve_bounded(manifest_path.parent.resolve(), bundle["path"], kind="bundleManifest"))
        ...
        if isinstance(provenance, dict) and isinstance(provenance.get("repositories"), list):
            repositories = [item for item in provenance["repositories"] if isinstance(item, dict)]
        ...
        return {
            ...
            "current_head_matches_snapshot": freshness == "exact",
    ...
    return unavailable(
        ...
        reason="no published RepoBrief manifest found for known default refs",
    )
