...
def _route_decision_v2(input_facts: dict[str, Any]) -> dict[str, Any]:
    ...
    critical_risk = bool(set(risk_flags) & critical_flags)
    r3_scale = bool(kind == "code" and novelty == "high" and duration >= 120 and changed_files >= 8)
    if trivial_work:
        risk_tier = "R0"
    elif critical_risk or r3_scale:
        risk_tier = "R3"
    elif (
        (kind == "code" and (changed_files >= 7 or duration >= 90 or novelty == "high" or bool(risk_flags) or connector_instability or concurrent_activity))
        or (kind == "operations" and (duration >= 90 or bool(risk_flags) or connector_instability or concurrent_activity))
        or (kind == "analysis" and novelty == "high" and duration >= 90)
    ):
        risk_tier = "R2"
    else:
        ...
        candidates = [
            {
                "provider": external_available[0],
                "mode": "competitor",
                "timing": "after_direct_operator_plan",
            },
            {
                "provider": external_available[1],
                "mode": "contrast",
                "timing": "after_direct_operator_candidate",
            },
        ]
    elif contrast_eligible:
        candidates = [
    ...
    parallel_writer_pilot = {
        ...
        "minimum_estimated_minutes_per_shard": 0,
        "assessment_blockers": [
            "direct-first policy forbids external authoritative writer shards"
        ] if parallelization_candidate else [],
        "implementation_blockers": [
            "external agents are limited to advisory review or contrast",
            "ChatGPT/Grabowski remains the only authoritative writer and integrator",
        ],
    }
    return {
        "score": score,
        ...
        "execution_mode": "direct_operator",
        "external_candidates": candidates,
        "design_space": design_space,
        "trivial_work": trivial_work,
        "parallel_writer_pilot": parallel_writer_pilot,
...
def _tmux_exact_session_identity(session: str) -> dict[str, Any] | None:
    ...
    if len(parts) != 2:
        raise AgentWorkspaceActionError("tmux exact session identity is invalid")
    session_id, session_created = parts
    if (
...
def _valid_tmux_session_identity(value: object) -> bool:
    return (
        ...
        and set(value) == {"session_id", "session_created"}
        and isinstance(value.get("session_id"), str)
        and str(value["session_id"]).startswith("$")
        and str(value["session_id"])[1:].isdigit()
        and isinstance(value.get("session_created"), int)
        and not isinstance(value.get("session_created"), bool)
        and int(value["session_created"]) >= 0
    )
...
def _candidate_round(manifest: dict[str, Any]) -> int:
    value = manifest.get("candidate_round", 1)
    if isinstance(value, bool) or not isinstance(value, int) or value not in {1, 2}:
        raise AgentWorkspaceError("candidate_round must be one or two")
    return value


def _role_receipt_path(
    manifest: dict[str, Any],
    role: str,
    *,
    attempt: int = 1,
    ...
    round_number = _candidate_round(manifest) if candidate_round is None else candidate_round
    if (
        isinstance(round_number, bool)
        or not isinstance(round_number, int)
        or round_number not in {1, 2}
    ):
        raise AgentWorkspaceError("candidate receipt round must be one or two")
    if role in READ_ONLY_ROLES and round_number > 1:
        stem = f"{role}-receipt.round-{round_number:04d}"
        name = f"{stem}.json" if attempt == 1 else f"{stem}.attempt-{attempt}.json"
    else:
        name = (
            f"{role}-receipt.json"
            if attempt == 1
            else f"{role}-receipt.attempt-{attempt}.json"
        )
...
def _writer_patch_path(
    manifest: dict[str, Any], *, round_number: int | None = None
) -> Path:
    selected = _candidate_round(manifest) if round_number is None else round_number
    if isinstance(selected, bool) or not isinstance(selected, int) or selected not in {1, 2}:
        raise AgentWorkspaceError("writer patch round must be one or two")
    name = "writer.patch" if selected == 1 else f"writer-round-{selected:04d}.patch"
    return _workspace_dir(str(manifest["workspace_id"])) / name
...
def _writer_task_argv(
    ...
    *,
    ...
    dirty_preimage_sha256: str | None = None,
) -> list[str]:
    allowed_arguments = [
        value
    ...
    if dirty_preimage_sha256 is not None and SHA256_RE.fullmatch(dirty_preimage_sha256) is None:
        raise AgentWorkspaceError("writer dirty preimage binding is invalid")
    return [
        sys.executable,
        "-m",
        ...
        *(
            []
            if dirty_preimage_sha256 is None
            else ["--expected-dirty-preimage-sha256", dirty_preimage_sha256]
        ),
        "--output",
...
def _writer_attempts(manifest: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if len(attempts) == 2:
        ...
        if actor not in {"operator_handoff", "candidate_revision"}:
        ...
        if actor == "operator_handoff":
            ...
            if handoff.get("revision_preimage_sha256") is not None:
                raise AgentWorkspaceError("failure handoff may not carry revision preimage")
        else:
            if handoff.get("previous_state") != "completed":
                raise AgentWorkspaceError("candidate revision requires completed writer attempt one")
            if handoff.get("candidate_round") != 1 or handoff.get("next_candidate_round") != 2:
                raise AgentWorkspaceError("candidate revision round binding is invalid")
            for field in (
...
def _effective_writer_attempt(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    dirty_preimage_sha256 = selected.get("revision_preimage_sha256")
    if selected.get("actor") == "candidate_revision":
        if (
            not isinstance(dirty_preimage_sha256, str)
    ...
    argv = _writer_task_argv(
        ...
        dirty_preimage_sha256=dirty_preimage_sha256,
    )
...
def _collection_round_receipt_path(
    manifest: dict[str, Any], *, round_number: int
) -> Path:
    if isinstance(round_number, bool) or not isinstance(round_number, int) or round_number not in {1, 2}:
        raise AgentWorkspaceError("collection round must be one or two")
    return (
        _workspace_dir(str(manifest["workspace_id"]))
        / f"collection-round-{round_number:04d}.json"
    )
...
def _validate_collection_round_receipt(
    ...
    if not isinstance(value, dict) or value.get("state") != "complete":
        raise AgentWorkspaceError("revision collection archive must be complete")
    result_sha256 = value.get("result_sha256")
    if (
        not isinstance(result_sha256, str)
        or SHA256_RE.fullmatch(result_sha256) is None
        or _collection_result_sha256(value) != result_sha256
    ):
        raise AgentWorkspaceError("revision collection archive integrity is invalid")
    candidate = value.get("candidate_manifest")
    if not isinstance(candidate, dict):
        raise AgentWorkspaceError("revision collection archive Candidate is missing")
    try:
        validated = candidate_verification.validate_candidate_manifest(candidate)
    except candidate_verification.CandidateVerificationError as exc:
        raise AgentWorkspaceError("revision collection archive Candidate is invalid") from exc
    if (
        validated.get("workspace_id") != workspace_id
        or validated.get("round") != round_number
        or value.get("candidate_id") != validated.get("candidate_id")
    ):
        raise AgentWorkspaceError("revision collection archive identity drifted")
    return value


def _persist_collection_round_archive(
    manifest: dict[str, Any], collection: dict[str, Any], *, round_number: int
) -> dict[str, Any]:
    workspace_id = str(manifest["workspace_id"])
    path = _collection_round_receipt_path(manifest, round_number=round_number)
    validator = lambda value: _validate_collection_round_receipt(
        value, workspace_id=workspace_id, round_number=round_number
    )
    created = candidate_verification.persist_immutable_receipt(
        path, collection, validator=validator
    )
    observed = candidate_verification.read_immutable_receipt(path, validator=validator)
    if observed != collection:
        raise AgentWorkspaceError("revision collection archive readback drifted")
    return {
        "path": str(path),
...
def _revision_collection_evidence(
    ...
    if _candidate_round(manifest) != 1:
        raise AgentWorkspaceError("candidate revision is limited to round one -> round two")
    history = manifest.get("revision_history", [])
    if history not in (None, []) and (not isinstance(history, list) or history):
        raise AgentWorkspaceError("candidate revision limit has already been consumed")
    if manifest.get("close_receipt") is not None:
        raise AgentWorkspaceError("closed workspace may not start a candidate revision")
    intents = manifest.get("task_start_intents")
    if not isinstance(intents, dict):
        raise AgentWorkspaceError("candidate revision start intents are invalid")
    foreign_intents = [key for key in intents if key != "writer_handoff"]
    if foreign_intents:
        raise AgentWorkspaceError("candidate revision has unresolved task start intents")
    for role in READ_ONLY_ROLES:
        if _role_final_attempt(manifest, role) != 1:
            raise AgentWorkspaceError(
                "P4 candidate revision requires first-attempt round-one verifiers"
            )
    collection = manifest.get("collection")
    if not isinstance(collection, dict) or collection.get("state") != "complete":
        raise AgentWorkspaceError("candidate revision requires a complete collection")
    if not _collection_integrity_status(manifest, collection).get("valid"):
        raise AgentWorkspaceError("candidate revision collection integrity is invalid")
    raw_candidate = collection.get("candidate_manifest")
    raw_receipts = collection.get("verification_receipts")
    raw_summary = collection.get("verification_summary")
    if (
        ...
        raise AgentWorkspaceError("candidate revision verification evidence is incomplete")
    try:
        candidate = candidate_verification.validate_candidate_manifest(raw_candidate)
        persisted_candidate = candidate_verification.read_immutable_receipt(
            _candidate_receipt_paths(manifest, round_number=1)["candidate"],
            validator=candidate_verification.validate_candidate_manifest,
        )
        if persisted_candidate != candidate:
            raise candidate_verification.CandidateVerificationError(
                "round-one Candidate differs from immutable receipt"
            )
        receipts: list[dict[str, Any]] = []
        for value in raw_receipts:
            receipt = candidate_verification.validate_verification_receipt(
                value, expected_candidate_id=candidate["candidate_id"]
            )
            path = _verification_receipt_path(
                manifest,
                str(receipt["verifier_kind"]),
            ...
            persisted = candidate_verification.read_immutable_receipt(
                path, validator=validator
            )
            if persisted != receipt:
                raise candidate_verification.CandidateVerificationError(
                    "round-one Verification differs from immutable receipt"
                )
            receipts.append(receipt)
        summary = candidate_verification.validate_verification_summary(
            raw_summary, candidate_manifest=candidate, verification_receipts=receipts
        )
    except candidate_verification.CandidateVerificationError as exc:
        raise AgentWorkspaceError(
            f"candidate revision evidence is invalid: {_error_summary(exc)}"
        ) from exc
    if (
        candidate.get("round") != 1
        or candidate.get("workspace_id") != manifest.get("workspace_id")
        or candidate.get("base_head") != manifest.get("expected_base_head")
        or candidate.get("lane_id") != _candidate_lane_id(manifest)
        or collection.get("candidate_id") != candidate.get("candidate_id")
    ):
        raise AgentWorkspaceError("candidate revision identity does not match workspace authority")
    if summary.get("outcome") != "NEEDS_CHANGE" or summary.get("missing_required_verifiers"):
        raise AgentWorkspaceError("candidate revision requires complete NEEDS_CHANGE verification")
    writer_result = collection.get("writer_result")
    if (
        not isinstance(writer_result, dict)
        or writer_result.get("type") != "patch"
        or writer_result.get("sha256") != candidate.get("patch_sha256")
        or not _verify_patch_artifact(
            writer_result, expected_path=_writer_patch_path(manifest, round_number=1)
        )
    ):
        raise AgentWorkspaceError("candidate revision round-one patch artifact is invalid")
    snapshot = _git_snapshot(manifest, _run)
    if (
        snapshot.get("writer_head") != manifest.get("expected_base_head")
        ...
        or snapshot.get("base_drift")
        or _sha256_json(snapshot.get("untracked_artifacts"))
        != candidate.get("untracked_manifest_sha256")
    ):
    ...
    if (
        ...
        or writer.get("state") != "completed"
        or writer.get("terminal") is not True
        or _writer_task_binding_reasons(manifest, effective, writer)
    ):
    ...
    if (
        not isinstance(result_sha256, str)
        or SHA256_RE.fullmatch(result_sha256) is None
        or not isinstance(preimage_sha256, str)
        or SHA256_RE.fullmatch(preimage_sha256) is None
    ):
        raise AgentWorkspaceError("candidate revision result or preimage binding is invalid")
    return collection, candidate, receipts, summary, snapshot


def _transition_to_candidate_revision(
    manifest: dict[str, Any],
    *,
    collection: dict[str, Any],
    candidate: dict[str, Any],
    ...
    history_entry = {
        ...
        "writer_final_attempt": collection.get("writer_final_attempt", 1),
        "transitioned_at": _utc(),
    }
    manifest["revision_history"] = [history_entry]
    manifest["candidate_round"] = 2
    manifest["collection"] = None
    manifest.pop("frozen_writer", None)
    tasks_map = dict(manifest.get("tasks", {}))
    tasks_map["tests"] = None
    tasks_map["review"] = None
    manifest["tasks"] = tasks_map
    final_attempts = dict(manifest.get("role_final_attempt", {}))
    final_attempts.pop("tests", None)
    final_attempts.pop("review", None)
    manifest["role_final_attempt"] = final_attempts
    _append_workspace_event(
        manifest,
        "candidate_revision_round_opened",
        role="writer",
        outcome="round_2_ready",
        evidence={
            "round": 1,
            "next_round": 2,
            "candidate_id": candidate["candidate_id"],
...
def _candidate_revision_status(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(collection, dict) or collection.get("state") != "complete":
        return {
            "eligible": False,
            "reasons": ["complete_collection_missing"],
            "max": 1,
            "used": 1 if _candidate_round(manifest) > 1 else 0,
        }
    try:
        ...
        return {
            "eligible": False,
            "reasons": ["candidate_revision_preconditions_not_met"],
            "error": _error_summary(exc),
            "max": 1,
            "used": 1 if _candidate_round(manifest) > 1 else 0,
        }
    return {
        ...
        "next_round": 2,
        "verification_outcome": summary["outcome"],
        "result_sha256": observed["result_sha256"],
        "preimage_sha256": snapshot["diff_sha256"],
        "tool": "grabowski_agent_workspace_writer_handoff",
    }
...
def _role_attempt_summary(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    for role in ("writer", "tests", "review"):
        final_attempt = (
            _writer_final_attempt(manifest)
            if role == "writer"
            else _role_final_attempt(manifest, role)
        )
...
def _first_pass_role_results(
    ...
    if _candidate_round(manifest) > 1:
        path = _collection_round_receipt_path(manifest, round_number=1)
        try:
            first_collection = _validate_collection_round_receipt(
                _load_json(path),
                workspace_id=str(manifest["workspace_id"]),
                round_number=1,
            )
    ...
    writer = {
        "state": (
            first_collection.get("writer_task", {}).get("state")
            if isinstance(first_collection.get("writer_task"), dict)
            else None
        ),
        "receipt_sha256": first_collection.get("writer_receipt_sha256"),
    }
    result: dict[str, Any] = {"writer": writer}
    for role in ("tests", "review"):
        path = _role_receipt_path(
            manifest, role, attempt=1, candidate_round=1
        )
        receipt = _load_json(path) if path.exists() else None
...
def _known_workspace_tool_calls(manifest: dict[str, Any], phase: str) -> dict[str, Any]:
    ...
    calls = {
        ...
        "writer_handoff": (
            event_counts.get("writer_handoff_start_intent", 0)
            + event_counts.get("candidate_revision_start_intent", 0)
        ),
        "close": (
...
def _status_data(manifest: dict[str, Any], runner: CommandRunner = _run) -> dict[str, Any]:
    ...
    elif candidate_revision.get("eligible"):
        recommended_next_action = "writer_handoff_for_candidate_revision"
    ...
    return {
        ...
        "candidate_round": _candidate_round(manifest),
...
def _start_role_task(manifest: dict[str, Any], role: str, snapshot: dict[str, Any]) -> dict[str, Any]:
    ...
    argv = _role_task_argv(
        ...
        bool(snapshot["dirty"]),
        output_path=_role_receipt_path(
            manifest, role, attempt=_role_final_attempt(manifest, role)
        ),
    )
...
def grabowski_agent_workspace_collect(workspace_id: str) -> dict[str, Any]:
    ...
    with _lock(identifier):
        ...
        if (
            ...
            or writer_receipt.get("expected_dirty_preimage_sha256")
            != effective_writer.get("revision_preimage_sha256")
            or writer_receipt.get("observed_dirty_preimage_sha256")
            != effective_writer.get("revision_preimage_sha256")
        ):
            return {
                "workspace_id": identifier,
        ...
        try:
            candidate_manifest, candidate_reference = _persist_candidate_evidence(
                ...
                writer_result,
                writer_receipt,
                round_number=_candidate_round(manifest),
            )
        except (
            AgentWorkspaceError,
            candidate_verification.CandidateVerificationError,
...
def _bind_writer_handoff_attempt(
    ...
    *,
    ...
    launch_nonce: str,
    reconciled: bool,
    actor: str = "operator_handoff",
    revision: dict[str, Any] | None = None,
) -> dict[str, Any]:
    if actor not in {"operator_handoff", "candidate_revision"}:
        raise AgentWorkspaceError("writer attempt actor is invalid")
    ...
    if actor == "candidate_revision":
        if not isinstance(revision, dict):
            raise AgentWorkspaceError("candidate revision binding is missing")
        ...
        raise AgentWorkspaceError("failure handoff may not carry candidate revision binding")
    manifest["writer_attempts"] = [attempts[0], record]
    manifest["writer_final_attempt"] = 2
    if actor == "candidate_revision":
        assert isinstance(revision, dict)
        transition = _transition_to_candidate_revision(
            manifest,
    ...
    if actor == "candidate_revision":
        event_type = (
            ...
            if reconciled
            else "candidate_revision_started"
        )
        state = (
            "writer_revision_start_reconciled"
            if reconciled
            else "writer_revision_started"
        )
        operation = (
            "agent-workspace-candidate-revision-reconcile"
            if reconciled
            else "agent-workspace-candidate-revision"
        )
    else:
        event_type = "writer_handoff_start_reconciled" if reconciled else "writer_handoff_started"
        state = "writer_handoff_start_reconciled" if reconciled else "writer_handoff_started"
        operation = (
...
def _reconcile_writer_handoff_start(
    ...
    *,
    ...
    if kind == "candidate_revision":
        actor = "candidate_revision"
        dirty_preimage_sha256 = intent.get("revision_preimage_sha256")
    elif kind == "operator_handoff":
        actor = "operator_handoff"
        dirty_preimage_sha256 = None
    else:
        return {
            "workspace_id": manifest["workspace_id"],
    ...
    task_argv = _writer_task_argv(
        ...
        attempt=2,
        launch_nonce=launch_nonce,
        dirty_preimage_sha256=(
            dirty_preimage_sha256 if isinstance(dirty_preimage_sha256, str) else None
        ),
    )
    expected_task_argv_sha = _task_argv_sha256(task_argv)
    host = _bound_task_host(manifest)
    cwd = str(manifest["writer_worktree"])
    previous_state = intent.get("previous_state")
    previous_state_valid = (
        previous_state == "completed"
        if actor == "candidate_revision"
        else previous_state in WRITER_HANDOFF_TERMINAL_STATES
    )
    revision: dict[str, Any] | None = None
    revision_valid = actor != "candidate_revision"
    if actor == "candidate_revision":
        collection = manifest.get("collection")
        raw_candidate = collection.get("candidate_manifest") if isinstance(collection, dict) else None
        try:
            candidate = (
                candidate_verification.validate_candidate_manifest(raw_candidate)
        ...
        revision_valid = bool(
            isinstance(collection, dict)
            and collection.get("state") == "complete"
            and _collection_integrity_status(manifest, collection).get("valid")
            and isinstance(candidate, dict)
            and candidate.get("round") == 1
            and intent.get("candidate_round") == 1
            and intent.get("next_candidate_round") == 2
            and intent.get("revision_candidate_id") == candidate.get("candidate_id")
            and intent.get("revision_result_sha256") == collection.get("result_sha256")
    ...
    if (
        ...
        or not previous_state_valid
        or not revision_valid
        or intent.get("launch_nonce_sha256")
        != hashlib.sha256(launch_nonce.encode()).hexdigest()
    ):
        return {
            "workspace_id": manifest["workspace_id"],
    ...
    candidate_task = next(iter(unique.values()))
...
def _candidate_revision_handoff(
    manifest: dict[str, Any],
    *,
    attempts: list[dict[str, Any]],
    effective: dict[str, Any],
    ...
    if not isinstance(intents_value, dict):
        return {
            "workspace_id": identifier,
            "state": "writer_handoff_start_intents_invalid",
            "handoff_status": "blocked",
            "reconcile_required": True,
        }
    ...
    try:
        collection, candidate, _receipts, summary, snapshot = (
            _revision_collection_evidence(manifest)
        )
    except Exception as exc:
        return {
            "workspace_id": identifier,
            "state": "candidate_revision_blocked",
            "error": _error_summary(exc),
            "handoff_status": "blocked",
        }
    ...
    if not preflight["passed"]:
        ...
        return {
            "workspace_id": identifier,
            "state": "writer_toolchain_preflight_failed",
            "preflight": preflight,
            "attempt_consumed": False,
            "candidate_revision": True,
            "handoff_status": "blocked",
    ...
    try:
        re_collection, re_candidate, _re_receipts, re_summary, re_snapshot = (
            _revision_collection_evidence(manifest)
        )
    except Exception as exc:
        _append_workspace_event(
            manifest,
            "candidate_revision_revalidation",
            role="writer",
            outcome="blocked",
            evidence={
                "candidate_id": candidate["candidate_id"],
        ...
        return {
            "workspace_id": identifier,
            "state": "candidate_revision_revalidation_blocked",
            "error": _error_summary(exc),
            "handoff_status": "blocked",
        }
    if (
        re_candidate.get("candidate_id") != candidate.get("candidate_id")
        or re_collection.get("result_sha256") != collection.get("result_sha256")
        or re_snapshot.get("diff_sha256") != snapshot.get("diff_sha256")
        or re_summary.get("outcome") != summary.get("outcome")
    ):
        return {
            "workspace_id": identifier,
            "state": "candidate_revision_revalidation_drifted",
            "handoff_status": "blocked",
        }
    lane_status = _require_live_lane_binding(manifest, _run)
    _append_workspace_event(
    ...
    if os.path.lexists(output):
        return {
            "workspace_id": identifier,
            "state": "writer_handoff_receipt_already_exists",
            "receipt_path": str(output),
            "handoff_status": "blocked",
        }
    preimage_sha256 = str(re_collection["diff_sha256"])
    launch_nonce = hashlib.sha256(
        f"{identifier}:writer:revision:{time.time_ns()}".encode()
    ).hexdigest()[:24]
    task_argv = _writer_task_argv(
        manifest,
        command=command,
        attempt=2,
        launch_nonce=launch_nonce,
        dirty_preimage_sha256=preimage_sha256,
    )
    host = _bound_task_host(manifest)
    cwd = str(manifest["writer_worktree"])
    intent = {
        ...
        "nonce": hashlib.sha256(
            f"{identifier}:writer:revision-intent:{time.time_ns()}".encode()
        ).hexdigest()[:24],
        "launch_nonce": launch_nonce,
        "launch_nonce_sha256": hashlib.sha256(launch_nonce.encode()).hexdigest(),
        "previous_task_id": effective["task_id"],
    ...
    _append_workspace_event(
        manifest,
        "candidate_revision_start_intent",
        role="writer",
        outcome="persisted",
        evidence={
            "attempt": 2,
    ...
    public = _validate_started_task(
        started.get("task") if isinstance(started, dict) else None,
        role="writer",
        expected_host=host,
        expected_argv=task_argv,
        expected_cwd=cwd,
...
def grabowski_agent_workspace_writer_handoff(workspace_id: str, replacement_argv: list[str]) -> dict[str, Any]:
    """Start one bounded writer attempt after terminal failure or one verified NEEDS_CHANGE Candidate."""
    operator._require_operator_mutation("durable_job")
    operator._require_operator_mutation("git_cli")
    operator._require_operator_capability("resource_lease")
    identifier = _required_string(workspace_id, "workspace_id", max_length=80)
    with _lock(identifier):
        ...
        if final > 1:
            if (
                final == 2
                and effective.get("actor") in {"operator_handoff", "candidate_revision"}
                and effective.get("command_sha256") == command_sha
            ):
                writer = _task_public(str(effective["task_id"]))
                ...
                return {
                    "workspace_id": identifier,
                    "state": (
                        "writer_revision_already_started"
                        if effective.get("actor") == "candidate_revision"
                        else "writer_handoff_already_started"
                    ),
                    "attempt": 2,
                    "task": writer,
                    "attempt_record": _writer_attempt_refs(manifest)[-1],
            ...
            return {
                "workspace_id": identifier,
                "state": "writer_handoff_limit_reached",
                "handoff_status": "blocked",
            }
        collection_value = manifest.get("collection")
        if isinstance(collection_value, dict) and collection_value.get("state") == "complete":
            return _candidate_revision_handoff(
                manifest,
                attempts=attempts,
                effective=effective,
                command=command,
...
def _idle_tmux_transition_intent_path(workspace_id: str) -> Path:
    return _workspace_dir(workspace_id) / "idle-tmux-transition-intent.json"


def _idle_tmux_transition_receipt_path(workspace_id: str) -> Path:
    return _workspace_dir(workspace_id) / "idle-tmux-transition-receipt.json"


def _verified_idle_tmux_transition_intent(workspace_id: str) -> dict[str, Any] | None:
    path = _idle_tmux_transition_intent_path(workspace_id)
    try:
    ...
    schema_version = value.get("schema_version")
    expected_keys = legacy_keys if schema_version == 1 else legacy_keys | {"session_identity"}
    if schema_version not in {1, 2} or set(value) != expected_keys:
        raise AgentWorkspaceActionError("idle tmux transition intent fields are invalid")
    digest = value.get("intent_sha256")
    if not isinstance(digest, str) or SHA256_RE.fullmatch(digest) is None:
        raise AgentWorkspaceActionError("idle tmux transition intent digest is invalid")
    body = dict(value)
    body.pop("intent_sha256")
    if _sha256_json(body) != digest:
        raise AgentWorkspaceActionError("idle tmux transition intent digest mismatch")
    if (
        value.get("kind") != "grabowski_agent_workspace_idle_tmux_transition_intent"
        or value.get("workspace_id") != workspace_id
        or not isinstance(value.get("source_plan_sha256"), str)
        or SHA256_RE.fullmatch(str(value["source_plan_sha256"])) is None
        or not isinstance(value.get("session_name"), str)
        or not value["session_name"]
        or value.get("mutation_performed") is not False
        or value.get("historical_evidence_preserved") is not True
        or (schema_version == 2 and not _valid_tmux_session_identity(value.get("session_identity")))
    ):
        raise AgentWorkspaceActionError("idle tmux transition intent contract is invalid")
    return value


def _prepare_idle_tmux_transition_intent(
    workspace_id: str,
    *,
    ...
    if not _valid_tmux_session_identity(session_identity):
        raise AgentWorkspaceActionError("idle tmux transition session identity is invalid")
    existing = _verified_idle_tmux_transition_intent(workspace_id)
    if existing is not None:
        if (
            existing["source_plan_sha256"] != source_plan_sha256
            or existing["session_name"] != session_name
            or (
                existing.get("session_identity") is not None
                and existing["session_identity"] != session_identity
            )
        ):
            raise AgentWorkspaceActionError(
                "idle tmux transition intent conflicts with the requested plan"
            )
        return existing
    body = {
        "schema_version": 2,
        "kind": "grabowski_agent_workspace_idle_tmux_transition_intent",
        "workspace_id": workspace_id,
        "source_plan_sha256": source_plan_sha256,
        "session_name": session_name,
    ...
    intent = {**body, "intent_sha256": _sha256_json(body)}
    _atomic_json(_idle_tmux_transition_intent_path(workspace_id), intent)
    return intent


def _verified_idle_tmux_transition_receipt(workspace_id: str) -> dict[str, Any] | None:
    path = _idle_tmux_transition_receipt_path(workspace_id)
    try:
        value = _load_json(path)
    except FileNotFoundError:
        return None
    ...
    if not isinstance(digest, str) or SHA256_RE.fullmatch(digest) is None:
        raise AgentWorkspaceActionError(
            "idle tmux transition receipt digest is invalid"
        )
    body = dict(value)
    body.pop("receipt_sha256")
    if _sha256_json(body) != digest:
        raise AgentWorkspaceActionError("idle tmux transition receipt digest mismatch")
    required = {
        "schema_version",
        "workspace_id",
        "source_plan_sha256",
        "session_name",
        "tmux_removed",
        "tmux_mutation_performed",
        "task_mutation_performed",
        "resource_mutation_performed",
        "worktree_mutation_performed",
        "historical_evidence_preserved",
        "transitioned_at",
        "receipt_sha256",
    }
    if not required.issubset(value):
        raise AgentWorkspaceActionError(
            "idle tmux transition receipt fields are incomplete"
        )
    if (
        ...
        or not isinstance(value.get("session_name"), str)
        or not value["session_name"]
        or value.get("tmux_removed") is not True
        or not isinstance(value.get("tmux_mutation_performed"), bool)
        or value.get("task_mutation_performed") is not False
        or value.get("resource_mutation_performed") is not False
        or value.get("worktree_mutation_performed") is not False
        or value.get("historical_evidence_preserved") is not True
        or (
            value.get("schema_version") == 2
            and not _valid_tmux_session_identity(value.get("session_identity"))
        )
    ):
        raise AgentWorkspaceActionError(
            "idle tmux transition receipt contract is invalid"
        )
    source_intent_sha256 = value.get("source_intent_sha256")
    if source_intent_sha256 is not None and (
        not isinstance(source_intent_sha256, str)
        or SHA256_RE.fullmatch(source_intent_sha256) is None
    ):
        raise AgentWorkspaceActionError(
            "idle tmux transition receipt intent binding is invalid"
        )
    recovered = value.get("recovered_after_ambiguous_effect")
    if recovered is not None and not isinstance(recovered, bool):
        raise AgentWorkspaceActionError(
            "idle tmux transition receipt recovery marker is invalid"
        )
    return value


def _publish_idle_tmux_transition_receipt(
    manifest: dict[str, Any],
    *,
    source_plan_sha256: str,
    session_name: str,
    intent: dict[str, Any],
    tmux_mutation_performed: bool,
    recovered_after_ambiguous_effect: bool,
) -> dict[str, Any]:
    identifier = str(manifest["workspace_id"])
    existing = _verified_idle_tmux_transition_receipt(identifier)
    if existing is not None:
        if (
            existing["source_plan_sha256"] != source_plan_sha256
            or existing["session_name"] != session_name
            ...
            raise AgentWorkspaceActionError(
                "idle tmux transition receipt conflicts with the requested plan"
            )
        return existing
    session_identity = intent.get("session_identity")
    body = {
        "schema_version": 2 if session_identity is not None else 1,
        "workspace_id": identifier,
        "source_plan_sha256": source_plan_sha256,
        "source_intent_sha256": intent["intent_sha256"],
        "session_name": session_name,
        **({"session_identity": dict(session_identity)} if session_identity is not None else {}),
        "tmux_removed": True,
        "tmux_mutation_performed": tmux_mutation_performed,
        "recovered_after_ambiguous_effect": recovered_after_ambiguous_effect,
        "task_mutation_performed": False,
        "resource_mutation_performed": False,
        "worktree_mutation_performed": False,
        "historical_evidence_preserved": True,
    ...
    receipt = {**body, "receipt_sha256": _sha256_json(body)}
    _atomic_json(_idle_tmux_transition_receipt_path(identifier), receipt)
    return receipt


def _persist_idle_tmux_transition_manifest(
    manifest: dict[str, Any], transition_receipt: dict[str, Any]
) -> None:
    identifier = str(manifest["workspace_id"])
    existing = manifest.get("idle_tmux_transition_receipt")
    if isinstance(existing, dict):
        if existing != transition_receipt:
            raise AgentWorkspaceActionError(
                "workspace manifest has conflicting idle tmux transition evidence"
            )
        return
    ...
    base._append_audit(
        {
            ...
            "workspace_id": identifier,
            "source_plan_sha256": transition_receipt["source_plan_sha256"],
            "session_name": transition_receipt["session_name"],
            "receipt_sha256": transition_receipt["receipt_sha256"],
            "source_intent_sha256": transition_receipt.get("source_intent_sha256"),
            "tmux_removed": True,
            "tmux_mutation_performed": transition_receipt["tmux_mutation_performed"],
            "recovered_after_ambiguous_effect": transition_receipt.get(
...
def grabowski_agent_workspace_reconcile_idle_tmux(
    ...
    tmux_mutation_performed_this_call = False
    with _lock(identifier):
        manifest = _manifest(identifier)
        existing = manifest.get("close_receipt")
        if (
            isinstance(existing, dict)
            and _close_integrity_status(manifest, existing)["valid"]
        ):
            return {
                "workspace_id": identifier,
                "state": "already_closed",
        ...
        transition_receipt = _verified_idle_tmux_transition_receipt(identifier)
        intent = _verified_idle_tmux_transition_intent(identifier)
        recovery_source_plan_sha256 = (
            str(transition_receipt["source_plan_sha256"])
            if transition_receipt is not None
            else str(intent["source_plan_sha256"])
            if intent is not None
            else None
        )
        ...
        if transition_receipt is not None and recovery_expected_matches:
            session_name = str(transition_receipt["session_name"])
            if _tmux_has_exact_session(session_name):
                raise AgentWorkspaceActionError(
                    "idle tmux transition receipt conflicts with a live exact session"
                )
            _persist_idle_tmux_transition_manifest(manifest, transition_receipt)
        elif intent is not None and recovery_expected_matches:
            session_name = str(intent["session_name"])
            live_identity = _tmux_exact_session_identity(session_name)
            intended_identity = intent.get("session_identity")
            if live_identity is None:
                transition_receipt = _publish_idle_tmux_transition_receipt(
                    manifest,
                    source_plan_sha256=str(intent["source_plan_sha256"]),
                    session_name=session_name,
                    intent=intent,
                    tmux_mutation_performed=False,
                    recovered_after_ambiguous_effect=True,
                )
                _persist_idle_tmux_transition_manifest(manifest, transition_receipt)
            else:
                if intended_identity is None:
                    raise AgentWorkspaceActionError(
                        "legacy idle tmux transition intent cannot authorize removal of a live exact session"
                    )
                if live_identity != intended_identity:
                    raise AgentWorkspaceActionError(
                ...
                if (
                    not isinstance(recovery_transition, dict)
                    or recovery_transition.get("eligible") is not True
                    or recovery_transition.get("session_name") != session_name
                ):
                    raise AgentWorkspaceActionError(
                        "idle tmux transition intent is no longer eligible under current workspace liveness"
                    )
                if _tmux_exact_session_identity(session_name) != intended_identity:
                    raise AgentWorkspaceActionError(
                        "idle tmux session lifetime changed before retry removal"
                    )
                killed = _tmux_result(
                    ["kill-session", "-t", str(intended_identity["session_id"])]
                )
                tmux_mutation_performed_this_call = killed.get("returncode") == 0
                remaining_identity = _tmux_exact_session_identity(session_name)
                if remaining_identity == intended_identity:
                    raise AgentWorkspaceActionError(
                ...
                transition_receipt = _publish_idle_tmux_transition_receipt(
                    ...
                    session_name=session_name,
                    intent=intent,
                    tmux_mutation_performed=tmux_mutation_performed_this_call,
                    recovered_after_ambiguous_effect=True,
                )
                _persist_idle_tmux_transition_manifest(manifest, transition_receipt)
        else:
            ...
            if (
                ...
                or transition.get("eligible") is not True
            ):
                return {
                    "workspace_id": identifier,
            ...
            session_identity = _tmux_exact_session_identity(session_name)
            if session_identity is None:
                raise AgentWorkspaceActionError(
                    "idle tmux transition exact session identity is unavailable"
                )
            intent = _prepare_idle_tmux_transition_intent(
                identifier,
                source_plan_sha256=expected_hash,
                session_name=session_name,
            ...
            current_identity = _tmux_exact_session_identity(session_name)
            if current_identity != session_identity:
                raise AgentWorkspaceActionError(
                    "idle tmux session lifetime changed before removal"
                )
            killed = _tmux_result(
                ["kill-session", "-t", str(session_identity["session_id"])]
            )
            tmux_mutation_performed_this_call = killed.get("returncode") == 0
            remaining_identity = _tmux_exact_session_identity(session_name)
            if remaining_identity == session_identity:
                raise AgentWorkspaceActionError(
                    str(killed.get("stderr") or "idle tmux session removal failed")
                )
            transition_receipt = _publish_idle_tmux_transition_receipt(
        ...
        recovered_after_ambiguous_effect = bool(
            transition_receipt.get("recovered_after_ambiguous_effect", False)
        )
        if not post_tmux_plan["stale_reconciliation"]["eligible"]:
            return {
                ...
                "tmux_removed": True,
                "tmux_mutation_performed": tmux_mutation_performed,
                "tmux_mutation_performed_this_call": tmux_mutation_performed_this_call,
                "recovered_after_ambiguous_effect": recovered_after_ambiguous_effect,
...
def _workspace_archive_from_recovery_intent(
    manifest: dict[str, Any],
    cleanup_plan: dict[str, Any],
    intent: dict[str, Any],
    *,
    ...
    """Verify one archive id that was durably persisted by recovery."""
    archive_id = intent.get("archive_id")
    if not isinstance(archive_id, str):
        return None
    checkout = cleanup_plan.get("checkout")
    ...
    if (
        ...
        or not isinstance(expected_branch, str)
        or not isinstance(expected_physical_identity, dict)
        or intent.get("owner_id") != owner
        or intent.get("writer_worktree") != cleanup_plan.get("writer_worktree")
        or intent.get("writer_branch") != expected_branch
        or intent.get("writer_head") != expected_head
        or intent.get("checkout_physical_identity") != expected_physical_identity
    ):
        raise AgentWorkspaceActionError(
            "workspace persisted recovery archive identity does not match current checkout"
        )
    try:
        archive = checkouts._load_archive(archive_id)
    except ValueError as exc:
        raise AgentWorkspaceActionError(
            "workspace persisted recovery archive is missing"
        ) from exc
    _workspace_archive_post_state(
        manifest,
        archive_id,
...
def _workspace_recover_persisted_archive_uncertainty(
    ...
    *,
    ...
    """Read back one exact prior archive outcome without clearing success early."""
    fence = _workspace_persisted_archive_uncertainty_fence(
        cleanup_plan, owner=owner
    )
    if fence is None:
        return {"state": "no_fence", "archive": None, "fence": None}
...
def grabowski_agent_workspace_cleanup(
    ...
    with _lock(identifier):
        ...
        if reconciliation is not None:
            ...
            durable_recovery_archive = (
                _workspace_archive_from_recovery_intent(
                    manifest, plan, prior_intent, owner=owner
                )
                if isinstance(prior_intent, dict)
                and prior_intent.get("state") in {"started", "recovery_required"}
                else None
            )
            recovery_fence = (
                ...
                in (
                    ...
                    {"cleanup_outcome_unknown", "active_checkout_coordination"},
            ...
            archive_recovery_only = bool(
                not plan["eligible"]
                and plan.get("lifecycle_state")
                in {"cleanup_outcome_unknown", "cleanup_recovery_required"}
                ...
                and (
                    isinstance(recovery_fence, dict)
                    or isinstance(durable_recovery_archive, dict)
                )
                ...
                and prior_intent.get("checkout_physical_identity")
                == plan["checkout"].get("physical_identity")
            ...
            if archive_recovery_only:
                ...
                _append_workspace_event(
                    manifest,
                    "workspace_cleanup_recovery_requested",
    ...
    recovery_archive_id = (
        str(durable_recovery_archive["archive_id"])
        if isinstance(durable_recovery_archive, dict)
        and isinstance(durable_recovery_archive.get("archive_id"), str)
        else None
    )
    archive_id = recovery_archive_id or reusable_archive_id
    ...
    try:
        ...
        if archive_id is None:
            ...
            recovery_result = _workspace_recover_persisted_archive_uncertainty(
                current_manifest,
                plan,
                owner=owner,
            )
            ...
            recovered_archive = recovery_result.get("archive")
            if recovery_state == "archive_verified":
                if not isinstance(recovered_archive, dict) or not isinstance(
                    recovered_archive.get("archive_id"), str
                ):
                    raise AgentWorkspaceActionError(
                        "workspace archive recovery returned invalid verified archive"
                    )
                archive_id = str(recovered_archive["archive_id"])
            elif recovery_state == "confirmed_no_effect":
                if archive_recovery_only:
                    with _lock(identifier):
                        current_manifest = _manifest(identifier)
                        ...
                        if (
                            ...
                            raise AgentWorkspaceActionError(
                                "workspace cleanup intent changed during no-effect recovery"
                            )
                        current_intent.update(
                            {
                                "state": "failed",
                                "archive_id": None,
                                "recovery_outcome": "confirmed_no_effect",
                                "updated_at": _utc(),
                            }
                        )
                    ...
                    return {
                        "workspace_id": identifier,
                        "state": "archive_recovery_confirmed_no_effect",
                        "idempotent": False,
                        "requires_fresh_cleanup_plan": True,
                        "worktree_preserved": True,
                    }
            elif recovery_state == "no_fence" and archive_recovery_only:
                raise AgentWorkspaceActionError(
                    "workspace archive recovery lost its durable fence before archive persistence"
                )
            elif recovery_state not in {"no_fence", "confirmed_no_effect"}:
                raise AgentWorkspaceActionError(
                    "workspace archive recovery returned an invalid outcome"
                )
        if archive_recovery_only and archive_id is not None:
            with _lock(identifier):
                current_manifest = _manifest(identifier)
                current_intent = current_manifest.get("workspace_cleanup_intent")
                if (
                    not isinstance(current_intent, dict)
                    or current_intent.get("intent_id") != intent["intent_id"]
                    or current_intent.get("state") != "recovery_required"
                ):
                    raise AgentWorkspaceActionError(
                        "workspace cleanup intent changed during archive recovery persistence"
                    )
                if current_intent.get("archive_id") != archive_id:
                    current_intent["archive_id"] = archive_id
                    current_intent["updated_at"] = _utc()
                    current_manifest["workspace_cleanup_intent"] = current_intent
                    _write_manifest(current_manifest)
                persisted_manifest = _manifest(identifier)
                persisted_intent = persisted_manifest.get("workspace_cleanup_intent")
                if (
                    not isinstance(persisted_intent, dict)
                    or persisted_intent.get("intent_id") != intent["intent_id"]
                    or persisted_intent.get("state") != "recovery_required"
                    or persisted_intent.get("archive_id") != archive_id
                ):
                    raise AgentWorkspaceActionError(
                        "workspace recovered archive was not durably persisted before fence release"
                    )
                _workspace_archive_post_state(
                    persisted_manifest,
                    archive_id,
                ...
                current_manifest = persisted_manifest
            persisted_fence = _workspace_persisted_archive_uncertainty_fence(
                plan, owner=owner
            )
            if isinstance(persisted_fence, dict):
                effect_mutation_ambiguous = True
                _workspace_reconcile_checkout_uncertainty(
                    ...
                    operation="archive",
                    operation_id=archive_id,
                    expected_evidence=dict(persisted_fence["evidence"]),
                    release_operation_lease=True,
                )
                effect_mutation_ambiguous = False
        if archive_id is None:
            ...
            try:
                ...
                try:
                    ...
                    with _lock(identifier):
                        ...
                        _write_manifest(current_manifest)
                        persisted_manifest = _manifest(identifier)
                        persisted_intent = persisted_manifest.get(
                            "workspace_cleanup_intent"
                        )
                        if (
                            not isinstance(persisted_intent, dict)
                            or persisted_intent.get("intent_id")
                            != intent["intent_id"]
                            or persisted_intent.get("archive_id") != archive_id
                        ):
                            raise AgentWorkspaceActionError(
        ...
        if archive_attempted_this_invocation or archive_recovery_only:
            with _lock(identifier):
                current_manifest = _manifest(identifier)
                current_intent = current_manifest.get("workspace_cleanup_intent")
                if (
