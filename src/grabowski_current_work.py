...
REPOSITORY_FILTERED_SOURCE_NAMES = (
    "checkouts",
...
def _scope_contract(repositories: list[str]) -> dict[str, Any]:
    return {
        "kind": "mixed_global_and_repository_filtered",
        "repository_filters": list(repositories),
        "repository_filtered_sources": list(REPOSITORY_FILTERED_SOURCE_NAMES),
...
def _historical_binding_drift_is_hygiene(item: dict[str, Any]) -> bool:
    """Keep coordination-free historical identity drift out of operative work.

    Completed/archived drift is historical hygiene as before.  An active-phase
    binding is also hygiene once retention has expired, or when a dirty linked
    checkout has no lease, process or coordination overlap.  This changes only
    the current-work projection: lifecycle repair and cleanup authority stay
    fail-closed. Dirty state remains visible/actionable; prunable/missing,
    unobservable and clean retained surfaces remain blocking.
    """
    ...
    return bool(
        item["lifecycle_state"] in {"managed_lifecycle_drift", "dirty"}
    ...
    *,
    ...
    for item in checkouts:
        ...
        if item["binding_present"] and not item["binding_consistent"]:
            if _historical_binding_drift_is_hygiene(item):
...
def _add_binding_reconciliation(
    ...
    for raw in rows:
        ...
        terminal_drift_hygiene = bool(
            ...
            and all(
                _historical_binding_drift_is_hygiene(item)
...
def build_current_work_projection(
    *,
    ...
    scope_contract = _scope_contract(repositories)
    snapshot_material = {
        "view": view,
    ...
    return {
        ...
        "total_projected_scope": MIXED_SOURCE_SCOPE,
        "state_counts": state_counts,
        "state_counts_scope": MIXED_SOURCE_SCOPE,
        "convergence_summary_scope": MIXED_SOURCE_SCOPE,
        "selection": {
