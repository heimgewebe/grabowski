...
def _database() -> sqlite3.Connection:
    ...
    connection.execute(
        "CREATE INDEX IF NOT EXISTS workers_kind_state_created_idx "
        "ON workers(kind, state, created_at_unix DESC, worker_id DESC)"
    )
    connection.execute(
        "CREATE INDEX IF NOT EXISTS workers_kind_created_idx "
        "ON workers(kind, created_at_unix DESC, worker_id DESC)"
    )
