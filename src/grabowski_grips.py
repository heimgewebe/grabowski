...
GRIP_SPECS: dict[str, GripSpec] = {
    ...
    "checkout-binding-identity-rebind-preview": GripSpec(
        ...
        acceptance_ids=(
            "checkout-key-bound",
            "identity-drift-mode-bound",
            "clean-uncoordinated",
            "remote-and-identity-proven",
            "snapshot-bound",
        ),
    ...
    "checkout-binding-identity-rebind-apply": GripSpec(
        ...
        version="1.1",
        summary="CAS-apply one preview-bound lifecycle identity rebind mode without Git, owner, archive or cleanup effects.",
        effect=MUTATING,
        required_parameters=(
            "checkout_key",
            "owner_id",
    ...
    "agent-execution-happy-path": GripSpec(
        ...
        summary=(
            ...
            "or resume an existing lane-backed Workspace, and advance its bound Candidate or Delivery profile to integration_ready."
        ),
        effect=MUTATING,
    ...
    "candidate-integration-ready": GripSpec(
        ...
        version="1.0",
        summary="Adopt one exact verified Candidate and publish its controller-owned commit to one exact ready PR.",
        effect=MUTATING,
        required_parameters=(
            "workspace_id",
        ...
        runner="candidate_integration_ready",
        uses_github=True,
        operation_effect_class="integration",
        operation_class="candidate-integration-ready",
    ),
    "forrest-server-exit-apply": GripSpec(
        name="forrest-server-exit-apply",
        version="1.0",
        summary="Remove only Forrest's fixed HTTPS 443 serve route on commonserver after exact protected-route and backend-port preconditions.",
        effect=MUTATING,
        required_parameters=(),
        acceptance_ids=(
            "fixed-target-bound",
    ...
    "remote-head-materialize": GripSpec(
        ...
        operation_effect_class="worktree_admin",
        operation_class="remote-head-materialize",
    ),
...
GRIP_SURFACE_TARGETS = {
    ...
    "checkout-binding-identity-rebind-preview": "one existing managed checkout with one proven bounded identity drift mode",
...
GRIP_RECOVERY_PATHS_BY_NAME = {
    ...
    "checkout-binding-identity-rebind-apply": (
        "rerun checkout-binding-identity-rebind-preview only while the same bounded branch-rename or repo-path-canonicalization drift still exists; "
...
GRIP_CONDITIONAL_PRECONDITIONS = {
    ...
    "agent-execution-happy-path": (
        ...
        "writer delivery never terminalizes, releases, merges or deploys; the outer controller alone may terminalize pr_opened/pr_updated and snapshot-release exact lane leases",
    ),
    "candidate-integration-ready": (
        ...
        "the source Work Lane becomes terminal candidate_adopted before branch/PR publication; its existing leases are retained only "
        "as short coordination until exact ready-PR readback and then released with snapshot guards",
    ),
    "checkout-binding-terminal-apply": (
    ...
    "checkout-binding-identity-rebind-apply": (
        ...
        "same owner, exactly one supported identity drift mode, remote security, coordination, live retention and both SQLite CAS preimages; branch rename additionally requires head ancestry, while repo-path canonicalization requires unchanged head/branch and same Git common-dir",
    ),
    "worktree-ensure": (
...
def _run_n8n_workflow_edge_verify(
    ...
    if provider_dispatcher is None:
        _check(receipt, "provider-readback", "fail", "dispatcher-unavailable")
        raise GripActionError("n8n provider dispatcher is unavailable")
...
def _run_n8n_workflow_edge_apply(
    ...
    if provider_dispatcher is None:
        _check(receipt, "single-edge-only", "fail", "dispatcher-unavailable")
        raise GripActionError("n8n provider dispatcher is unavailable")
    _check(receipt, "provider-profile-bound", "pass", request["provider_profile"])
    _check(receipt, "secret-hash-bound", "pass", request["expected_secret_sha256"])
    _check(
...
def _run_checkout_binding_identity_rebind_preview(
    ...
    mode = output.get("rebind_mode")
    drift = output.get("allowed_drift_reasons")
    if (
    ...
    mode_bound = (
        mode == "branch_rename" and drift == branch_drift
    ) or (
        mode == "repo_path_canonicalization"
        and isinstance(drift, list)
        and bool(drift)
        and set(drift).issubset(repo_path_drift)
    )
    if not mode_bound:
        raise GripActionError(
            "checkout identity rebind preview exceeded one supported identity drift mode"
        )
    if (
    ...
    if (
        ...
        raise GripActionError("checkout identity rebind preview lacks remote and identity proof")
    if (
        not isinstance(target, dict)
        or target.get("repo_path") != checkout.get("repo_path")
    ...
    _check(
        ...
        "remote-and-identity-proven",
        "pass",
        f"{mode}:{lineage.get('recorded_head')}",
    )
...
def _run_checkout_binding_identity_rebind_apply(
    ...
    try:
        ...
        identity_rows_agree = (
            ...
            and lifecycle.get("repo_path") == retention.get("repo_path")
    ...
    if (
        ...
        or mode not in {"branch_rename", "repo_path_canonicalization"}
    ...
    if (
        ...
        or after_lifecycle.get("repo_path") != after_retention.get("repo_path")
        or after_lifecycle.get("repo_path") != target.get("repo_path")
        or after_lifecycle.get("expected_head") != after_retention.get("expected_head")
        or after_lifecycle.get("expected_head") != target.get("expected_head")
        or after_lifecycle.get("expected_branch") != after_retention.get("expected_branch")
        or after_lifecycle.get("expected_branch") != target.get("expected_branch")
    ...
    if mode == "branch_rename":
        if (
            after_lifecycle.get("expected_head") == before_lifecycle.get("expected_head")
            or after_lifecycle.get("expected_branch") == before_lifecycle.get("expected_branch")
            or audit.get("effects")
            != [
                "lifecycle_expected_identity_update",
                "retention_expected_identity_update",
            ]
        ):
            raise GripActionError("checkout branch identity rebind exceeded its bounded effect")
    else:
        if (
            after_lifecycle.get("expected_head") != before_lifecycle.get("expected_head")
            or after_retention.get("expected_head") != before_retention.get("expected_head")
            or after_lifecycle.get("expected_branch") != before_lifecycle.get("expected_branch")
            or after_retention.get("expected_branch") != before_retention.get("expected_branch")
            or (
                before_lifecycle.get("repo_path") == target.get("repo_path")
                and before_retention.get("repo_path") == target.get("repo_path")
            )
        ):
            raise GripActionError("checkout repo-path identity rebind exceeded its bounded effect")
        expected_effects = []
        if before_lifecycle.get("repo_path") != target.get("repo_path"):
            expected_effects.append("lifecycle_repo_path_update")
        if before_retention.get("repo_path") != target.get("repo_path"):
            expected_effects.append("retention_repo_path_update")
        if not expected_effects or audit.get("effects") != expected_effects:
            raise GripActionError("checkout repo-path identity rebind effect list is not exact")
    _check(receipt, "checkout-key-bound", "pass", checkout_key)
    _check(receipt, "snapshot-cas-bound", "pass", expected_snapshot)
    _check(
        receipt,
    ...
    _check(receipt, "identity-only-effect", "pass", f"{mode}:{','.join(audit['effects'])}")
    _check(
        receipt,
        "target-identity-converged",
        "pass",
        f"{mode}:{after_lifecycle.get('repo_path')}:{after_lifecycle.get('expected_branch')}@{after_lifecycle.get('expected_head')}",
    )
    return {**output, "receipt_status": "passed"}
...
def _validate_remote_materialization_target(
    repo: Path,
    remote: str,
    receipt: Receipt,
    runner: CommandRunner,
    ...
    effective = _git_optional(repo, runner, ["remote", "get-url", "--all", remote])
    effective_urls = (
        str(effective.get("stdout", "")).splitlines()
        if int(effective.get("returncode", 1)) == 0
        else []
    )
    if len(configured_urls) != 1 or len(effective_urls) != 1:
        _check(receipt, "fetch_remote_target", "fail", "remote_url_count_not_one")
        raise GripPreflightError(
    ...
    effective_identity = _remote_target_identity(effective_urls[0])
    if configured_identity is None or effective_identity is None:
        _check(receipt, "fetch_remote_target", "fail", "unsupported_network_target")
        raise GripPreflightError(
            "remote-head-materialize requires one supported network remote target"
        )
    if not effective_identity[3]:
        _check(receipt, "fetch_remote_target", "fail", "effective_target_not_ssh")
        raise GripPreflightError(
            "remote-head-materialize requires one effective SSH remote target"
        )
    same_repository = effective_identity[:2] == configured_identity[:2]
    if configured_identity[3] and effective_identity[3]:
        same_user = effective_identity[2] == configured_identity[2]
    elif effective_identity[3]:
        same_user = effective_identity[2] in {"", "git"}
    else:
        same_user = True
    if not same_repository or not same_user:
        _check(receipt, "fetch_remote_target", "fail", "url_rewrite_changed_identity")
        raise GripPreflightError(
            "remote-head-materialize refuses URL rewriting that changes remote identity"
        )
    _check(
        receipt,
        "fetch_remote_target",
...
def _run_remote_head_materialize(
    spec: GripSpec,
    parameters: dict[str, Any],
    receipt: Receipt,
    runner: CommandRunner,
    ...
    del spec
    repo = _repo_path(parameters)
    lane_id = _string_parameter(parameters, "lane_id")
    ...
    if remote_branch in INTRINSIC_PROTECTED_BRANCHES:
        raise GripPreflightError(
            "remote-head-materialize refuses protected remote branches"
        )
    remote = parameters.get("remote", "origin")
    if (
    ...
    try:
        lane = work_acquire._stored_lane_inputs(lane_id)
    except (OSError, RuntimeError, TypeError, ValueError) as exc:
        _check(receipt, "lane_binding", "fail", type(exc).__name__)
        raise GripPreflightError(
            "remote-head-materialize could not verify the durable Work Lane"
        ) from exc
    lane_path = Path(str(lane.get("target_path", ""))).expanduser().resolve(
        strict=False
    )
    lane_branch = str(lane.get("branch", ""))
    lane_owner = str(lane.get("lease_owner_id", ""))
    if (
        lane_path != repo
        or not lane_branch
        or lane_owner != f"lane:{lane_id}"
    ):
        _check(receipt, "lane_binding", "fail", "lane_identity_mismatch")
        raise GripPreflightError(
            "remote-head-materialize target does not match the Work Lane"
        )
    _check(receipt, "lane_binding", "pass", f"{lane_id}:{lane_branch}")

    resource_keys = lane.get("resource_keys")
    if (
    ...
    if (
        ...
        or str(shallow.get("stdout", "")).strip() != "false"
    ):
        _check(receipt, "non_shallow_repository", "fail", "shallow_or_unknown")
        raise GripPreflightError(
    ...
    if orientation["branch"] != lane_branch or orientation["dirty"]:
        ...
        raise GripPreflightError(
            "remote-head-materialize requires the clean attached Work Lane branch"
        )
    if orientation["head"] not in {expected_local_head, expected_remote_head}:
        _check(receipt, "local_head", "fail", str(orientation["head"]))
        raise GripPreflightError(
            "remote-head-materialize local head does not match a bound head"
        )
    _check(receipt, "writer_checkout", "pass", lane_branch)
    ...
    _check(
        ...
        "object_import",
        "pass",
        str(fetch.get("argv_sha256") or "exact_remote_head"),
    )
    commit = _git_optional(
        repo,
        runner,
        ["rev-parse", "--verify", f"{expected_remote_head}^{{commit}}"],
    )
    if (
        int(commit.get("returncode", 1)) != 0
        or str(commit.get("stdout", "")).strip().lower()
        != expected_remote_head
    ):
        raise GripActionError(
            "remote-head-materialize exact remote commit was not materialized"
        )
    if (
...
def _agent_execution_source_request(
    ...
    effect_profile = parameters.get("effect_profile", "candidate")
    ...
    if not isinstance(route, dict) or route.get("effect_profile") != effect_profile:
    ...
    if effect_profile == "delivery":
        request_material["effect_profile"] = "delivery"
    ...
    lane_parameters = {
        ...
        "target_path": str(target),
        "purpose": (
            f"AEF P6 delivery {source_kind}:{source_id} {request_sha256[:12]}"
            if effect_profile == "delivery"
            else f"AEF P5 one-shot {source_kind}:{source_id} {request_sha256[:12]}"
        ),
        "retention_until_unix": retention_until_unix,
        "artifact_class": "implementation-worktree",
        "idempotency_key": f"aef-{source_identity_sha256[:20]}-{request_sha256[:20]}",
...
def _agent_execution_bound_effect_profile(
    workspace: Any,
    workspace_id: str,
    *,
    frontdoor: dict[str, Any] | None,
) -> str:
    if isinstance(frontdoor, dict):
        plan = frontdoor.get("execution_plan")
        route_binding = plan.get("route_binding") if isinstance(plan, dict) else None
        profile = (
            route_binding.get("effect_profile")
            if isinstance(route_binding, dict)
            else None
        )
        if profile in {"candidate", "delivery"}:
            return str(profile)
        raise GripPreflightError("Source-mode ExecutionPlan effect profile is invalid")
    try:
        manifest = workspace._manifest(workspace_id)
    except Exception as exc:
        raise GripPreflightError("workspace manifest is not safely readable") from exc
    if not isinstance(manifest, dict) or not workspace._lane_backed(manifest):
        raise GripPreflightError("effect profile requires a lane-backed Agent Workspace")
    ...
    # P1-P5 lane workspaces without a bound P6 profile remain Candidate-mode.
    if not isinstance(lane_binding, dict) or not hasattr(workspace, "_lane_receipt"):
        return "candidate"
    lane_id = lane_binding.get("lane_id")
    lane_receipt_sha256 = lane_binding.get("receipt_sha256")
    if not isinstance(lane_id, str) or not isinstance(lane_receipt_sha256, str):
        return "candidate"
    try:
        lane = workspace._lane_receipt(lane_id, lane_receipt_sha256)
    except Exception as exc:
        raise GripPreflightError("bound Work Lane is not safely readable") from exc
    inputs = lane.get("inputs") if isinstance(lane, dict) else None
    plan = inputs.get("execution_plan") if isinstance(inputs, dict) else None
    if plan is None:
        return "candidate"
    try:
        import grabowski_execution_plan as execution_plan

        validated = execution_plan.validate_execution_plan(plan)
    except Exception as exc:
        raise GripPreflightError("bound ExecutionPlan.v1 is invalid") from exc
    profile = validated["route_binding"]["effect_profile"]
    if profile == "delivery" and validated["route_binding"]["executor"] != "scoped_writer":
        raise GripPreflightError("delivery profile drifted from scoped_writer authority")
    return str(profile)
...
def _run_agent_execution_happy_path(
    ...
    effect_profile = _agent_execution_bound_effect_profile(
        workspace,
        workspace_id,
        frontdoor=frontdoor,
    )
    ...
    if effect_profile == "delivery":
        delivery = _run_candidate_delivery_profile(
            workspace_id=workspace_id,
        ...
        if delivery.get("delivery_ready") is not True:
            _check(
                receipt,
                "candidate-integration-composed",
                "fail",
                str(delivery.get("state")),
            )
            _check(
                receipt,
                "unknown-effect-no-retry",
                "pass",
                str(delivery.get("next_action")),
            )
            return with_frontdoor(
                {
                    **delivery,
        ...
        terminalization = _controller_terminalize_delivered_source(
            delivery=delivery,
            runner=runner,
            github_runner=github_runner,
        )
        if terminalization.get("terminalized") is not True:
            _check(
                receipt,
                "candidate-integration-composed",
                "fail",
                str(terminalization.get("state")),
            )
            _check(
                receipt,
                "unknown-effect-no-retry",
                "pass",
                str(terminalization.get("next_action")),
            )
            return with_frontdoor(
                {
                    **delivery,
                    "state": str(terminalization.get("state")),
                    "workspace_id": workspace_id,
                    "candidate": candidate,
                    "coordinator": coordinated,
                    ...
                    "reconcile_required": True,
                    "next_action": terminalization.get("next_action"),
                    "receipt_status": "blocked",
                }
            )
        _check(
        ...
        _check(
            ...
            "unknown-effect-no-retry",
            "pass",
            "delivery publication has exact remote and PR readback",
        )
        return with_frontdoor(
            {
                **delivery,
                ...
                "later_correction": "open_a_new_lane; never revive_this_terminalized_source_lane",
                "nonclaims": [
                    "merge remains controller-only",
                    "deployment remains controller-only",
                    "delivery does not authorize source-lane revival",
                ],
                "receipt_status": "passed",
            }
        )
...
def _candidate_integration_modules() -> tuple[Any, Any, Any, Any, Any]:
    ...
    import grabowski_resources as resource_store
    import grabowski_checkouts as checkouts

    return workspace, work_acquire, lane_closeout, resource_store, checkouts


def _candidate_sha256_parameter(parameters: dict[str, Any], name: str) -> str:
    value = _string_parameter(parameters, name).lower()
    if re.fullmatch(r"[0-9a-f]{64}", value) is None:
        raise GripPreflightError(f"{name} parameter must be a lowercase SHA-256")
    return value


def _candidate_lane_record(work_acquire: Any, lane_id: str) -> dict[str, Any]:
    try:
        with work_acquire._lane_lock(lane_id) as receipt_path:
            record = work_acquire._read_state(receipt_path)
    except Exception as exc:
        raise GripActionError("source Work Lane is not safely readable") from exc
    if not isinstance(record, dict) or record.get("lane_id") != lane_id:
        raise GripActionError("source Work Lane receipt is missing or mismatched")
    return record


def _candidate_process_readback(checkouts: Any, worktree: Path) -> list[dict[str, Any]]:
    try:
        observed = checkouts._processes_under([worktree])
    except Exception as exc:
        raise GripActionError("source workspace process liveness is not safely observable") from exc
    if not isinstance(observed, list) or any(not isinstance(item, dict) for item in observed):
        raise GripActionError("source workspace process readback is invalid")
    return observed


def _candidate_lane_lease_snapshots(
    resource_store: Any,
    *,
    owner_id: str,
    resource_keys: list[str],
    allow_all_absent: bool = False,
) -> list[dict[str, Any]]:
    try:
        observed = resource_store.inspect_resources(resource_keys)
    except Exception as exc:
        raise GripActionError("source Work Lane leases are not safely observable") from exc
    if not isinstance(observed, dict):
        raise GripActionError("source Work Lane lease readback is invalid")
    if not observed and allow_all_absent:
        return []
    if set(observed) != set(resource_keys):
        raise GripActionError("source Work Lane leases are partial, missing or drifted")
    snapshots: list[dict[str, Any]] = []
    for key in sorted(resource_keys):
        lease = observed.get(key)
        if not isinstance(lease, dict) or lease.get("owner_id") != owner_id:
            raise GripActionError("source Work Lane lease ownership drifted")
        snapshots.append(dict(lease))
    return snapshots
    ...
    value = _json_stdout(
        _github(
            ...
            [
                ...
                "number,url,state,baseRefName,headRefName,headRefOid,isDraft",
            ],
        )
    )
    if not isinstance(value, list):
        raise GripActionError("open PR readback returned an invalid shape")
    if len(value) > 1:
        raise GripActionError("multiple open PRs found for the adopted branch")
    if not value:
        return None
    item = value[0]
    if not isinstance(item, dict):
        raise GripActionError("open PR readback item is invalid")
    if (
        item.get("baseRefName") == base
        and item.get("headRefName") == branch
        and item.get("headRefOid") == expected_head
...
def _candidate_delivery_modules() -> tuple[Any, Any, Any, Any, Any, Any, Any]:
    workspace, work_acquire, lane_closeout, resource_store, checkouts = (
        _candidate_integration_modules()
    )
    import grabowski_candidate_adoption as delivery_profile
    import grabowski_execution_plan as execution_plan
...
def _delivery_remote_readback(
    ...
    *,
    ...
    result = _git_optional(repo, runner, ["ls-remote", "origin", f"refs/heads/{branch}"])
    if int(result.get("returncode", 1)) != 0:
        return {"state": "unavailable", "head": None}
    lines = [line.split() for line in str(result.get("stdout", "")).splitlines() if line.strip()]
    if not lines:
        return {"state": "absent", "head": None}
    if len(lines) != 1 or len(lines[0]) < 2 or re.fullmatch(r"[0-9a-f]{40}", lines[0][0]) is None:
        return {"state": "unavailable", "head": None}
    head = lines[0][0]
    return {"state": "exact" if head == expected_head else "drifted", "head": head}


def _delivery_pr_readback(
    repo: Path,
    github_runner: GithubRunner,
    *,
    branch: str,
        ...
        value = _json_stdout(
            _github(
                ...
                [
                    ...
                    "open",
                    "--json",
                    "number,url,state,baseRefName,headRefName,headRefOid,isDraft,title,body",
                ],
            )
        )
    except Exception as exc:
        return {"state": "unavailable", "pr": None, "error": type(exc).__name__}
    if not isinstance(value, list):
        return {"state": "unavailable", "pr": None, "error": "invalid_shape"}
    if len(value) > 1:
        return {"state": "duplicate", "pr": None, "count": len(value)}
    if not value:
        return {"state": "absent", "pr": None}
    item = value[0]
    if not isinstance(item, dict):
        return {"state": "unavailable", "pr": None, "error": "invalid_item"}
    ...
    if not structural_exact:
        return {"state": "drifted", "pr": dict(item)}
    metadata_exact = bool(
        item.get("isDraft") is draft
        and item.get("title") == title
        and item.get("body") == body
    )
    return {
        "state": "exact" if metadata_exact else "update_required",
        "pr": dict(item),
    }


def _delivery_local_commit_readback(
    worktree: Path,
    runner: CommandRunner,
    *,
    branch: str,
    ...
    try:
        observed_branch = str(
            _git(worktree, runner, ["branch", "--show-current"]).get("stdout", "")
        ).strip()
        observed_head = str(_git(worktree, runner, ["rev-parse", "HEAD"]).get("stdout", "")).strip().lower()
        parents = str(
            _git(worktree, runner, ["rev-list", "--parents", "-n", "1", observed_head]).get(
                "stdout", ""
            )
        ).strip().split()
        tree = str(
            _git(worktree, runner, ["rev-parse", f"{observed_head}^{{tree}}"]).get(
                "stdout", ""
            )
        ).strip().lower()
        message = str(
            _git(worktree, runner, ["show", "-s", "--format=%s", observed_head]).get(
                "stdout", ""
            )
        ).rstrip("\n")
        status = str(
            _git(worktree, runner, ["status", "--porcelain=v1", "--untracked-files=all"]).get(
                "stdout", ""
            )
        )
    except Exception:
    ...
    if not (
        ...
        and observed_head == head_commit
        and parents == [head_commit, base_commit]
...
def _delivery_identity_context(
    *,
    workspace: Any,
    work_acquire: Any,
    execution_plan: Any,
    ...
    manifest = workspace._manifest(workspace_id)
    if not isinstance(manifest, dict) or not workspace._lane_backed(manifest):
        raise GripPreflightError("delivery requires a lane-backed Agent Workspace")
    resources_value = manifest.get("resources")
    ...
    if not isinstance(lane_binding, dict):
        raise GripPreflightError("delivery Work Lane binding is missing")
    lane_id = lane_binding.get("lane_id")
    lane_receipt_sha256 = lane_binding.get("receipt_sha256")
    if (
        not isinstance(lane_id, str)
        or re.fullmatch(r"[0-9a-f]{32}", lane_id) is None
        or not isinstance(lane_receipt_sha256, str)
        or re.fullmatch(r"[0-9a-f]{64}", lane_receipt_sha256) is None
    ):
        raise GripPreflightError("delivery Work Lane identity is invalid")
    record = _candidate_lane_record(work_acquire, lane_id)
    terminal_closeout = record.get("terminal_closeout")
    if terminal_closeout is not None and (
        not isinstance(terminal_closeout, dict)
        or terminal_closeout.get("closeout_state") not in {"pr_opened", "pr_updated"}
    ):
        raise GripPreflightError("terminalized source lane cannot be revived for delivery")
    inputs = record.get("inputs")
    if not isinstance(inputs, dict):
        raise GripPreflightError("delivery Work Lane inputs are missing")
    plan_value = inputs.get("execution_plan")
    try:
        plan = execution_plan.validate_execution_plan(plan_value)
    except Exception as exc:
        raise GripPreflightError("delivery ExecutionPlan.v1 is invalid") from exc
    ...
    if (
        route.get("effect_profile") != "delivery"
        or route.get("executor") != "scoped_writer"
        or not isinstance(scoped_writer, dict)
        or scoped_writer.get("role") != "scoped_writer"
        or scoped_writer.get("actor") != f"scoped-writer:{route.get('writer_route')}"
        or inputs.get("source") != plan.get("source_binding")
        ...
        or (
            terminal_closeout is None
            and record.get("receipt_sha256") != lane_receipt_sha256
        )
        or (
            isinstance(terminal_closeout, dict)
            and terminal_closeout.get("expected_receipt_sha256")
            != lane_receipt_sha256
        )
        or not isinstance(allowed_paths, list)
        or sorted(allowed_paths) != plan.get("write_scope")
        or sorted(inputs.get("write_paths", [])) != plan.get("write_scope")
    ):
        raise GripPreflightError("delivery lane, route, plan, branch or scope identity drifted")
    collection, candidate, receipts, summary = workspace._adoption_collection_evidence(
        manifest,
        expected_candidate_id=expected_candidate_id,
        expected_result_sha256=expected_result_sha256,
    )
    workspace._require_closed_lane_workspace_for_adoption(manifest, collection)
    if terminal_closeout is None:
        workspace._require_adoption_lane_authority(manifest)
    if (
        candidate.get("workspace_id") != workspace_id
        or candidate.get("lane_id") != lane_id
        or candidate.get("base_head") != manifest.get("expected_base_head")
    ):
        raise GripPreflightError("delivery Candidate identity drifted from lane authority")
    owner_id = inputs.get("lease_owner_id")
    resource_keys = inputs.get("resource_keys")
    if not isinstance(owner_id, str) or not isinstance(resource_keys, list) or not resource_keys:
        raise GripPreflightError("delivery lane lease scope is invalid")
    return {
        "manifest": manifest,
        "record": record,
        "inputs": inputs,
        ...
        "base_commit": str(manifest["expected_base_head"]),
        "terminal_closeout": terminal_closeout,
    }


def _run_candidate_delivery_profile(
    *,
    workspace_id: str,
    expected_candidate_id: str,
    expected_result_sha256: str,
    ...
    (
        ...
        delivery_profile,
        execution_plan,
    ) = _candidate_delivery_modules()
    try:
        context = _delivery_identity_context(
            workspace=workspace,
        ...
        if context["terminal_closeout"] is None:
            _candidate_lane_lease_snapshots(
                resource_store,
                owner_id=context["owner_id"],
                resource_keys=context["resource_keys"],
            )
        candidate_tree = workspace._candidate_patch_tree_sha(
            context["manifest"], context["collection"], context["candidate"]
        )
    except Exception as exc:
        return {
            "state": "delivery_identity_drift",
            "delivery_ready": False,
            "reconcile_required": True,
            "leases_preserved": True,
            "next_action": "read_back_closed_candidate_lane_route_plan_branch_and_scope",
            "error": type(exc).__name__,
        }
    worktree = context["worktree"]
    branch = context["branch"]
    base_commit = context["base_commit"]
    round_number = int(context["candidate"]["round"])
    manifest_path = (
        workspace._workspace_dir(workspace_id)
        / f"delivery-manifest-round-{round_number:04d}.json"
    )
    delivery_manifest: dict[str, Any] | None = None
    terminal_replay = context["terminal_closeout"] is not None
    if terminal_replay and not manifest_path.exists():
        return {
    ...
    if manifest_path.exists():
        try:
            delivery_manifest = delivery_profile.read_candidate_delivery_manifest(
                manifest_path,
                candidate_manifest=context["candidate"],
                verification_receipts=context["receipts"],
                verification_summary=context["summary"],
                collection_result=context["collection"],
                execution_plan_value=context["plan"],
            )
        except Exception as exc:
            return {
                "state": "delivery_identity_conflict",
                "delivery_ready": False,
                "reconcile_required": True,
                "leases_preserved": True,
                "next_action": "inspect_immutable_delivery_manifest_before_retry",
                "error": type(exc).__name__,
            }
        expected_pr = delivery_manifest["actions"]["pr"]
        if (
            delivery_manifest["branch"]["writer_branch"] != branch
            or delivery_manifest["branch"]["base_branch"] != base
            or expected_pr.get("title") != title
            or expected_pr.get("body") != body
            or expected_pr.get("draft") is not False
        ):
            return {
                "state": "delivery_action_identity_stale",
                "delivery_ready": False,
                "reconcile_required": True,
                "leases_preserved": True,
                "next_action": "use_the_exact_manifest_bound_delivery_action",
            }
        head_commit = str(delivery_manifest["commit_range"]["head_commit"])
    else:
        try:
            current_head = str(
                _git(worktree, runner, ["rev-parse", "HEAD"]).get("stdout", "")
            ).strip().lower()
        except Exception as exc:
            return {
                "state": "delivery_commit_outcome_unknown",
                "delivery_ready": False,
                "reconcile_required": True,
                "leases_preserved": True,
                "next_action": "read_back_local_writer_branch_before_retry",
                "error": type(exc).__name__,
            }
        if current_head == base_commit:
            try:
                workspace._adoption_candidate_snapshot(
                    context["manifest"], context["collection"], context["candidate"]
                )
                staged_tree = workspace._stage_candidate_for_adoption(
                    context["manifest"], context["collection"], context["candidate"]
                )
                if staged_tree != candidate_tree:
                    raise GripActionError("staged Candidate tree identity changed")
                commit = _git(
                    worktree,
                    runner,
                    [
                        "-c",
                        "user.name=Grabowski Scoped Writer",
                        "-c",
                        "user.email=grabowski-writer@localhost",
                        "-c",
                        ...
                        base_commit,
                        "-m",
                        f"Deliver candidate {expected_candidate_id[:12]}",
                    ],
                )
                head_commit = str(commit.get("stdout", "")).strip().lower()
                if re.fullmatch(r"[0-9a-f]{40}", head_commit) is None:
                    raise GripActionError("writer delivery commit identity is invalid")
                update = _git_optional(
                    worktree,
                    runner,
                    ["update-ref", f"refs/heads/{branch}", head_commit, base_commit],
                )
                observed_head = str(
                    _git(worktree, runner, ["rev-parse", "HEAD"]).get("stdout", "")
                ).strip().lower()
                if observed_head != head_commit:
                    return {
                        ...
                        "leases_preserved": True,
                        "next_action": "read_back_local_writer_branch_before_retry",
                        "update_returncode": update.get("returncode"),
                    }
            except Exception as exc:
                return {
                    "state": "delivery_commit_outcome_unknown",
                    ...
                    "reconcile_required": True,
                    "leases_preserved": True,
                    "next_action": "read_back_local_writer_branch_and_candidate_tree_before_retry",
                    "error": type(exc).__name__,
                }
        else:
        ...
        local = _delivery_local_commit_readback(
            ...
            expected_message=f"Deliver candidate {expected_candidate_id[:12]}",
        )
        if local is None:
            return {
                "state": "delivery_commit_identity_drift",
                "delivery_ready": False,
                "reconcile_required": True,
                "leases_preserved": True,
                "next_action": "read_back_exact_single_parent_writer_commit_and_tree",
            }
        try:
            delivery_manifest = delivery_profile.build_candidate_delivery_manifest(
                candidate_manifest=context["candidate"],
                verification_receipts=context["receipts"],
                verification_summary=context["summary"],
                collection_result=context["collection"],
                execution_plan_value=context["plan"],
                lane_receipt_sha256=context["lane_receipt_sha256"],
                base_commit=base_commit,
                head_commit=head_commit,
                candidate_git_tree_sha=candidate_tree,
                commit_git_tree_sha=local["git_tree_sha"],
                writer_branch=branch,
                base_branch=base,
                title=title,
                body=body,
            ...
            delivery_profile.persist_candidate_delivery_manifest(
                manifest_path,
                delivery_manifest,
                candidate_manifest=context["candidate"],
                verification_receipts=context["receipts"],
            ...
            observed_manifest = delivery_profile.read_candidate_delivery_manifest(
                manifest_path,
                candidate_manifest=context["candidate"],
                verification_receipts=context["receipts"],
                verification_summary=context["summary"],
            ...
            if observed_manifest != delivery_manifest:
                raise GripActionError("immutable delivery manifest readback drifted")
        except Exception as exc:
            return {
                "state": "delivery_manifest_reconcile_required",
                "delivery_ready": False,
                "reconcile_required": True,
                "leases_preserved": True,
                "next_action": "read_back_immutable_delivery_manifest_before_any_publication",
                "error": type(exc).__name__,
            }
    ...
    if (
        local is None
        or delivery_manifest["commit_range"]["candidate_git_tree_sha"] != candidate_tree
        or delivery_manifest["commit_range"]["commit_git_tree_sha"] != candidate_tree
    ):
        if terminal_replay:
            return {
                "state": "terminal_delivery_replay_requires_new_lane",
        ...
        return {
            "state": "delivery_tree_mismatch",
            "delivery_ready": False,
            "reconcile_required": True,
            "leases_preserved": True,
            "next_action": "prove_commit_tree_equals_verified_candidate_tree",
        }

    remote = _delivery_remote_readback(
        worktree, runner, branch=branch, expected_head=head_commit
    )
    if terminal_replay and remote["state"] != "exact":
        return {
            "state": "terminal_delivery_replay_requires_new_lane",
            "delivery_ready": False,
            "reconcile_required": True,
    ...
    if remote["state"] in {"unavailable", "drifted"}:
        return {
            "state": "outcome_unknown" if remote["state"] == "unavailable" else "delivery_branch_drift",
            "delivery_ready": False,
            "reconcile_required": True,
            "leases_preserved": True,
            "delivery_manifest": delivery_manifest,
            "remote_readback": remote,
            "next_action": "authoritative_remote_branch_readback_before_retry",
        }
    branch_publication: dict[str, Any]
    if remote["state"] == "absent":
        branch_publication = run_grip(
            "branch-publish",
            {"repo": str(worktree), "branch": branch, "expected_head": head_commit},
            allow_mutation=True,
            command_runner=runner,
            github_runner=github_runner,
        )
        ...
        branch_publication = {"replayed": True, "remote_head": head_commit}

    pr_readback = _delivery_pr_readback(
        worktree,
        github_runner,
        branch=branch,
        base=base,
    ...
    if pr_readback["state"] in {"unavailable", "duplicate", "drifted"}:
        return {
            ...
            "branch_publication": branch_publication,
            "pr_readback": pr_readback,
            "next_action": "authoritative_open_pr_readback_before_retry",
        }
    if pr_readback["state"] == "exact":
        pr_publication = {"replayed": True, "pr": pr_readback["pr"]}
        pr_action = "reused"
    else:
        pr_publication = run_grip(
            "pr-create-or-update",
            {
                "repo": str(worktree),
                "branch": branch,
        ...
        action_output = pr_publication.get("output") if isinstance(pr_publication, dict) else None
        action_value = action_output.get("action") if isinstance(action_output, dict) else None
        pr_action = str(action_value) if action_value in {"created", "updated"} else "updated"
    return {
        ...
        "leases_preserved": True,
        "reconcile_required": False,
        "next_action": "controller_terminalize_source_lane_from_exact_pr_readback",
        "nonclaims": [
            "writer delivery does not merge",
            "writer delivery does not deploy",
            "writer delivery does not terminalize or release the source lane",
        ],
    }


def _controller_terminalize_delivered_source(
    *,
    delivery: dict[str, Any],
    runner: CommandRunner,
    github_runner: GithubRunner,
) -> dict[str, Any]:
    """Controller-only source closeout after exact delivery publication readback."""
    (
        workspace,
        work_acquire,
        lane_closeout,
    ...
    workspace_id = delivery.get("workspace_id")
    manifest_value = delivery.get("delivery_manifest")
    pr_expected = delivery.get("pr")
    if (
        delivery.get("delivery_ready") is not True
        or not isinstance(workspace_id, str)
        or not isinstance(manifest_value, dict)
        or not isinstance(pr_expected, dict)
    ):
        return {
            "state": "controller_terminalization_precondition_failed",
            "terminalized": False,
            "reconcile_required": True,
            "next_action": "revalidate_exact_delivery_manifest_remote_branch_and_pr",
        }
    try:
        manifest = workspace._manifest(workspace_id)
        resources_value = manifest.get("resources") if isinstance(manifest, dict) else None
        lane_binding = resources_value.get("lane_binding") if isinstance(resources_value, dict) else None
        if not isinstance(lane_binding, dict):
            raise GripActionError("source Work Lane binding is missing")
        lane_id = str(lane_binding["lane_id"])
        record = _candidate_lane_record(work_acquire, lane_id)
        inputs = record.get("inputs")
        if not isinstance(inputs, dict):
            raise GripActionError("source Work Lane inputs are missing")
        owner_id = str(inputs["lease_owner_id"])
        resource_keys = list(inputs["resource_keys"])
        worktree = Path(str(manifest["writer_worktree"]))
        branch = str(manifest["writer_branch"])
        base_commit = str(manifest["expected_base_head"])
        head_commit = str(delivery["resulting_commit_sha"])
        candidate_tree = str(delivery["candidate_git_tree_sha"])
        action_pr = manifest_value["actions"]["pr"]
        if (
            manifest_value["identity"]["workspace_id"] != workspace_id
            or manifest_value["identity"]["lane_id"] != lane_id
            or manifest_value["identity"]["lane_receipt_sha256"]
            != lane_binding.get("receipt_sha256")
            or manifest_value["commit_range"]["head_commit"] != head_commit
            or manifest_value["commit_range"]["candidate_git_tree_sha"] != candidate_tree
            or manifest_value["commit_range"]["commit_git_tree_sha"] != candidate_tree
            or manifest_value["branch"]["writer_branch"] != branch
        ):
            raise GripActionError("delivery manifest drifted before controller closeout")
    except Exception as exc:
        return {
            "state": "controller_terminalization_identity_drift",
            "terminalized": False,
            "reconcile_required": True,
            "next_action": "read_back_lane_delivery_manifest_and_local_commit",
            "error": type(exc).__name__,
        }
    ...
    if terminal_wrapper is None:
        ...
        try:
            ...
            if not isinstance(tasks, dict) or "writer" not in tasks:
                raise GripActionError("workspace task readback is incomplete")
            task_records = [item for item in tasks.values() if isinstance(item, dict)]
            if len(task_records) != len(tasks) or any(
                not isinstance(item.get("terminal"), bool) for item in task_records
            ):
                raise GripActionError("workspace task liveness is ambiguous")
            task_active = any(item["terminal"] is False for item in task_records)
            writer_state = tasks["writer"].get("state")
            ...
            local = _delivery_local_commit_readback(
                ...
                head_commit=head_commit,
                expected_tree=candidate_tree,
                expected_message=str(manifest_value["actions"]["commit"]["message"]),
            )
            remote = _delivery_remote_readback(
                worktree, runner, branch=branch, expected_head=head_commit
            )
            pr_readback = _delivery_pr_readback(
                ...
                github_runner,
                branch=branch,
                base=str(action_pr["base"]),
                expected_head=head_commit,
                title=str(action_pr["title"]),
                body=str(action_pr["body"]),
                draft=False,
            )
            if (
                task_active
                or process_active
                or local is None
                or remote.get("state") != "exact"
                or pr_readback.get("state") != "exact"
            ):
                raise GripActionError(
                    "controller closeout readback is not exact and inactive"
                )
            pr = pr_readback["pr"]
            pr_origin = "opened" if delivery.get("pr_action") == "created" else "updated"
            observation = lane_closeout.LaneCloseoutObservation(
                ...
                pr_state="open",
                pr_head_sha=head_commit,
                pr_origin=pr_origin,
            )
            assessment = lane_closeout.assess(observation)
            if (
                assessment.get("phase") != "terminal"
                or assessment.get("closeout_state") not in {"pr_opened", "pr_updated"}
                or assessment.get("lease_release_ready") is not True
            ):
                raise GripActionError("controller closeout assessment is not terminal")
            terminal_lane = work_acquire.persist_terminal_closeout(
                lane_id,
                assessment,
                expected_receipt_sha256=str(record["receipt_sha256"]),
        ...
        if (
            not isinstance(terminal_wrapper, dict)
            or terminal_wrapper.get("closeout_state") not in {"pr_opened", "pr_updated"}
        ):
            return {
                "state": "controller_terminalization_conflict",
                "terminalized": False,
                "reconcile_required": True,
    ...
    return {
        ...
        "later_correction": "new_lane_required",
        "nonclaims": [
            "source terminalization does not merge",
            "source terminalization does not deploy",
            "a later CI correction may not revive this source lane",
        ],
    }


def _run_candidate_integration_ready(
    spec: GripSpec,
    parameters: dict[str, Any],
    receipt: Receipt,
    runner: CommandRunner,
    ...
    allowed = {"workspace_id", "expected_candidate_id", "expected_result_sha256", "base", "title", "body"}
    unknown = sorted(set(parameters) - allowed)
    if unknown:
        raise GripPreflightError(
            "candidate-integration-ready does not accept caller-selected repository, branch, head or extra fields: "
            + ", ".join(unknown)
        )
    workspace_id = _string_parameter(parameters, "workspace_id")
    expected_candidate_id = _candidate_sha256_parameter(parameters, "expected_candidate_id")
    expected_result_sha256 = _candidate_sha256_parameter(parameters, "expected_result_sha256")
    base = _short_branch_name(parameters, "base")
    title = _string_parameter(parameters, "title")
    body_value = parameters.get("body", "")
    ...
    workspace, work_acquire, lane_closeout, resource_store, checkouts = _candidate_integration_modules()
    try:
        manifest = workspace._manifest(workspace_id)
    except Exception as exc:
        raise GripPreflightError("workspace is not safely readable") from exc
    if not isinstance(manifest, dict) or not workspace._lane_backed(manifest):
        raise GripPreflightError("candidate integration requires a lane-backed Agent Workspace")
    resources_value = manifest.get("resources")
    lane_binding = resources_value.get("lane_binding") if isinstance(resources_value, dict) else None
    if not isinstance(lane_binding, dict):
        raise GripPreflightError("workspace Work Lane binding is missing")
    lane_id = lane_binding.get("lane_id")
    expected_lane_receipt_sha256 = lane_binding.get("receipt_sha256")
    if (
        not isinstance(lane_id, str)
        or re.fullmatch(r"[0-9a-f]{32}", lane_id) is None
        or not isinstance(expected_lane_receipt_sha256, str)
        or re.fullmatch(r"[0-9a-f]{64}", expected_lane_receipt_sha256) is None
    ):
        raise GripPreflightError("workspace Work Lane identity is invalid")

    try:
        adoption = workspace.grabowski_agent_workspace_adopt(workspace_id, expected_candidate_id, expected_result_sha256)
    except Exception as exc:
        _check(receipt, "candidate-adoption-bound", "fail", "adoption raised; reconcile before retry")
        raise GripActionError("Candidate adoption did not return a trustworthy outcome; reconcile the workspace before retry") from exc
    if not isinstance(adoption, dict) or adoption.get("state") != "adopted":
        _check(receipt, "candidate-adoption-bound", "fail", str(adoption.get("state") if isinstance(adoption, dict) else "invalid-output"))
        return {"state": "adoption_reconcile_required", "integration_ready": False, "workspace_id": workspace_id, "candidate_id": expected_candidate_id, "adoption": adoption, "reconcile_required": True, "next_action": "read_back_candidate_adoption_before_retry", "receipt_status": "blocked"}
    adoption_receipt = adoption.get("adoption_receipt")
    if (
        adoption.get("candidate_id") != expected_candidate_id
        or not isinstance(adoption_receipt, dict)
        or adoption_receipt.get("candidate_id") != expected_candidate_id
        or not isinstance(adoption_receipt.get("receipt_sha256"), str)
        or re.fullmatch(r"[0-9a-f]{64}", str(adoption_receipt.get("receipt_sha256"))) is None
        or not isinstance(adoption_receipt.get("resulting_commit_sha"), str)
        or re.fullmatch(r"[0-9a-f]{40}", str(adoption_receipt.get("resulting_commit_sha"))) is None
    ):
        _check(receipt, "candidate-adoption-bound", "fail", "adoption receipt binding mismatch")
        raise GripActionError("Candidate adoption receipt is not bound to the requested Candidate")
    adopted_commit = str(adoption_receipt["resulting_commit_sha"])
    adoption_receipt_sha256 = str(adoption_receipt["receipt_sha256"])
    _check(receipt, "candidate-adoption-bound", "pass", adopted_commit)
    ...
    if not isinstance(inputs, dict):
        raise GripActionError("source Work Lane inputs are missing")
    owner_id = inputs.get("lease_owner_id")
    resource_keys_value = inputs.get("resource_keys")
    if (
        not isinstance(owner_id, str)
        or not isinstance(resource_keys_value, list)
        or not resource_keys_value
        or any(not isinstance(item, str) for item in resource_keys_value)
    ):
        raise GripActionError("source Work Lane lease identity is invalid")
    resource_keys = list(resource_keys_value)
    worktree = Path(str(manifest.get("writer_worktree", ""))).expanduser()
    if not worktree.is_absolute():
        raise GripActionError("workspace writer worktree is invalid")
    ...
    if record.get("terminal_closeout") is None:
        ...
        try:
            ...
            raise GripActionError("workspace status is not safely observable after adoption") from exc
        tasks = status.get("tasks") if isinstance(status, dict) else None
        if not isinstance(tasks, dict) or "writer" not in tasks:
            raise GripActionError("workspace task liveness readback is incomplete")
        task_records = [item for item in tasks.values() if isinstance(item, dict)]
        if len(task_records) != len(tasks) or any(not isinstance(item.get("terminal"), bool) for item in task_records):
            raise GripActionError("workspace task liveness readback is ambiguous")
        task_active = any(item["terminal"] is False for item in task_records)
        writer_state = tasks["writer"].get("state")
        if not isinstance(writer_state, str):
            raise GripActionError("workspace writer state is unavailable")
        ...
        observation = lane_closeout.LaneCloseoutObservation(
            ...
            workspace=str(worktree),
            branch=branch,
            base_revision=str(manifest.get("expected_base_head")),
            writer_state=writer_state,
            task_active=task_active,
            process_active=process_active,
            lease_active=True,
            git_dirty=orientation.get("dirty"),
            head_sha=orientation.get("head"),
            candidate_id=expected_candidate_id,
            adoption_receipt_sha256=adoption_receipt_sha256,
            adoption_commit_sha=adopted_commit,
        )
        try:
            assessment = lane_closeout.assess(observation)
        except Exception as exc:
            raise GripActionError("source Work Lane terminal assessment failed") from exc
        if assessment.get("phase") != "terminal" or assessment.get("closeout_state") != "candidate_adopted" or assessment.get("lease_release_ready") is not True:
            _check(receipt, "source-lane-terminal", "fail", ",".join(assessment.get("reason_codes", [])) or str(assessment.get("phase")))
            return {"state": "source_lane_reconcile_required", "integration_ready": False, "workspace_id": workspace_id, "candidate_id": expected_candidate_id, "adoption": adoption, "lane_assessment": assessment, "reconcile_required": True, "next_action": "refresh_task_process_lease_and_git_readback", "receipt_status": "blocked"}
        try:
            terminal_lane = work_acquire.persist_terminal_closeout(
                lane_id,
                assessment,
            ...
            raise GripActionError("source Work Lane terminal closeout did not persist safely") from exc
    else:
        terminal_lane = record
        wrapper = record.get("terminal_closeout")
        if not isinstance(wrapper, dict) or wrapper.get("closeout_state") != "candidate_adopted":
            _check(receipt, "source-lane-terminal", "fail", "different terminal state already exists")
            return {"state": "source_lane_terminal_conflict", "integration_ready": False, "workspace_id": workspace_id, "candidate_id": expected_candidate_id, "adoption": adoption, "reconcile_required": True, "next_action": "inspect_existing_source_lane_terminal_state", "receipt_status": "blocked"}
    _check(receipt, "source-lane-terminal", "pass", "candidate_adopted")

    orientation = _run_repo_orient(spec, {"repo": str(worktree)}, receipt, runner)
    if orientation.get("head") != adopted_commit or orientation.get("branch") != branch or orientation.get("dirty") is not False:
        _check(receipt, "branch-published", "fail", "local adopted branch drifted before publication")
        return {"state": "publication_reconcile_required", "integration_ready": False, "workspace_id": workspace_id, "candidate_id": expected_candidate_id, "adoption": adoption, "source_lane": terminal_lane, "reconcile_required": True, "next_action": "read_back_local_adopted_branch_before_publication", "receipt_status": "blocked"}

    existing_ready_pr: dict[str, Any] | None = None
    remote_result = _git_optional(worktree, runner, ["ls-remote", "origin", f"refs/heads/{branch}"])
    remote_line = str(remote_result.get("stdout", "")).splitlines()[0] if int(remote_result.get("returncode", 1)) == 0 and remote_result.get("stdout") else ""
    remote_head = remote_line.split()[0] if remote_line.split() else ""
    if remote_head == adopted_commit:
        try:
            existing_ready_pr = _candidate_open_pr_readback(
                worktree,
                github_runner,
                branch=branch,
                base=base,
                expected_head=adopted_commit,
            )
        except GripActionError as exc:
            _check(receipt, "pr-exact-head", "fail", "open PR readback unavailable")
            return {
                "state": "pr_readback_reconcile_required",
                "integration_ready": False,
                "workspace_id": workspace_id,
                "candidate_id": expected_candidate_id,
                "adoption": adoption,
                "source_lane": terminal_lane,
                "remote_head": adopted_commit,
                "error": str(exc),
                "leases_preserved": True,
                "reconcile_required": True,
                "next_action": "read_back_open_pr_before_any_publication_retry",
                "receipt_status": "blocked",
            }
    if existing_ready_pr is not None:
        branch_publication: dict[str, Any] = {"replayed": True, "remote_head": adopted_commit, "receipt": None}
        pr_publication: dict[str, Any] = {"replayed": True, "pr": existing_ready_pr, "receipt": None}
        _check(receipt, "branch-published", "pass", adopted_commit)
        _check(receipt, "pr-exact-head", "pass", str(existing_ready_pr.get("number")))
    else:
        try:
            _candidate_lane_lease_snapshots(resource_store, owner_id=owner_id, resource_keys=resource_keys)
        except GripActionError as exc:
            _check(receipt, "branch-published", "fail", "publication coordination leases are absent or drifted")
            return {"state": "publication_coordination_missing", "integration_ready": False, "workspace_id": workspace_id, "candidate_id": expected_candidate_id, "adoption": adoption, "source_lane": terminal_lane, "error": str(exc), "reconcile_required": True, "next_action": "establish_new_controller_publication_authority_before_mutation", "receipt_status": "blocked"}
        if remote_head == adopted_commit:
            branch_publication = {"replayed": True, "remote_head": adopted_commit, "receipt": None}
        else:
            branch_publication = run_grip(
                "branch-publish",
                {"repo": str(worktree), "branch": branch, "expected_head": adopted_commit},
                allow_mutation=True,
                command_runner=runner,
                github_runner=github_runner,
            )
            if branch_publication.get("receipt", {}).get("status") != "passed":
                _check(receipt, "branch-published", "fail", "branch publication did not reach exact readback")
                return {"state": "branch_publication_reconcile_required", "integration_ready": False, "workspace_id": workspace_id, "candidate_id": expected_candidate_id, "adoption": adoption, "source_lane": terminal_lane, "branch_publication": branch_publication, "leases_preserved": True, "reconcile_required": True, "next_action": "read_back_remote_branch_before_retry", "receipt_status": "blocked"}
        _check(receipt, "branch-published", "pass", adopted_commit)
        pr_publication = run_grip(
            "pr-create-or-update",
            {"repo": str(worktree), "branch": branch, "base": base, "expected_head": adopted_commit, "title": title, "body": body, "draft": False},
            allow_mutation=True,
            command_runner=runner,
            github_runner=github_runner,
        )
        if pr_publication.get("receipt", {}).get("status") != "passed":
            _check(receipt, "pr-exact-head", "fail", "PR publication did not reach exact readback")
            return {"state": "pr_publication_reconcile_required", "integration_ready": False, "workspace_id": workspace_id, "candidate_id": expected_candidate_id, "adoption": adoption, "source_lane": terminal_lane, "branch_publication": branch_publication, "pr_publication": pr_publication, "leases_preserved": True, "reconcile_required": True, "next_action": "read_back_open_pr_before_retry", "receipt_status": "blocked"}
        pr = pr_publication.get("output", {}).get("pr")
        if not isinstance(pr, dict):
            raise GripActionError("PR publication omitted exact readback")
        _check(receipt, "pr-exact-head", "pass", str(pr.get("number")))
    ...
    return {
        ...
        "source_lane_closeout_state": "candidate_adopted",
        "branch": branch,
        "base": base,
        "pr": existing_ready_pr,
        "branch_publication": branch_publication,
        "pr_publication": pr_publication,
        "lease_release": lease_release,
        "reconcile_required": False,
...
_RUNNERS = {
    ...
    "remote_head_materialize": _run_remote_head_materialize,
