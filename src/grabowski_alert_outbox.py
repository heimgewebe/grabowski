...
EVENT_PRESENTATION = {
    "blocked_operation": ("Grabowski operation blocked", "high", ["warning"]),
    "recovery": ("Grabowski recovery completed", "default", ["white_check_mark"]),
    "service_failure": ("Grabowski service failure", "high", ["rotating_light"]),
    "long_run_completed": ("Grabowski long run completed", "default", ["checkered_flag"]),
    "owner_decision": ("Grabowski owner decision recorded", "default", ["memo"]),
}
ALERT_NON_CLAIMS = [
...
def notification_payload(alert: dict[str, Any]) -> dict[str, str]:
    candidate = dict(alert)
    candidate.pop("file_sha256", None)
    validated = _validate_alert(candidate)
    title, priority, tags = EVENT_PRESENTATION[validated["event_class"]]
    body = f"{title}: {validated['subject']}"
    if validated["fields"]:
        body += " (" + ", ".join(
            f"{key}={value}" for key, value in validated["fields"].items()
        ) + ")"
    return {
        "title": title,
        "body": body,
