...
For ordinary agent browsing, use the `browser_operator_contract` path by default: Grabowski owns lifecycle; Google Chrome Stable is `canonical-operator`; one-off work uses an isolated ephemeral profile and loopback-only CDP. For recurring authenticated provider work, start with a stable named `operator_profile`, resolve it only under `~/.local/state/grabowski/browser-profiles`, and reuse that same name after controlled worker restarts so the managed session persists. Never substitute a normal human Chrome/Brave/Chromium/Firefox desktop profile. Drive navigation/actions/readback directly over CDP; treat `target_unavailable` as a bounded stop/restart condition rather than an unbounded retry; then stop the worker and verify profile/port lease cleanup while preserving persistent profile state. Brave remains the human desktop default/fallback and is not agent-primary. Vendor MCPs are optional diagnostics/adapters, never browser lifecycle authority. Firefox/WebDriver BiDi remains a separate future adapter.
...
`grabowski_status` exposes the live registered/expected tool counts and name hashes. A client-side count or hash mismatch requires a connector refresh; the runtime cannot refresh ChatGPT's frozen snapshot itself. The top-level response `schema_version` is 3 for `minimal`/`concise` and 2 for `standard`/`evidence`. In schema 3, `transport_roundtrip.mutation_gate_open` is always present and is the fail-closed effective readiness of the selected normal mutation path. The previously published selected-path fields `normal_mutation_path_ready`, `normal_mutation_path`, and `legacy_roundtrip_required` keep their existing semantics and remain optional when the transport status cannot select a normal path (for example `connector_identity_required` or `runtime_invalid`); absence is not readiness, and `mutation_gate_open` is then `false`. In schema 2, `mutation_gate_open` remains the raw legacy roundtrip gate while `normal_mutation_path_ready` describes the selected normal path when present. Consumers shared across views must branch on the top-level response `schema_version`; only schema-3 consumers may use `mutation_gate_open` as the selected-path gate.
...
Operator Relay v0 is the fallback rule for blocked ChatGPT/Grabowski actions: try a typed Grabowski tool first; if that is blocked, use one bounded Grabowski Micro-Task; then read status, logs, diff or another receipt before deciding the next step. For helper routing, use Codex as the default for complex code or repo tasks, Antigravity (`agy --print`) for quick light reasoning, Ollama API with qwen coder for local micro-reasoning, Claude for review, tmux first for session/resume, Bureau for prioritization, and Grabowski + Git for audit. Local patch files should go through `tools/operator_patch_relay.py` for check/apply receipts before asking the user to download and run a patch manually. Aider remains a bounded patch fallback with no auto-commit. Details: `docs/blocked-action-protocol-v0.md`.

For repo, PR, branch, pull, switch and merge-prep work, derive target state from the narrowest native typed reads: `grabowski_git_status` for the exact checkout, `grabowski_checkout_inventory` when linked-checkout identity or lifecycle matters, and `grabowski_github_pr_view` / `grabowski_github_checks` for remote PR truth. These reads are observations, not approval gates; mutation authority remains in the action-specific lease, review, CI and Captain contracts.
...
`--external-review-evidence`, `tools/external_review_claude.py`, and the provider-neutral `tools/external_review_plain.py` remain available for optional second opinions or incident investigation. The plain path can call account-backed Gemini or Grok in one isolated turn through a fixed environment allowlist, null stdin, a bounded process group, owner-controlled executable identity, and pre/post workspace readback; it emits create-only prompt, raw-response, and schema-valid evidence artifacts. The central gate independently reconstructs the nonce-bound prompt from the current PR and diff and requires the evidence to declare no review-gate authority. Grok receives the prompt through a short-lived `0600` file and must resolve to the canonical native binary; Gemini still uses an explicitly recorded argv transport. Cost and quota remain a separate route-preflight truth, not an adapter claim. `tools/external_review_antigravity.py` remains a Gemini compatibility wrapper. Invalid optional external evidence is warned about but does not block a valid self-review. Legacy `external_review_required`, `self_review_required=false`, and Claude policy waivers are deprecated and ignored.
