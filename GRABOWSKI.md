...
`tools/external_review_agy.py` may be used as an optional external LLM provider for a packet produced by `--write-external-review-packet`. It must be run against the packet manifest and writes ordinary `external_review` evidence; it is not a privileged trust anchor. The provider invokes `gemini`/`agy` with the full diff embedded in the prompt, requires non-empty parseable output, fails closed on timeout or upstream errors, and only marks evidence triaged automatically for `PASS` with `finding_count: 0`. Non-PASS reviews still require terminal top-level triage before the gate can pass.
