...
`reposkop report <absolute-target> --purpose grabowski-repo-state-context --json` is available as a target-bound read-only coherence signal for repo, PR, branch, pull, switch and merge-prep work when the selected repository state matters. The probe run is accepted; do not keep a separate useful-signal/decision-change/noise trial log. Only fields derived from the explicitly selected target count. The report is not an approval gate and does not replace Git status, PR checks, review gates or action readiness.
