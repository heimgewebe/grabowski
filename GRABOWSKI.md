...
`tools/external_review_claude.py` is the required provider for high-critical PRs and for every non-documentation-only PR in `heimgewebe/weltgewebe`. It binds Claude CLI `ultrareview` to the packet, live PR head, and live diff before and after review, records that the packet prompt was not transmitted, and fails closed on missing PR-input binding, drift, timeout, command failure, or invalid JSON. `tools/external_review_agy.py` remains an optional provider for ordinary non-exempt PRs. Neither provider replaces self-review, CI, mergeability, or finding triage.
