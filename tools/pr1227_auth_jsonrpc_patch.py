...
RUNNER = Path("tools/repobrief_agent_benchmark_codex_runner.py")
TESTS = Path("tests/test_repobrief_agent_benchmark_codex_runner.py")
...
def block(text: str, start: str, end: str, new: str, label: str) -> str:
    if text.count(start) != 1 or text.count(end) < 1:
        raise SystemExit(f"{label}: block markers are not unique/present")
...
'''
runner = once(runner, helper_marker, "\n" + helpers + helper_marker.lstrip("\n"), "authorization helpers")

new_bind = '''def _bind_mcp_upstream(
...
''',
    "build command proxy authorization",
)
runner = once(
    runner,
...
''',
    "proxy CLI authorization",
)

new_proxy_helper = '''def file_identity(path: Path) -> dict:
...
'''
tests = block(tests, "def proxy_command(", "\n\ndef answer()", new_proxy_helper, "test proxy helper")
tests = once(
    tests,
...
''',
    "test symlink authorization",
)

insert = '''    def test_mcp_proxy_rejects_missing_treatment_request_id(self) -> None:
'''
new_tests = '''    def test_preflight_mcp_authorization_binds_exact_program_files_before_launch(self) -> None:
...
'''
tests = once(tests, insert, new_tests + insert, "new regression tests")
