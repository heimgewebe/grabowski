#!/usr/bin/env python3
"""CLI for the explicit anonymous GitHub web StructuredToolProvider backend."""
...
from grabowski_browser_structured_provider_github_web import (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Inspect or execute the explicit anonymous github.public-web repository.read "
            "provider. This tool has no provider selection, routing, fallback, session or credentials."
        )
    )
    ...
    read = sub.add_parser("read", help="perform one bounded anonymous public repository page read")
...
def _failure(exc: Exception) -> dict[str, Any]:
    return {
        "schema_version": 1,
        "kind": "github_web_structured_tool_provider_cli_failure",
        "state": "failed_closed",
