...
class AntigravityReviewError(RuntimeError):
    pass
...
def load_json(path: Path, *, label: str) -> dict[str, Any]:
    try:
        ...
        raise AntigravityReviewError(f"cannot read {label}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise AntigravityReviewError(f"cannot parse {label} as JSON: {exc}") from exc
    if not isinstance(data, dict):
        raise AntigravityReviewError(f"{label} is not a JSON object")
    return data
...
def resolve_packet_file(manifest_path: Path, value: Any, *, label: str) -> Path:
    if not isinstance(value, str) or not value.strip():
        raise AntigravityReviewError(f"manifest {label} is missing or not a string")
    raw = Path(value)
    ...
    try:
        ...
        raise AntigravityReviewError(f"cannot resolve manifest {label}: {exc}") from exc
    if not is_inside(resolved, packet_dir):
        raise AntigravityReviewError(f"manifest {label} escapes external review packet directory")
    if not resolved.is_file():
        raise AntigravityReviewError(f"manifest {label} is not a regular file")
    return resolved
...
def validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    for key, expected in required.items():
        if manifest.get(key) != expected:
            raise AntigravityReviewError(f"manifest {key} is not {expected!r}")
    for key in ("repo", "head_sha", "diff_sha256", "prompt_sha256", "diff_path", "prompt_path"):
        if not manifest.get(key):
            raise AntigravityReviewError(f"manifest {key} is missing")
    pr = manifest.get("pr")
    if isinstance(pr, bool) or not isinstance(pr, int) or pr <= 0:
        raise AntigravityReviewError("manifest pr is not a positive integer")


def build_antigravity_prompt(packet_prompt: str, diff_text: str) -> str:
    return (
...
def parse_review_json(stdout: str) -> dict[str, Any]:
    ...
    try:
        ...
        if start < 0 or end <= start:
            raise AntigravityReviewError("Antigravity review output does not contain a JSON object")
        try:
            parsed = json.loads(text[start : end + 1])
        except json.JSONDecodeError as exc:
            raise AntigravityReviewError(f"Antigravity review output JSON is invalid: {exc}") from exc
    if not isinstance(parsed, dict):
        raise AntigravityReviewError("Antigravity review output JSON is not an object")
    verdict = parsed.get("verdict")
    if verdict not in VERDICTS:
        raise AntigravityReviewError("Antigravity review verdict is missing or invalid")
    ...
    if isinstance(finding_count, bool) or not isinstance(finding_count, int) or finding_count < 0:
        raise AntigravityReviewError("Antigravity review finding_count is not an integer >= 0")
    findings = parsed.get("findings")
    if not isinstance(findings, list):
        raise AntigravityReviewError("Antigravity review findings is not a list")
    return parsed
...
def run_antigravity(
    ...
    *,
    ...
    argv = [antigravity_bin, f"--print-timeout={timeout_seconds}s"]
    if model:
...
def run_from_manifest(
    *,
    ...
    if actual_diff_sha256 != manifest["diff_sha256"]:
        raise AntigravityReviewError("diff file sha256 does not match manifest")
    ...
    if packet_prompt_sha256 != manifest["prompt_sha256"]:
        raise AntigravityReviewError("prompt file sha256 does not match manifest")
    prompt = build_antigravity_prompt(packet_prompt, diff_bytes.decode("utf-8", errors="replace"))
    prompt_bytes = prompt.encode("utf-8")
    if len(prompt_bytes) > max_prompt_bytes:
        raise AntigravityReviewError(
            f"Antigravity prompt is too large for argv transport: {len(prompt_bytes)} bytes > {max_prompt_bytes}"
        )
    completed = run_antigravity(prompt, antigravity_bin=antigravity_bin, model=model, timeout_seconds=timeout_seconds)
    if completed.returncode != 0:
        raise AntigravityReviewError(f"Antigravity CLI exited with {completed.returncode}: {strip_ansi(completed.stderr or completed.stdout).strip()}")
    if not completed.stdout.strip():
        raise AntigravityReviewError("Antigravity CLI returned empty stdout")
    ...
    evidence = build_evidence(
        ...
        review=review,
        source=f"antigravity:{model or 'default'}",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run Antigravity external review for a pr_review_gate packet.")
    ...
    parser.add_argument("--raw-review-output", help="optional path for raw Antigravity stdout")
    parser.add_argument("--antigravity-bin", "--gemini-bin", dest="antigravity_bin", default="agy")
