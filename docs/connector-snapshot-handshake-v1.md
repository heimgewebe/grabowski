...
`client_snapshot_observable=true` means only that a fresh client declaration was compared with and matched the current server contract. It does not mean the platform independently attested the client process. `grabowski_status(view="minimal")` uses response schema 3. In that schema, `transport_roundtrip.normal_mutation_path_ready` and `transport_roundtrip.mutation_gate_open` are always present and carry the same effective normal-path readiness used by warnings and `recommended_next_action`; `connector_identity_required` and `runtime_invalid` therefore expose both as `false`. Schema-2 standard/evidence are diagnostic views: their `mutation_gate_open` is the raw legacy roundtrip gate, while `normal_mutation_path_ready` describes the selected normal path when available. Consumers must interpret these keys under the response `schema_version`, not carry the schema-2 legacy-gate meaning into schema 3.
