...
`client_snapshot_observable=true` means only that a fresh client declaration was compared with and matched the current server contract. It does not mean the platform independently attested the client process. `grabowski_status(view="minimal")` uses response schema 3. For compatibility it retains `transport_roundtrip.normal_mutation_path_ready` and additionally exposes `transport_roundtrip.mutation_gate_open` as the explicit effective readiness of the selected normal mutation path; both values are equivalent in schema 3. New consumers should use `mutation_gate_open`. Schema-2 standard/evidence retain the raw legacy gate as diagnostic detail alongside `normal_mutation_path_ready`.
