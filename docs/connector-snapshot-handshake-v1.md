...
`client_snapshot_observable=true` means only that a fresh client declaration was compared with and matched the current server contract. It does not mean the platform independently attested the client process. `grabowski_status(view="minimal")` uses top-level response schema 3. In that schema, `transport_roundtrip.mutation_gate_open` is always present and is the fail-closed effective readiness of the selected normal mutation path. The compatibility fields `normal_mutation_path_ready`, `normal_mutation_path`, and `legacy_roundtrip_required` are projected only when the underlying transport status exposes a selected path; early degraded states such as `connector_identity_required` and `runtime_invalid` may omit them, and that absence must be treated as not ready. Schema-2 standard/evidence remain diagnostic views: their `mutation_gate_open` is the raw legacy roundtrip gate, while `normal_mutation_path_ready` describes the selected normal path when available. Consumers must interpret these keys under the top-level response `schema_version`.
