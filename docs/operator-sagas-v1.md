...
### Prospective post-fix terminal pairing — 2026-09-07

PR #1106 has merged the Bureau-liveness semantic repair. Its merge remains a prerequisite for prospective acceptance, but neither that completed effect nor any historical T121 run, TaskSpec digest, baseline SHA, deployment, or prose in this document is current execution authority.

This documentation-only PR is a prospective PR-settlement target, not an authority record. Before any future PR-settlement `saga-plan` or durable `saga-run`, the PR must be current with protected `main`; required CI and the policy-derived self-review must pass for one exact head/base/diff; and, if Bureau participation is included, a freshly observed Bureau run must be active with non-blocking coordination, `active-bound` required leases, and an `actively_bound` execution binding. Historical run IDs or hashes may remain evidence only and must never be rebound to a later PR identity.

Only after those preconditions converge may a fresh `saga-plan` and durable `saga-run` freeze the exact PR identity and, when applicable, the exact live Bureau execution binding. From that run through Captain completion, any head, base, diff, review-bound identity, or Bureau execution-binding drift invalidates the attempt and requires a new plan/run. No update-branch, rebase, commit, force-push, or retroactive authority rebinding is valid inside the frozen window.

Pilot 1 exists only when verified Captain merge, authoritative GitHub readback, verified Captain audit binding, and reference-based `saga-settle == settled` all agree on the same frozen identity. Pilot 2 may target only that exact settled merge commit while it is still protected `main`; it then requires its own fresh runtime-deployment plan and durable run reference, normal Captain deployment of exactly that commit, authoritative deployment-identity convergence, verified Captain audit binding, and reference-based `saga-settle == settled`. If protected `main` advances first, the attempt stops rather than deploying stale repository state.

### Terminal acceptance attempt — 2026-09-10

The current coordinated T121 assignment is Bureau run `BUR-RUN-20260910T204821Z-24ca394c37`, bound to TaskSpec digest `633456e6dfa0c2774f4ded0a3da5a38256f0cf0069581278d766701aea9c7db7` and workspace baseline `38555286d263aeffefcfdcc76553d3a4f5b8133e`. Protected `main` subsequently advanced by the scope-disjoint PR #1159 to `6f792cfdc05e2c3989b4ab725b984cac423a9eea`; that advance touches none of T121's declared write paths and is not itself pilot evidence.

This documentation-only change is the next prospective PR-settlement target. Before `saga-plan` or durable `saga-run`, the PR must converge to the then-current protected `main` and pass the normal CI/review gates on one exact head/base/diff. The resulting Saga run freezes that identity: any later head, base, diff or review-binding change invalidates the attempt before Captain instead of being retroactively rebound.

Pilot 1 is established only by a verified Captain merge, authoritative GitHub readback, verified Captain audit binding and reference-based `saga-settle == settled` for that frozen identity. Pilot 2 may then target only that exact settled merge commit while it is still protected `main`; it independently requires a fresh runtime-deployment plan and durable run reference, Captain deployment of exactly that commit, deployment-identity convergence and reference-based `saga-settle == settled`. The final Bureau acceptance evidence, not this prospective paragraph, owns the measured before/after call counts, operator decisions, blocks, partial failures and elapsed times.

### Recovered exact terminal pairing — 2026-09-11

The coordinated T121 assignment `BUR-RUN-20260910T204821Z-24ca394c37` was resumed through its exact journal-bound retry path after its prior lease generation expired. The recovery reused the same run and TaskSpec, migrated its registry binding only through the canonical successor proof, and reacquired the declared T121 path leases. PR #1162 remains useful PR-effect and partial-failure evidence, but it is not the final paired acceptance: protected `main` advanced before its merge commit could begin the required runtime-deployment Saga, so that older commit is never deployed merely to complete the test.

This documentation-only continuation is the fresh PR-settlement target. It may be frozen by `saga-plan` and durable `saga-run` only after it has converged to the then-current protected `main` and its CI/review evidence is exact for one head/base/diff. Pilot 1 requires the normal Captain merge, authoritative GitHub readback, verified Captain audit binding and reference-based `saga-settle == settled` for that unchanged identity.
