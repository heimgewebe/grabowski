...
This documentation-only PR is a prospective PR-settlement target, not an authority record. Before any future PR-settlement `saga-plan` or durable `saga-run`, the PR must be current with protected `main`; required CI and high-critical self-review must pass for one exact head/base/diff; and, if Bureau participation is included, a freshly observed Bureau run must be active with non-blocking coordination, `active-bound` required leases, and an `actively_bound` execution binding. Historical run IDs or hashes may remain evidence only and must never be rebound to a later PR identity.
