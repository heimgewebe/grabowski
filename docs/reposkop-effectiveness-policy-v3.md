...
Independently of that decision path, eligible local repository writers have a best-effort checkout-continuity shadow. It records a purpose-bound canonical Reposkop BEFORE observation before launch, then asks Reposkop for its canonical continuity artifact after authoritative task terminalization. This applies to both prospective sample and control cohorts. Shadow failure is recorded as unavailable but never changes task start, terminal state, leases, resume authority, publication, merge, deployment or recovery. The private evidence is hash-bound; public audit contains only bounded identifiers, artifact digests, canonical state/reason/anomaly codes and `decision_effect=false`.
