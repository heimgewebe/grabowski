...
3. Der Helper ergänzt ausschließlich die commitgebundenen Verträge für `publish_recovery_marker`, `operator_blockade_marker_lifecycle` und `operator_root_task_systemd_unit` sowie die daraus abgeleiteten Power-Gate-Felder. Eine bereits vorhandene Root-Task-Aktion muss bytewertgleich zum gebundenen Commit sein; Abweichungen werden nicht überschrieben. Alle übrigen Aktionen und alle nicht migrierten Felder von `operator_power_argv` bleiben bytewertgleich.
