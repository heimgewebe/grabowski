...
The relay reuses the validated Fleet identity `heim-pc`, but callers never supply an executable, shell program or remote path. The implementation constructs one server-owned Python bootstrap and accepts only canonical JSON for a small allowlist of existing typed operations. The primary process imports the deployed Grabowski runtime and executes the same high-level typed adapter there. For Bureau this is important: Resource-Leases, Bureau runtime checks, publication receipts and canonical registry effects stay with the authority host that executes the typed operation; the secondary does not maintain a parallel registry or lifecycle database.

Proposal lifecycles are authority-affine. A proposal whose immutable local plan artifact already exists must remain local; losing that local authority fails closed rather than migrating the proposal mid-lifecycle. Conversely, a proposal created through the primary relay has no local plan artifact and its review/preview/publication operations continue on the primary even if the secondary local Bureau authority later becomes available. The immutable proposal artifact therefore supplies the affinity evidence without a second mutable routing database.

The relay response is bound to the canonical request SHA-256 and the serving primary runtime identity. Clean primary refusals explicitly attest `effect_dispatched=false`; only failures after dispatch become `ambiguity=true` for mutations. Ambiguous failures retain the request SHA-256 in the secondary audit and require target-state readback; they are never retried unchanged. The existing G6.5 Heimberry fence remains the mutation authority before the secondary typed effect, and every mutating relay additionally requires the enforcement configuration to be active.

Failback remains automatic for stateless reads and new work: every new operation tries the local canonical authority first. Proposal-scoped operations are the deliberate exception because their immutable plan artifact binds the lifecycle to its originating authority until publication or closeout. There is no sticky global failover state.
