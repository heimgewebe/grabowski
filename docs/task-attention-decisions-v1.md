...
The default `current` view is the operator-attention surface. It automatically hides non-actionable records so operators see only remaining work:

- valid current-attempt `closed` or `superseded` decisions (`already_decided`);
- verified retry successors (`retry_succeeded` / `superseded_by_verified_retry`);
- `already_satisfied` and plain `superseded` convergence classes;
- explicit failure classes `expected_red` / `expected_red_phase` and `historical_environment_failure` (including environment toolchain/tooling signals on the terminal outcome observation).

`deferred`, unresolved, outcome-unknown, invalid-evidence and other truly actionable records remain visible. The `history` view preserves the complete technical attention projection, including auto-hidden classes, for audit and drill-down. Current-view cursors and history-view cursors are scope-separated so they cannot be replayed across meanings. Pagination is bounded by a fixed raw scan budget; a full attention corpus scan is not required to consume a page.

Each classified record receives exactly one conservative classification:

- `actionable`: technical attention exists and no valid decision or non-actionable failure class closes it;
...
- `expected_red`, `historical_environment_failure`, `retry_succeeded`, `already_satisfied`, `already_decided`, `superseded`: auto-hidden non-actionable classes;
