...
- `grabowski_work_acquire` kann optional genau einen validierten `ExecutionPlan.v1` konsumieren. Bei Plan-Nutzung müssen `source_binding` und der normalisierte repository-relative `write_scope` exakt der Work-Lane-Source beziehungsweise den `write_paths` entsprechen; der vollständige Plan geht in die Lane-Identität ein. Ungültige oder driftende Pläne blockieren vor Lease- oder Worktree-Effekten. Planlose Aufrufe behalten ihre bisherige Lane-Identität unverändert.
