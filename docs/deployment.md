...
Ein Deployment, das Operator und Tunnel neu startet, darf nicht an den Lebenszyklus des aufrufenden MCP-Requests gebunden sein. Dafür existiert das typisierte Werkzeug `grabowski_runtime_deploy_schedule(expected_head, delay_seconds=8, source_repository=None, source_lease_owner_id=None)`.

Ohne explizite Quelle bleibt der kanonische, saubere und mit `origin/main` synchronisierte `main`-Checkout der einzige Ursprung. Für einen absichtlich ausgewählten detached Worktree müssen `source_repository` und `source_lease_owner_id` gemeinsam vorliegen. Der Worktree muss denselben Git-Common-Directory wie der kanonische Checkout besitzen, auf dem exakten `expected_head` detached stehen, denselben `origin/main` zeigen, sauber sein und eine aktuelle exakt ownergebundene Pfad-Lease besitzen. Beliebige Befehle oder unabhängige Clones werden nicht akzeptiert. Source-Kind, Lease-Snapshot und Source-Identity-SHA-256 fließen in Dispatch, Argument-Hash, Scheduler-Receipt und Captain-Nachprüfung ein. Anschließend startet das Werkzeug einen eigenständigen dauerhaften systemd-Job und gibt dessen Unit und Logpfade zurück. Der Runner wartet zunächst, prüft die gebundene Quelle erneut, führt `make validate` und danach `make deploy-apply` aus und verifiziert das Live-Manifest.

Ein vollständiger Self-Deploy endet danach nicht mehr am Runtime-Pointer. Der Runner prüft den Source-Checkout erneut, führt den vorhandenen atomaren Installer für `agent-route`, dessen privaten SHA-256-Pin und `coding_agent_probe_scheduler.py` aus und verlangt anschließend einen unabhängigen `--check` mit `installed=true`. Apply und Check müssen identische Wrapper-, Scheduler- und Runtime-Katalog-Digests melden; automatische Ausführungsautorität bleibt `false`. Danach werden Source und Live-Manifest nochmals geprüft. Erst dann entsteht `coding-agent-sidecars-complete` und das Deployment darf `completed` werden. Ist die Runtime bereits live, aber der Sidecar-Cutover oder sein Readback nicht terminal grün, endet der Job ausdrücklich mit `SidecarInstallOutstanding`; er behauptet keinen vollständigen Self-Deploy und lässt die atomare Installer-Rollbacklogik unverändert wirksam.

Die Verzögerung ist Teil des Antwortvertrags: Der MCP-Request kann abgeschlossen werden, bevor Operator und Tunnel neu starten. Nach der Wiederverbindung liefern `grabowski_job_status` und `grabowski_job_logs` den dauerhaften Nachweis. Jeder neu akzeptierte Ad-hoc-Job erhält einen hashgebundenen Finalisierungsvertrag und erwartet ein create-only `finalization.json`. Der Stop-Finalizer bindet darin Unit, Job-ID, Argument-Hash, Receipt-Pfade, Vertrags-Hash und einen begrenzten Terminalstatus. Runtime-Deploy-Jobs verwenden weiterhin den stärkeren, runner-eigenen Beleg mit erwartetem Git-Head; der allgemeine Finalizer validiert diesen Vertrag, überschreibt dessen Datei aber nicht. Job-Status enthält eine eigene `terminalization_evidence`: akzeptierte Starts werden als `launch_submitted` markiert, sichtbare systemd-Daten bleiben primär, und erst bei gesammelten oder später verschwundenen Units darf ein vollständig gültiger persistierter Beleg den Status liefern. Ungültige, fehlende oder widersprüchliche Daten ergeben `missing_finalization_evidence`. Bei `notify_on_done.requested=true` erzeugt der Stop-Finalizer zusätzlich ein lokales origin-gebundenes Outbox-Receipt. Keiner dieser Belege behauptet externe Pushzustellung oder darf einen Launch-, Hauptprozess- oder Finalizerfehler verdecken.
