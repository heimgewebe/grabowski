...
- **BrowserIntent**: abstrakte Aktionsart aus dem systemeigenen `BROWSER_ACTION_CATALOG` (`read_state`, `navigate`, `scroll_into_view`, `activate`, `submit_maulwurfx_proposal_e2e`). Effektklasse sowie Element- oder Navigationszielbedarf kommen ausschließlich aus diesem Katalog; der Aufrufer liefert keine Effektklasse. `activate` ist kanonisch publiziert; `submit_maulwurfx_proposal_e2e` ist in dieser Slice intern implementiert und testbar, bleibt aber bis zur separaten Runtime-Contract-Promotion ein gestagter Intent.
...
Effektvokabular (`BROWSER_EFFECT_CONTRACTS`): `read`, `local_ui`, `network_navigation`, `bounded_external_submit`, `reversible_external`, `external_mutation`, `high_impact`. Dieser eine serverseitige Katalog ist die maschinenlesbare Quelle für Admission, Mutationsgate und Ambiguitätssemantik. Das Gateway liefert seine begrenzte öffentliche Projektion mit jeder erfolgreichen `observe`-Operation. `read`, `local_ui` und `network_navigation` bleiben kanonisch publiziert. `bounded_external_submit` ist in dieser Slice intern ausschließlich für den serverfixierten maulwurfX-E2E-Prompt implementiert, bleibt aber bis zur separaten Runtime-Contract-Promotion in `BROWSER_EFFECT_CONTRACTS` fail-closed. `navigate` und das linkgebundene `activate` verwenden `network_navigation`; ausschließlich `submit_maulwurfx_proposal_e2e` verwendet `bounded_external_submit`. Beide mutierenden Klassen verlangen das Operator-Mutationsgate. Aktionsarten mit einer nicht publizierten oder nicht implementierten Effektklasse scheitern fail-closed, bevor ein Effekt versucht wird. `reversible_external`, `external_mutation` und `high_impact` bleiben unverändert gesperrt.
...
- Es wird **keine** generische externe Übermittlung implementiert. `reversible_external`, `external_mutation` und `high_impact` bleiben fail-closed. Die gestagte Submit-Ausnahme `submit_maulwurfx_proposal_e2e` ist exakt auf `https://grok.com`, das snapshot-gebundene Textfeld `Ask Grok anything`, ein leeres editierbares Feld, einen serverfixierten Prompt ohne Caller-Text und CDP begrenzt; sie ist in dieser Slice noch nicht im kanonischen Runtime-/Operatorvertrag freigeschaltet. Selbst nach erfolgreicher Enter-Übermittlung bleibt `effect_state=unknown`; erst der separate maulwurfX-Finding-Sink ist autoritativer Readback. Ein Retry wird daraus niemals automatisch autorisiert.
