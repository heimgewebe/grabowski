...
- Es wird **keine** generische externe Übermittlung implementiert. `reversible_external`, `external_mutation` und `high_impact` bleiben fail-closed. Die einzige Submit-Ausnahme ist `submit_maulwurfx_proposal_e2e`: exakt `https://grok.com`, exakt das snapshot-gebundene Textfeld `Ask Grok anything`, nur ein leeres editierbares Feld, serverfixierter Prompt ohne Caller-Text und CDP-only. Selbst nach erfolgreicher Enter-Übermittlung bleibt `effect_state=unknown`; erst der separate maulwurfX-Finding-Sink ist autoritativer Readback. Ein Retry wird daraus niemals automatisch autorisiert.
