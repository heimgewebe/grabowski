...
Dieser Pfad bleibt im versionierten Runtime-Entrypoint-Contract als allgemeiner Default verankert. Schema 2 beschreibt weiterhin ein partiell abdeckendes `semantic_gateway`: `grabowski_browser_worker_semantic` unterstützt `observe`/`act` für `read_state`, `navigate` und `scroll_into_view`. Die Abdeckung bleibt partiell, weil Console, Network, Screenshot und weitere Browserfunktionen nicht Teil dieses Vertrags sind. Navigation ist öffentlich backend-neutral; der aktuelle Chrome-Adapter setzt sie intern ausschließlich mit CDP `Page.navigate` um. Ein neu generierter und anschließend deployter `grabowski_context` kann diesen Teilvertrag später tragen; der Repository-Stand allein behauptet keine Runtime-Aktualisierung.
...
Für `navigate` wird derselbe bestätigte Pre-Snapshot verwendet. `ChromeCDPAdapter` sendet danach intern genau `Page.navigate`. Dessen erfolgreicher Command-ACK enthält absichtlich keine Observation und kann niemals allein `ok=true` begründen. Erst ein separater frischer Adapter-Read erzeugt eine neue `BrowserObservation` und daraus den tatsächlich beobachteten Post-Navigate-Snapshot; genau dieser Snapshot wird als `post_action_snapshot_id`, `observation.snapshot_id` und `authoritative_post_action_observation` zurückgegeben. Ein CDP-`errorText` ergibt `navigation_failed` mit `effect_state=unknown`; soweit möglich wird auch dann eine frische Observation mitgeführt. Geht der ACK- oder Transportausgang verloren, bleibt der Effekt unbekannt. Scheitert der zwingende Post-ACK-Readback, endet die Aktion fail-closed mit `observation_failed`, `effect_state=unknown`, ohne Post-Snapshot und ohne Retry-Autorität. Auch ein erfolgreicher Readback nach einem unbekannten Effekt autorisiert keinen automatischen Retry.
...
- `navigate` behauptet weder vollständiges Laden noch Erreichen eines bestimmten Seitentitels oder DOM-Zustands. Ein erfolgreicher Outcome belegt den akzeptierten Command und den konkret beobachteten frischen Post-Command-Snapshot; dessen `ready_state` darf weiterhin einen laufenden Ladevorgang zeigen. `scroll_into_view` wirkt nur lokal auf ein zuvor beobachtetes, opak gebundenes Element.
