...
2. the operator admission gate stops new MCP tool calls before tool execution while already admitted effect-capable or unclassified calls finish;
...
4. Require two operator samples with the exact token, head and source digest and zero drain-blocking tool calls. On predecessor runtimes that do not expose the additive field, fall back to `active_tool_calls == 0`.
