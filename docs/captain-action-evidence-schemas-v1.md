...
- `pr-merge` binds `repo`, `pr`, `base`, `expected_head`, `expected_base_sha`, `diff_sha256`, `status_projection`, a passing `grabowski_self_review_audit` as `review_evidence`, one conditional Codex release alternative, `ci_evidence` and `human_authorization`. When `review_evidence.independent_review_required=true` (mandatory for `high_critical`), Captain requires at least one exact-revision decision-bound PASS from an admissible `independent-*` reviewer slot and re-reconciles it under the atomic merge guard. This is provider-neutral and does not grant external transport authority. Separately, when `codex_review_required=true`, the named `codex_review_release` group requires exactly one of `codex_review_evidence` and `codex_review_exception`. `codex_review_evidence` binds the exact GitHub request, one trusted current-head completion (pull-request review, deterministic clean-result issue comment, or request reaction), the diff and the terminal Codex thread set. `codex_review_exception` is mutually exclusive and declares every field required by the runtime validator: schema and kind, repo, PR, base, head, diff, generation and expiry timestamps, approver, reason and `exception_sha256`. It is valid for at most two hours and bypasses only the Codex settlement gate. The self-review audit remains independently required and records risk-scaled review depth, terminal triage and exact repo/PR/head/diff binding without posting its prose to the PR. Status projection evidence requires one replay field (`receipt_ref`, `run_id` or `nonce`) and the top-level `status_projection_sha256` parameter covering the projection object. Manual authorization requires `authorized_by` plus either `statement` or `reference`.
