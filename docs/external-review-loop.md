...
Grabowski self-review remains the universal required review control. For `high_critical` pull requests, Captain additionally requires a provider-neutral exact-revision decision-bound independent review. That requirement does not itself authorize external transport: a local or on-prem reviewer may satisfy it when registered through the decision-bound review path. GitHub Codex settlement remains separate and is mandatory only when `codex_review_required=true`, in which case one current-head settlement or one explicit short-lived exception is required. Other external LLM reviews, GitHub approvals, PR comments, and provider-specific packet reviews remain optional diagnostics. No review prose is treated as authoritative by itself.

External review is optional to request, not optional to settle after a concrete finding exists. The atomic Captain merge guard treats every unresolved inline review thread whose root comment was authored by a trusted supported review actor, plus every outstanding structured `CHANGES_REQUESTED` review from such an actor, as merge debt. Current trusted sources are GitHub Codex and the recognized Claude review bots; both REST `[bot]` logins and GraphQL bare bot logins are accepted. That debt survives head changes: an unresolved finding on an older commit still blocks until the thread is resolved. A resolved trusted thread is terminal for this transport-level gate; an untrusted review bot does not acquire merge authority merely by commenting.

The universal finding-debt check is independent of whether the provider was required to run. It executes from live GitHub review-thread state both before merge-resource acquisition and immediately before merge dispatch. Missing or truncated thread visibility fails closed. The existing current-head Codex settlement remains a separate provider-specific requirement only for pull requests where `codex_review_required=true`; it is neither implied by nor a substitute for the generic independent-review floor.

The required controls remain independent:

1. current base, head and `gh pr diff` binding;
2. risk-scaled Grabowski self-review;
3. terminal finding triage;
4. green CI;
5. mergeability and target identity;
6. exact-revision decision-bound independent review for high-critical PRs, plus current-head Codex settlement only when explicitly required;
...
For `high_critical`, the self-review audit carries `independent_review_required=true`. Captain preflight and the atomic merge guard reconcile the bounded durable-job registry for the exact repo/PR/head/base. At least one PASS in an `independent-*` slot must additionally be backed by server-derived origin provenance showing that the job launched the canonical `grabowski_agent_role --role review` wrapper, that its inner command matches exactly one enabled catalog `review_only` route with `independent-review` capability, and that the create-only role receipt proves an unchanged frozen worktree in the read-only role sandbox. A caller-selected slot name or generic result marker alone contributes zero independent PASSes. For pre-deployment bootstrap, a job created by an older trusted Grabowski runtime may omit the explicit provenance object only when its stored argv is byte-for-byte hash-equivalent to the origin-bound launch argv; reconciliation then derives the same role/route provenance from that exact argv. Any redaction, hash mismatch or route ambiguity disables this compatibility path. Preflight may mark a non-request diff digest as `diff_identity_deferred` rather than claiming equivalence; the atomic merge guard alone proves accepted canonical, canonical-v1 or current-provider raw aliases from live diff bytes. The `independent-*` slot remains a grouping convention and does not by itself prove use of a different provider family. Captain scans the bounded durable-job registry for every matching PR/head immediately before final dispatch. Non-terminal jobs, invalid origin/finalization data, successful jobs without a valid result marker, base/diff drift and any material REJECT block. A terminal infrastructure failure without a semantic marker is retained as an infrastructure attempt and may be followed by a successful replacement in the same slot; it never counts as PASS itself. A later PASS does not erase a prior material REJECT. The same per-PR/head filesystem lock serializes decision-bound job registration with the final reconciliation and merge dispatch, so a cooperating reviewer cannot be registered in the gap after reconciliation.

This contract governs only reviews launched as decision-bound through the typed Grabowski job surface. Generic decision-bound jobs may still create blocking finding debt, but only the server-derived read-only reviewer-role provenance described above can satisfy the independent-review floor. It does not turn arbitrary external LLM output into merge authority, prove review quality, prove a different provider family, prevent a non-cooperating external GitHub actor from merging, or claim complete detection of deliberately indirect same-UID execution.
...
- `independent_review_required: true` for `high_critical` and `false` otherwise under the current policy.
...
When `codex_review_required=true`, `codex_review_evidence` is a separate Captain gate. `tools/codex_review_settlement.py` creates an idempotent `@codex review` request whose hidden marker binds repository, PR, current base, current head and current diff SHA-256. If identical trusted markers are duplicated, the earliest canonical request remains authoritative and fixes the review and finding cutoff. A trusted thumbs-up may appear on any identical canonical marker; the completion evidence separately binds the exact reacted comment, and Captain revalidates that marker and reaction while retaining the earliest cutoff. Settlement requires one of three trusted completion forms after the canonical request: a Codex pull-request review bound to the full head, a deterministic clean-result issue comment whose reviewed-commit token is a 10–40 character prefix of that head, or a thumbs-up reaction on a bound identical request marker. Every bounded review thread containing a trusted Codex comment bound to the current head must be resolved, including threads whose root belongs to an older commit or another actor and threads created before a duplicate marker. This current-head settlement set intentionally matches the atomic merge revalidator. Global historical or unsolicited finding debt remains narrower: only a thread whose root comment is from a trusted review actor creates cross-head debt, so a later trusted reply cannot promote someone else's root into merge authority. A new base, head or diff invalidates the request and all prior settlement evidence.
...
Captain validates the evidence structurally and binds its digest into the short-lived execution intent. A `COMMENTED` review never clears an outstanding current-head `CHANGES_REQUESTED` or `PENDING` review. GitHub represents an unsubmitted `PENDING` review with no submission timestamp; it still blocks conservatively regardless of when the canonical request was created. A later trusted `APPROVED` review may supersede `CHANGES_REQUESTED` only when that approval is bound to the current pull-request head; submission order alone never lets a delayed approval for an older commit clear a newer blocker. Such a pre-request current-head approval does not itself complete the new request; an accepted review, clean result or reaction after the request is still required. A `PENDING` review must disappear from the live set through submission or deletion before a required Codex settlement can pass. When Codex is optional, a current-head `PENDING` draft with no concrete unresolved finding debt remains the non-blocking `optional_review_pending` diagnostic; any actual unresolved thread or unsuperseded structured `CHANGES_REQUESTED` review still blocks. Unsuperseded changes-requested debt is evaluated before the pending-only exemption, while a later valid approval may clear that structured blocker before a remaining draft is treated as optional pending. The atomic merge guard then reads the exact request, the bounded canonical issue-comment set, the bounded current-head review set, the selected review, clean-result comment or reaction completion, and the current thread set from GitHub twice: once before acquiring merge resources and once immediately before `gh pr merge`. Any missing object, body or actor drift, non-earliest request, stale head, changed diff, outstanding blocking review, changed thread set, unresolved thread, truncated result window, failed GitHub read or evidence mismatch blocks fail-closed. Request, issue-comment, review and reaction revalidation uses 100 items per page with pagination enabled and accepts exactly one page; any second page is an explicit boundedness failure. A clean-result comment is accepted only from a trusted Codex actor, after the request, when the entire normalized body matches the deterministic `Didn't find any major issues` prefix, one bounded single-line connector closing of at most 80 characters ending in punctuation or a bounded emoji shortcode, the reviewed-commit prefix matching the current head, and the known GitHub-Codex footer through end of body. Its exact body digest remains bound into the evidence. Multiline or unbounded success prose is rejected; the closing text itself has no authority.

The workflow requests one idempotent Codex review for every current PR head so a later explicit `codex_review_required=true` policy can already have a bound request even when the earlier self-review evidence did not require Codex. Captain does not make Codex settlement mandatory merely because a PR is high-critical; missing Codex evidence blocks only when the provider-specific Codex policy is explicitly required. Issue comments authored by `github-actions[bot]` do not start another settlement job, because the workflow's own request comment would otherwise recursively trigger itself. A comment from `chatgpt-codex-connector[bot]` may start settlement re-evaluation, but the request step is skipped so the completion cannot create another request loop.

The workflow status `Codex review settled` is a diagnostic projection. GitHub Actions supports review, review-comment and issue-comment triggers, but neither review-thread resolution nor reaction changes as direct workflow triggers. A separate default-branch dispatcher refreshes the projection every 15 minutes and on manual dispatch by invoking the trusted settlement workflow for at most 100 open pull requests. It lists 101 candidates and fails closed instead of silently truncating a larger inventory. The dispatcher has only `actions: write`, `contents: read` and `pull-requests: read`, checks out no code and never evaluates a pull-request head. During the one-time bootstrap before the evaluator itself reaches the default branch, the settlement workflow reports `trusted_evaluator_missing_on_default_branch` and blocks explicitly rather than executing the pull-request copy of the evaluator. Empty, non-object or structurally malformed evaluator output is replaced with a bounded `trusted_evaluator_output_invalid` block result so the status publication still runs and cannot leave the head silently pending. The introducing pull request therefore needs an authorized out-of-band current-head settlement; after merge, ordinary refreshes use only the trusted default-branch evaluator. Reaction and thread-resolution projections may be delayed by one refresh interval, and scheduled workflows can be delayed by the platform. Existing unresolved Codex finding threads and outstanding trusted Codex `CHANGES_REQUESTED` reviews publish failure rather than advisory success even when no current-head request marker exists or requesting Codex was optional. Structured review debt is scanned independently of requests and uses the same supersession rule as the atomic merge guard: only a later trusted approval bound to the current PR head clears the latest blocker for that actor. Provider absence or a still-pending optional request without findings remains non-blocking. Captain does not trust this projection and always performs the authoritative live finding-debt and current-head settlement revalidation. The diagnostic status must not be configured as a required repository check. The local PR review gate likewise excludes `Codex review settled` from blocking check counts. It tolerates GitHub `mergeStateStatus=UNSTABLE` only when every non-green check is one of the explicitly non-blocking diagnostic review projections, all required checks are green and current, no other check is blocking, and GitHub still reports the pull request as mergeable. Any other `UNSTABLE`, `BLOCKED` or unknown state remains fail-closed.

A `grabowski_codex_review_exception` is allowed only as an explicit, repo/PR/base/head/diff-bound record with approver, reason and at most two hours of validity. The published Captain evidence schema exposes it together with `codex_review_evidence` as the mutually exclusive `codex_review_release` alternative group and lists every runtime-required exception field. When Codex settlement is required, exactly one alternative must be present. The exception bypasses only the Codex settlement gate; it does not weaken self-review, CI, full-diff binding, mergeability, authorization or live target checks.
...
External review tools other than the conditional GitHub Codex settlement remain available for unusual uncertainty, incident analysis, or a deliberate second opinion. `tools/external_review_plain.py` supplies a provider-neutral, single-turn Gemini/Grok path that receives only the hash-bound packet prompt and diff, runs from a private empty temporary working directory, passes a fixed non-secret environment allowlist with null stdin, bounds the provider process group and output while reading, verifies owner-controlled executable identity and workspace immutability, and writes create-only transmitted-prompt, raw-response, and strict `external_review` evidence artifacts. A random nonce binds the untrusted diff fences; the central gate reconstructs both packet prompt and full transmitted prompt independently from the current PR state. Grok reads a short-lived `0600` prompt file through the canonical native binary, while Gemini currently exposes the prompt through an explicitly recorded argv transport. The former Ox Alpha plain-review provider is retired: new `--provider ox-alpha` execution is rejected before provider launch. Historical Ox Alpha evidence remains readable for compatibility, but neither the paid `z-ai/glm-5.3-flash` route nor a separately published `z-ai/glm-5.3-flash:free` route is an implicit replacement; any future free integration requires fresh exact routing and cost evidence. Provider findings remain advisory and untriaged until Grabowski checks them; the evidence explicitly has no review-gate authority, and cost/quota eligibility must come from the separate subscription-aware route preflight. See `docs/plain-llm-external-review.md`. Evidence may be supplied with `--external-review-evidence`; invalid evidence produces warnings, not a merge block. Legacy Claude packet requirements, policy waivers, and `self_review_required=false` are deprecated. No external review satisfies or shortens the required self-review loop.

Agent-competition packets are a different, deliberately bounded artifact: they contain only the explicitly selected `context_paths` under the packet size limits, not the manifest/hash-bound complete PR diff. Their outputs must never be described or counted as complete PR reviews. A `codex-sol-high` route remains an OpenAI advisory contrast or current-head Codex settlement where that control applies; it is not an independent-provider reviewer and cannot satisfy an independent-provider slot.
