...
External review tools other than the conditional GitHub Codex settlement remain available for unusual uncertainty, incident analysis, or a deliberate second opinion. `tools/external_review_plain.py` supplies a provider-neutral, single-turn Gemini/Grok/Ox Alpha path that receives only the hash-bound packet prompt and diff, runs from a private empty temporary working directory, passes a fixed non-secret environment allowlist with null stdin, bounds the provider process group and output while reading, verifies owner-controlled executable identity and workspace immutability, and writes create-only transmitted-prompt, raw-response, and strict `external_review` evidence artifacts. A random nonce binds the untrusted diff fences; the central gate reconstructs both packet prompt and full transmitted prompt independently from the current PR state. Grok reads a short-lived `0600` prompt file through the canonical native binary, while Gemini currently exposes the prompt through an explicitly recorded argv transport. Ox Alpha requires an explicit `public-context`, `synthetic-context`, or `non-sensitive-context` attestation and receives the full prompt through a short-lived `0600` file using exact-model OpenCode `--pure` mode plus an isolated temporary HOME/XDG runtime and a generated `grabowski-reviewer` agent with every OpenCode permission denied; only a private copy of account auth enters that runtime, and there is no paid-model fallback. Ox availability and zero-cost status remain separate fresh-preflight facts rather than adapter claims. Provider findings remain advisory and untriaged until Grabowski checks them; the evidence explicitly has no review-gate authority, and cost/quota eligibility must come from the separate subscription-aware route preflight. See `docs/plain-llm-external-review.md`. Evidence may be supplied with `--external-review-evidence`; invalid evidence produces warnings, not a merge block. Legacy Claude packet requirements, policy waivers, and `self_review_required=false` are deprecated. No external review satisfies or shortens the required self-review loop.
