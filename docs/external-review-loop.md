...
The settlement workflow evaluates the current head but does not create a Codex request by itself. An explicit request uses the `request` command or an equivalent trusted connected-user action; the canonical hidden marker binds repository, PR, head and diff. When external review is required, a missing marker remains pending. When external review is optional, not-requested, pending, provider-unavailable and advisory-finding states remain distinct non-blocking diagnostics. Issue comments authored by `github-actions[bot]` do not start another settlement job, and a comment from `chatgpt-codex-connector[bot]` may start settlement re-evaluation without granting completion or request authority.
