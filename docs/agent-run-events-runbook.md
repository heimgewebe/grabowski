...
The default repository pointer is Chronik's deployed `current` symlink. The default Python interpreter is Grabowski's currently running interpreter (`sys.executable`). Grabowski vendors Chronik's producer-owned `tools/coding_memory.runtime.v1.json` with an exact producer commit, Git-blob SHA and content SHA-256 binding. Every distribution declared by that contract (`filelock`, `jsonschema`, `pydantic`, `pydantic-settings`, and `PyYAML`) must exist in Grabowski's hashed runtime lock at a version satisfying the producer constraint. Packages already provided transitively remain transitive instead of being copied into a second set of direct requirements; if a later lock regeneration removes or downgrades one, the cross-repository gate fails before deployment. Grabowski's existing runtime-lock contract remains authoritative for its own direct requirement and lock consistency. `make validate` and all Grabowski deployment entry points fail closed on producer-contract or runtime-lock incompatibility before deployment. `GRABOWSKI_CHRONIK_CODING_MEMORY_PYTHON` remains an optional explicit override for controlled installations and is accepted only when it names an absolute executable file. Grabowski resolves the `current` repository pointer before execution and accepts it only when it names one direct 40-hex release directory below the sibling `releases/` directory; arbitrary repository symlinks remain fail-closed. The cross-repository binding does not assert that the currently deployed Chronik release equals the bound producer commit or that compatibility alone proves runtime success; missing or stale releases, missing `tools/coding_memory.py`, invalid CLI output, timeout, or non-zero exit remain bounded fail-soft unavailable results and do not affect normal task creation, execution, reconciliation, or lifecycle event writing.
