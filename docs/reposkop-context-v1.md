...
Receipt root, pending-file, final-file and lock paths pass Grabowski's central write-root policy before any state is created. The resolver uses an explicit effect-free mode for still-missing parent directories; after root creation, every path is resolved and checked again against the real filesystem identity before publication begins. The receipt directory is owner-held mode `0700`; lock, pending and final files use mode `0600` and reject symlinks, foreign ownership, unexpected link counts, size drift, inode drift and any byte mismatch.

Publication is serialized by a per-key advisory file lock. A normal first publication appends the audit contract `audit-before-create-exact-bytes-v1` **before** any receipt bytes are published and binds the exact expected receipt SHA-256 and byte count. The receipt is then written to a deterministic pending file, fsynced, linked create-only to its final name and reduced to one final link. A crash after the audit append but before final publication is recoverable from the same exact audit binding. A partial pending file is replaced only when its bytes are an exact proper prefix of the expected deterministic receipt; wrong bytes, a complete mismatch or an unexpected link fail closed. A crash after linking but before pending-file removal is recovered only when both paths resolve to the same exactly bound inode.
