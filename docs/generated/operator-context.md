...
| `ipad_bluetooth_read` | device-worker | no | high | Read bounded raw values from exact requested readable GATT characteristics on one exact BLE identifier without writes, subscriptions, pairing requests or auth bypass. |
