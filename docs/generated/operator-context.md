...
| `repoground_agent_handoff` | knowledge | yes | low | Route one bounded caller-selected agent handoff explicitly between native live reads and RepoGround context. |
...
| `grabowski_git_branch_preimage` | version-control | yes | low | Read one exact local branch/index preimage for a later CAS-bound mutation. |
...
| `grabowski_runtime_deploy_schedule` | deployment | unknown | high | Schedule a validated delayed deployment after fresh public GitHub-main and root-authority verification. |
...
| `ipad_bluetooth_read` | device-worker | no | high | Read bounded raw values from exact requested readable GATT characteristics on one exact BLE identifier without writes, subscriptions, explicit pairing APIs or auth bypass; CoreBluetooth system-pairing risk requires affirmative acknowledgement. |
...
| `grabowski_browser_worker_semantic` | browser-worker | no | high | Observe or act through snapshot-bound semantic browser handles, including explicitly audited network navigation, with bounded Accessibility labels and no raw browser locators. |
