...
The tool reads recent friction events, `grabowski_status`, fixed user-service status for `grabowski-operator.service` and `tunnel-client-grabowski.service`, and JSON journal records from bounded recent samples. A generic structured `error` field is not transport proof by itself: the exact tunnel-client startup warning `harpoon host auto-registration failed` is excluded only when it is the known OAuth loopback PRMD auto-registration case whose exact bare local HTTP base URL (without a path) is rejected because Harpoon requires HTTPS. Unobserved path or trailing-slash variants remain fail-closed until separately evidenced. Nearby Harpoon warnings or the same message outside that exact source/role/loopback contract remain fail-closed. The exact tunnel messages `MCP connection TTL reached; stopping response forwarding` and `response already fulfilled or unknown request` are first-class response-lifecycle failures even though the former is logged at INFO level. Their `cmd_request_id` and `rpc_request_id` are correlated through a SHA-256 identity that is never returned in raw form:
...
The journal capture is internally bounded to 500 records and 1 MiB so verbose JSON cannot silently hide the newest lifecycle evidence behind the former 128-KiB ceiling; the public receipt remains compact and sample-bounded. Each unit exposes nested `response_lifecycle.schema_version: 1`, classification counts, bounded redacted samples and explicit non-claims at `journal_transport_probes[<unit>].response_lifecycle`; the top-level aggregate deliberately does not duplicate those unit samples. `transport_health_state` is `healthy`, `indeterminate`, `degraded` or `unavailable_suspected`; ordinary successful forwarding after a lifecycle failure proves continued activity but leaves the window degraded. The top-level receipt projects the strictest unit state and `transport_degraded` without claiming a current outage or vendor repair.
...
`pre_runtime_http_404_recovery` is guidance, not an enforced state machine or a detected condition. The receipt exposes `authority: guidance_only`, `machine_enforced: false`, `applicability: caller_must_establish` and a stable `recommended_action` enum. Grabowski cannot prove from its own logs that a platform-side `404` occurred before the request reached Grabowski or that no mutation may have happened; it therefore does not infer this from operation names, surfaces or timestamps.
...
```json
{
  ...
  "friction_log": {
    "connector_transport_diagnostics": {
      ...
      "historical_http_status_counts_semantics": "bounded supplied friction-event window; each distinct status counts at most once per event; not a time series or lifetime aggregate",
...
For genuine errors, `window_state` distinguishes `no_errors`, `errors_without_later_activity` and `errors_followed_by_activity`; an output-truncated journal is reported as `indeterminate_truncated`; failed, timed-out or partially unparsable probes are `indeterminate_incomplete`. Response-lifecycle evidence also has a source-filtered 15-minute lookback, capped at 500 matching records, for the exact TTL and already-fulfilled/unknown messages. This prevents high normal request volume from evicting a recent lifecycle signal from the ordinary tail. When that lookback finds positive lifecycle evidence while the ordinary tail is clean, the public state is `lifecycle_errors_outside_tail_window` and health is `degraded`. When the lookback itself is incomplete and there is no stronger positive evidence, an otherwise healthy result becomes `indeterminate`; it must not remain falsely green.

Neither indeterminate state can establish absence of errors or later recovery. `post_error_activity_counts` shows bounded forwarding or control-plane acknowledgements after the last error. This is recovery evidence, not proof that the earlier error was harmless or that the transport is currently reliable. The compatibility field `live_transport_errors_observed` means that an error was positively observed either in the bounded general tail or in the recent source-filtered lifecycle lookback; it does not mean that the transport is currently unavailable. `response_lifecycle_lookback_probes` preserves the completeness of each filtered probe, while `response_lifecycle_lookback` provides the bounded aggregate classifications. HTTP codes still come only from explicit structured fields or explicit status syntax. Samples contain only bounded timestamp, invocation, level, component, status and error-domain fields; raw log messages are not returned. This remains diagnostic evidence only and does not authorize retry, mutation, merge, deploy or policy exceptions.
