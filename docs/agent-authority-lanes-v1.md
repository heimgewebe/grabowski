...
A branch/path lease proves the logical owner and resource scope, but it does not by itself distinguish two concurrent controller attempts that reuse that same owner. Local branch or index mutation through `grabowski_git` therefore requires a `branch_attempt` binding containing the owner, operation id, attempt id and attached branch. The normal strict form also carries the exact observed Git preimage. For the common finalization path only, that long-lived hash may be omitted for `git add -- <literal-tracked-file>...` (including tracked deletions) and ordinary `git commit` without implicit staging and with a server-verifiable noninteractive message path (`-m`/`--message` or `--no-edit` as appropriate). New or otherwise untracked files stay outside the JIT path because the branch preimage intentionally covers index-declared worktree paths only. Grabowski captures the full preimage immediately before acquiring the branch-attempt lease and uses that server-owned digest as the expected value. JIT commit additionally pins `core.hooksPath=/dev/null` so repository hooks cannot mutate the index after the final preimage check; editor-opening commit forms remain on strict caller CAS. Existing directories, untracked or non-literal/broad staging shapes, commit forms that can stage content, and every other mutator still require the caller-bound exact preimage. The preimage itself continues to bind HEAD, the staged index, tracked worktree bytes and in-progress merge/cherry-pick/revert/rebase refs.
...
The existing branch resource remains the only serialization point. When the same owner already holds that branch through a Work Lane or another live lease, the attempt CAS overlays only an ephemeral attempt marker: it preserves the lease purpose, acquisition time, expiry and prior metadata. A different attempt from the same owner fails before the Git effect with `reconcile_required`. In strict mode, stale caller state still reconciles before lease acquisition. In server-JIT mode there is intentionally no long interval in which that preimage can age: Grabowski captures it immediately before the lease, then performs the same second exact preimage read after lease acquisition. Drift in that narrow preimage-to-lease interval still reconciles before effect. Disjoint branch resources remain independent, so this does not create a repository-wide lock.
