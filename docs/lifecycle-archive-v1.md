...
Nach Ablauf serialisiert eine exakte `component:grabowski-task-output-cleanup:<task-id>:a<attempt>`-Lease den Effekt. Jeder Aufruf verwendet dabei einen eigenen Owner, sodass parallele Aufrufe desselben Taskversuchs tatsächlich konkurrieren und einander weder als Lease-Verlängerung noch als vorzeitige Freigabe behandeln. Ein create-only Intent bindet Taskversuch, Lifecycle-Receipt, Retentionsgrenze, Segment-/Manifest-/Plan-/Record-Digests sowie die stabilen per-Task-Projektionsfelder `segment_id` und `switch_sha256`; der global veränderliche Gesamt-Projection-Digest wird nicht persistiert. Ohne Intent wird die vorgeschlagene Grenze mindestens auf die feste 24-Stunden-Untergrenze angehoben. Existiert bereits ein gültiges Intent, bleibt dessen per-Versuch-Grenze kanonisch, sofern sie diese feste Untergrenze einhält; ein später strengerer Aufrufsvorschlag schreibt das create-only Intent nicht rückwirkend um. Vor jedem Effekt wird die aktuelle Projection-Mitgliedschaft desselben Taskrecords separat revalidiert. Dadurch bleiben Intent und Completion auch nach der Archivierung anderer Tasks wiederholbar. Zusätzlich bindet das Intent SHA-256 und Bytegröße beider Streams. Erst danach darf der hashgebundene lokale oder Fleet-Adapter wirken. Der Adapter akzeptiert nur den kanonischen Outputpfad, `stdout.log` und `stderr.log`, private Eigentümer-/Modus-/Linkzustände und höchstens 8 MiB je Stream. Unerwartete Verzeichniseinträge blockieren.
Resume-Versuche werden in Batches von höchstens drei Versuchen traversiert. Ein `deferred`-Versuch verschiebt den Cursor absichtlich weiter, damit eine strengere per-Versuch-Grenze bereits bereite spätere Versuche nicht verhungern lässt; nach dem Wrap wird er erneut besucht. Jeder idempotente Closeout-Replay stößt diesen begrenzten Cleanup erneut an. Das ist der Eventual-Cleanup-Mechanismus: vor Ablauf bleibt er wirkungsfrei, nach Completion reduziert er sich auf validierte Receipt- und Abwesenheitsprüfungen.
...
Der generische Produktions-Fleet-Pfad erhält keine Python-Freigabe. Reader und Cleanup verwenden stattdessen feste SHA-256-gebundene Programme mit serverseitig validierter Task-ID-, Versuch-, Pfad-, Stream-, Digest- und Größenform. Das begründet weder Same-UID-Authentizität noch eine Löschbefugnis für andere Task-, Workspace- oder Checkoutdaten. Der aktuelle Fleet-Vertrag setzt für lokale und entfernte Taskhosts denselben absoluten Operator-HOME-Pfad voraus. Die Pfadprüfung wird nicht auf beliebige entfernte Homes aufgeweitet, weil das die Löschziel-Autorität verbreitern würde; abweichende Remote-Homes benötigen einen separat persistierten, hostgebundenen Output-Root-Vertrag.

Der periodische globale Task-Refresh führt einen separaten bounded Cleanup-Replay aus. Pro Lauf werden höchstens zehn archivierte Taskbindungen geprüft. Taskstore-Lesen, Statusmutation und Cursor-Vorbereitung bleiben unter dem globalen Task-Mutationslock; dieser ist vor jeder lokalen oder entfernten Output-Inspektion beziehungsweise Löschung freigegeben. Eine eigene 15-minütige `component:grabowski-task-output-cleanup-reconcile`-Lease verhindert konkurrierende Cursor-/Cleanup-Läufe, ohne andere Taskmutationen zu blockieren. Der Cursor persistiert seine stabile Task-ID-Position über Änderungen des Gesamt-Projection-Digests hinweg. Der gespeicherte Digest bleibt Integritätsevidenz, invalidiert die Position bei additiven Projection-Änderungen aber nicht; jede ausgewählte Taskbindung wird vor dem Effekt separat gegen die aktuelle Projection revalidiert. Dadurch kann fortlaufende Archivierung spätere Task-IDs nicht durch wiederholte Neustarts verhungern lassen. Ein einzelner fehlerhafter Task wird als begrenzter `error`-Eintrag ausgewiesen, der Cursor schreitet jedoch fort und der Fall wird im nächsten Zyklus erneut besucht. Ein globaler Projection-, Cursor- oder Cleanup-Lease-Fehler degradiert ausschließlich den Cleanup-Teil des Refresh-Ergebnisses und verhindert nicht die Taskzustandsaktualisierung.
