...
SSH-Hosts müssen `/usr/bin/python3` mit `O_NOFOLLOW` bereitstellen. Reader und Cleanup laufen jedoch nicht über die generische Fleet-Executable-Allowlist: Grabowski akzeptiert ausschließlich die fest eingebetteten, SHA-256-gebundenen Python-Programme sowie kanonische Task-ID-, Versuch-, Stream-, Pfad-, Hash-, Byte- und Zeilenargumente. Dadurch bleibt eine enge Produktions-Allowlist wie auf `wg-prod-1` unverändert; beliebige Python-argv bleiben über `grabowski_fleet_run` verboten. Jede Code- oder Argumentdrift wird vor SSH-Ausführung abgelehnt.
