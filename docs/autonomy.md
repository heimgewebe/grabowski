...
Job-Metadaten sind dauerhafte Evidence, keine Erfolgsmeldung. Ein Job-Record enthält `job_id`, `owner`, `scope`, `started_at`, `expected_receipt`, `final_status`, `terminalization_evidence` und einen hashgebundenen Finalisierungsvertrag. Vor erfolgreichem `systemd-run` gilt nur `launch_prepared`; nach angenommenem Launch gilt `launch_submitted`; Launch-Fehler bleiben als `launch_failed` sichtbar. Jeder neu akzeptierte Ad-hoc-Job erwartet ein create-only `finalization.json`, das Unit, Job-ID, Argument-Hash, Receipt-Pfade, Vertrags-Hash und den beobachteten Terminalstatus bindet. Runtime-Deploy-Jobs behalten zusätzlich die stärkere Bindung an den erwarteten Git-Head und ihren runner-eigenen Beleg. Solange systemd die Unit noch verlässlich sieht, bleibt `systemctl show` die Primärquelle. Erst bei gesammelten oder später nicht mehr sichtbaren Units darf ein vollständig validierter persistierter Beleg den Terminalstatus wiederherstellen; fehlende, widersprüchliche oder ungültige Belege ergeben `missing_finalization_evidence`. Bei `notify_on_done.requested=true` erzeugt der Stop-Finalizer daneben ein lokales Outbox-Receipt. Origin-Hash und Startwerkzeug werden gegen die beim Unit-Start gesetzte Precondition geprüft. Der Status trennt Hauptprozess und Postflight: Ein fehlgeschlagener `ExecStopPost` überschreibt einen erfolgreichen `ExecMainStatus` nicht, bleibt aber in `postflight_evidence` sichtbar. Daraus folgen weder externe Zustellung noch vollständige Isolation gegen Same-UID-Code.


### Session capability profiles

Access profiles attenuate a session beyond coarse capabilities. A profile declares `read_roots`, `write_roots`, `allowed_grips`, `forbidden_hosts` and `max_risk_level`. `grip_run` checks the active session profile before dispatch: low-risk read-only grips can be allowed while medium mutating grips or high-risk Captain grips remain blocked. High-risk grip execution additionally requires `session_escalation` with target, reason, expiry and recovery or irreversibility metadata unless the active profile itself explicitly declares `trusted_owner: true`; in that profile the server-side trusted-owner binding supplies the session-level high-risk authority. A top-level inherited `trusted_owner` value is not sufficient for this waiver, and `allowed_grips`, `max_risk_level` plus every grip-specific Captain/action gate remain unchanged. Operator argv validation rejects configured `forbidden_hosts` in command targets before execution. These checks do not establish host reachability, action safety or successful execution; they are preconditions.
