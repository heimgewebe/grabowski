...
Job-Metadaten sind dauerhafte Evidence, keine Erfolgsmeldung. Ein Job-Record enthält `job_id`, `owner`, `scope`, `started_at`, `expected_receipt`, `final_status` und `terminalization_evidence`. Vor erfolgreichem `systemd-run` gilt nur `launch_prepared`; nach angenommenem Launch gilt `launch_submitted`; Launch-Fehler bleiben als `launch_failed` sichtbar. `expected_receipt` beschreibt Nachweispfade und beweist weder Receipt-Existenz noch Job-Erfolg. Bei `notify_on_done.requested=true` erzeugt der Stop-Finalizer ein lokales Outbox-Receipt. Origin-Hash und Startwerkzeug werden gegen die beim Unit-Start gesetzte Precondition geprüft; fehlende oder ungültige Receipts bleiben sichtbar. Der Status trennt den Hauptprozess vom Postflight: Ein fehlgeschlagener `ExecStopPost` überschreibt einen erfolgreichen `ExecMainStatus` nicht, bleibt aber in `postflight_evidence` sichtbar. Daraus folgen weder externe Zustellung noch vollständige Isolation gegen Same-UID-Code.
