...
The caller must bind the current target from fresh live authority first. The plan now makes historical selector eligibility machine-readable instead of merely advising the caller to use whatever filters are available. For `pr`, the fabric derives the exact historical selector `{repo, pr_number}` from the live-bound repository and pull-request number; the current `head_sha` remains live revision evidence and is not used to erase older history for the same PR. An exact miss is discarded without a coarse fallback.

For `bureau`, the profile's `run_id` is a Bureau run identity and is deliberately **not** treated as Chronik's `source.run_id` or as a `bureau_task_id`; therefore the profile currently reports exact historical recall as ineligible unless a separate caller has one of those real Chronik identities. For `deployment`, the current Chronik agent-run event contract carries no `release_id`, so the fabric likewise reports exact deployment-history selection as unsupported rather than pretending `repo_head` is a release identity. These are fail-closed identity boundaries, not missing-authority claims.

Historical recall never replaces the required live authority and cannot establish current truth, safe retry, routing or policy authority, merge readiness, deployment authorization, Bureau publication authority or task completion. Before any effect, the owning live authority is read again through the normal gate. T200 only improves historical target binding; it does not implement the later semantic enforcement gate.
