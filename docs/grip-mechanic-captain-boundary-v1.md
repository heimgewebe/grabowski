...
Target binding is action-specific: `pr-merge` requires exactly one syntactically bounded `owner/repo` slug and a positive integer `pr`; `runtime-deploy` requires one concrete `repo` or `service` plus one concrete `environment` or `runtime_target`; `service-restart` requires one concrete non-wildcard `host` and one concrete non-wildcard `unit`; `fleet-mutation` requires a concrete `fleet_target` and an explicit non-generic `operation`; `cleanup-apply` requires one concrete `cleanup_target` plus a bounded `repo` or concrete `checkout_path`. Target changes require an explicit non-empty `target_change` record; silent target switches are rejected.
...
Recovery and irreversibility records are preconditions for any later mutation. Irreversible actions require a non-empty `irreversibility_record`. These records do not prove that a later execution would be safe.
