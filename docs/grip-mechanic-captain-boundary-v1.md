...
`mechanic-loop` is the normal-action role. It may dispatch only known normal grips, and every action must carry a target object, scope object and relative receipt path. Unknown actions, missing target, missing scope, missing receipt path and high-impact actions fail closed.
