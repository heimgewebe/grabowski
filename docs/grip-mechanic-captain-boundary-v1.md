...
Status projection is mandatory evidence: `status_projection` must be a non-empty object with `status_projection_fresh=true`, an allowlisted `status_projection_source` label that matches the hash-bound `status_projection.source`, a supported `schema_version`, required typed status fields including `healthy=true`, a timezone-aware or numeric `generated_at` inside the configured max-age window with bounded clock-skew tolerance, replay-reference metadata such as `run_id`, `nonce` or `receipt_ref`, and a `status_projection_sha256` that matches the deterministic JSON hash of the projection object. A missing, stale, non-allowlisted, unhashed, replay-reference-less, unhealthy or drifted projection blocks. Caller-declared freshness is not enough by itself. Replay-reference metadata is presence-checked but not uniqueness-verified. Captain evidence must be bound to an explicit `expected_head`; review and CI evidence must match that head. The projection is evidence about observed state, never runtime truth.
