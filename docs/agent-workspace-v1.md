...
`grabowski_agent_execution_route` returns a deterministic `recommendation_id` bound to the normalized score, recommended execution mode, input facts and external-candidate plan. A new workspace may receive that recommendation as `route_evidence` together with the actual route and, when they differ, a bounded deviation reason. The create path independently replays score, route and candidate selection from normalized input facts, then recomputes the recommendation hash and rejects shape, policy, candidate or deviation drift. Existing v1 manifests without this field remain readable and closeable as `legacy_absent`; newly created workspaces without complete route evidence cannot become `success_ready` and close returns `route_evidence_incomplete` with `recreate_with_route_evidence`.
