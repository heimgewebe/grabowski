...
`grabowski_agent_execution_route` returns a deterministic `recommendation_id` bound to the direct-first policy version, risk tier, normalized score, `direct_operator` recommendation, input facts, advisory provider-level candidate plan, concrete catalog route candidates, catalog identity, Paid-Autorisierungsentscheidung and disabled parallel-writer assessment. Legacy schema-1 evidence retains its original hash contract. New workspace creation accepts only complete schema-2 evidence for an explicitly requested advisory contrast or competition route; the create path replays and rehashes the full decision before any mutation. Existing v1 manifests without this field remain readable and closeable as `legacy_absent`, but they do not authorize new external writer work. Route-bound Candidate execution is a separate evidence layer and uses schema-3 packet/manifest/receipt artifacts.
