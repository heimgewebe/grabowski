...
## Workspace-v1-Kompatibilitätsrouting v3.0 — Direct-first

Für den eingefrorenen Workspace-v1-Vertrag wird historische Schema-2-Evidenz weiterhin nach `direct-first-routing-v3.0` validiert: Dort lautet `execution_mode=direct_operator`, und diese Projektion verändert den Workspace-v1-Autoritätsumfang nicht. Das ist eine Kompatibilitätsregel des v1-Manifests. Neue systemweite Ausführungsentscheidungen werden dagegen kanonisch als Controller- oder lane-scoped-writer-Route getroffen.
...
Route-Evidenzschema 2 bindet Policy-Version, Risikostufe, die deterministische providerbasierte Workspace-Entscheidung, externe Kandidaten und den Parallel-Writer-Status an die `recommendation_id`. Für die aktuelle Policy `direct-first-routing-v3.0` bleibt die konkrete katalogbasierte Route bewusst außerhalb dieses Workspace-Hashes und wird erst im separaten Schema-3-Candidate-Vertrag gebunden. Ein neuer Agenten-Workspace verlangt vollständig verifizierte Schema-2-Evidenz der aktuellen Direct-first-Policy, eine ausdrückliche externe Anforderung, mindestens einen beratenden Kandidaten und einen zur Kandidatenzahl passenden tatsächlichen Kontrast- oder Wettbewerbsmodus. `full_workspace` ist für neue Direct-first-Erzeugungen unzulässig.
