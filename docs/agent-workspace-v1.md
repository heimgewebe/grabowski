...
Ein blockierter Preflight ist kein Rollenversuch: Der erste tatsächlich gestartete Ersatzlauf bleibt Versuch 1 und verwendet die kanonische `<rolle>-receipt.json`. Nur wenn bereits ein Rollen-Task lief, wird der Ersatzlauf Versuch 2 und schreibt `<rolle>-receipt.attempt-2.json`; das Receipt von Versuch 1 bleibt bytegenau unverändert. Das Manifest bindet zusätzlich vorherige Task-ID und Receipt-Hash, die typisierte Fehlklassifikation, alte und neue Kommando-Hashes, die neue Task-ID, die Versuchsnummer sowie den ausgewählten, für `collect` maßgeblichen finalen Versuch je Rolle. Bestehende v1-Workspaces und -Manifeste ohne diese Felder bleiben lesbar.
