...
## Workspace Routing v2.1

`grabowski_agent_execution_route` ordnet neue Aufgaben den Risikostufen R0 bis R3 zu. Kleine und bekannte Coding-Aufgaben bleiben im isolierten Worktree; ein Full Workspace ist für R2 mit substanziellem Validierungsbedarf und für R3 vorgesehen. Die frühere Eingabe `parallel_work` bleibt als Alias lesbar, wird aber auf `concurrent_external_activity` abgebildet. Laufende fremde Arbeit erhöht nur das Kollisionsrisiko. `parallelization_candidate` bewertet dagegen ausschließlich, ob eine spätere Zwei-Shard-Prüfung sinnvoll sein könnte, und verändert weder Score noch Live-Route.

Ein externer Kontrastpfad ist auf qualifizierte R3-Codeaufgaben mit echter Designunsicherheit begrenzt. Competition verlangt zusätzlich `decision_fork=true`, mindestens zwei `architecture_hypotheses` und zwei verfügbare externe Agenten. Ein hoher Komplexitätsscore allein erzeugt keine Competition. Externe Ergebnisse bleiben advisory-only; automatische Siegerwahl und Patchübernahme sind weiterhin ausgeschlossen.

Eine Parallel-Writer- oder Workspace-Group-Architektur ist nicht implementiert. Die Route kann für lange R3-Codearbeit lediglich `parallel_writer_pilot.eligible_for_assessment=true` melden. Auch dann bleiben `execution_authorized=false` und `workspace_group_implemented=false`, bis ein expliziter Zwei-Shard-Scope, Konfliktdomänen-Belege, ein Integrations-Workspace und Cross-Shard-Tests existieren.

Route-Evidenzschema 2 bindet zusätzlich Policy-Version, Risikostufe und den Parallel-Writer-Pilotstatus an die `recommendation_id`. Schema-1-Evidenz wird weiterhin mit der ursprünglichen Routinglogik validiert, damit vorhandene Workspace-Receipts lesbar bleiben. Schattenkalibrierung vergleicht nur Schema-2-Läufe derselben Runtime-Kohorte, Policy-Version, Risikostufe, Aufgabenart, Neuheit und Risikomarker; sie verändert die Live-Route nicht.

## Evidence-bound outcomes and observer v3

`grabowski_agent_execution_route` returns a deterministic `recommendation_id` bound to the route-policy version, risk tier, normalized score, recommended execution mode, input facts, external-candidate plan and parallel-writer assessment. Legacy schema-1 evidence retains its original hash contract. A new workspace may receive that recommendation as `route_evidence` together with the actual route and, when they differ, a bounded deviation reason. The create path independently replays score, route and candidate selection from normalized input facts, then recomputes the recommendation hash and rejects shape, policy, candidate or deviation drift. Existing v1 manifests without this field remain readable and closeable as `legacy_absent`; newly created workspaces without complete route evidence cannot become `success_ready` and close returns `route_evidence_incomplete` with `recreate_with_route_evidence`.
