...
Der Reentry ist zusätzlich an den aktuellen Grip-Vertrag gebunden. Seit `post-merge-sync-apply` v1.1 gehört `expected_physical_identity_sha256` zum exakten signierten Intent. Der vorausgehende `post-merge-sync`-Dry-run liefert diesen Digest aus der bestehenden `grabowski.physical_checkout_identity`: Checkout-Root, Git-Dir und Git-Common-Dir werden ohne Symlink-Following über Device/Inode gebunden und während der Erfassung revalidiert. Der Apply-Wrapper prüft den rohen lexikalischen Repository-Pfad gegen diesen Digest **vor** `Path.resolve()`; dadurch kann weder ein Symlink-Alias noch ein zwischen Planung und Apply ausgetauschtes Verzeichnis unter demselben Pfad den Replay-Reentry auf einen anderen Checkout umlenken.
...
Die terminalen Remote-, Physical- und Local-Checks sind dabei bewusst eine gebundene **Sequenz von Beobachtungen**, kein atomarer Snapshot über einen lokal und extern unabhängig veränderbaren Zustand. `remote_head_bound_observed=true` bedeutet, dass der exakt gebundene Remote-Head am vorgesehenen Remote-Readback beobachtet wurde und erfüllt damit `remote-head-bound`. Sobald danach noch Local-/Physical-Prüfungen laufen, bleibt `remote_head_verified=false`: Der Receipt behauptet ausdrücklich **nicht**, dass ein extern unabhängig veränderbarer Remote beim späteren Return weiterhin unverändert ist. Eine spätere externe Änderung ist neuer Zustand und keine Replay-Autorität; der Vertrag simuliert deshalb keine atomare Gleichzeitigkeit durch endlos alternierende Remote-/Local-Rechecks.
